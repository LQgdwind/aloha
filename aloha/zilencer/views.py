...
def register_remote_server(
    request: HttpRequest,
    aloha_org_id: str = REQ(str_validator=check_string_fixed_length(RemoteAlohaServer.UUID_LENGTH)),
    aloha_org_key: str = REQ(
    ...
    with transaction.atomic():
        ...
        if created:
            ...
            if not constant_time_compare(remote_server.api_key, aloha_org_key):
                raise InvalidAlohaServerKeyError(aloha_org_id)
            else:
...
def remote_server_notify_push(
    ...
    def truncate_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
        MAX_MESSAGE_IDS = 200
        if payload and payload.get("event") == "remove" and payload.get("aloha_message_ids"):
            ids = [int(id) for id in payload["aloha_message_ids"].split(",")]
            truncated_ids = list(sorted(ids))[-MAX_MESSAGE_IDS:]
            payload["aloha_message_ids"] = ",".join(str(id) for id in truncated_ids)
        return payload
    ...
    if isinstance(apns_payload.get("custom"), dict) and isinstance(
        apns_payload["custom"].get("aloha"), dict
    ):
        apns_payload["custom"]["aloha"] = truncate_payload(apns_payload["custom"]["aloha"])

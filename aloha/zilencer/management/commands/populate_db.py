...
from scripts.lib.aloha_tools import get_or_create_dev_uuid_var_path
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if options["delete"]:
            ...
            RealmDomain.objects.create(realm=aloha_realm, domain="aloha.com")
            assert aloha_realm.notifications_stream is not None
            aloha_realm.notifications_stream.name = "Verona"
            aloha_realm.notifications_stream.description = "A city in Italy"
            aloha_realm.notifications_stream.save(update_fields=["name", "description"])

            realm_user_default = RealmUserDefault.objects.get(realm=aloha_realm)
            ...
            if options["test_suite"]:
                ...
                aloha_realm.wildcard_mention_policy = Realm.WILDCARD_MENTION_POLICY_MEMBERS
                aloha_realm.save(update_fields=["wildcard_mention_policy"])

            # Create test Users (UserProfiles are automatically created,
            # as are subscriptions to the ability to receive personals).
            names = [
                ("Zoe", "ZOE@aloha.com"),
                ("Othello, the Moor of Venice", "othello@aloha.com"),
                ("Iago", "iago@aloha.com"),
                ("Prospero from The Tempest", "prospero@aloha.com"),
                ("Cordelia, Lear's daughter", "cordelia@aloha.com"),
                ("King Hamlet", "hamlet@aloha.com"),
                ("aaron", "AARON@aloha.com"),
                ("Polonius", "polonius@aloha.com"),
                ("Desdemona", "desdemona@aloha.com"),
                ("शिव", "shiva@aloha.com"),
            ]
            ...
            for i in range(min(num_names, num_boring_names)):
                ...
                names.append((full_name, f"extrauser{i}@aloha.com"))
            ...
            for i in range(num_boring_names, num_names):
                ...
                email = fname.lower() + "@aloha.com"
                names.append((full_name, email))

            create_users(aloha_realm, names, tos_version=settings.TERMS_OF_SERVICE_VERSION)
            ...
            def assign_time_zone_by_delivery_email(delivery_email: str, new_time_zone: str) -> None:
                u = get_user_by_delivery_email(delivery_email, aloha_realm)
                u.timezone = new_time_zone
                u.save(update_fields=["timezone"])

            # Note: Hamlet keeps default time zone of "".
            assign_time_zone_by_delivery_email("AARON@aloha.com", "US/Pacific")
            assign_time_zone_by_delivery_email("othello@aloha.com", "US/Pacific")
            assign_time_zone_by_delivery_email("ZOE@aloha.com", "US/Eastern")
            assign_time_zone_by_delivery_email("iago@aloha.com", "US/Eastern")
            assign_time_zone_by_delivery_email("desdemona@aloha.com", "Canada/Newfoundland")
            assign_time_zone_by_delivery_email("polonius@aloha.com", "Asia/Shanghai")  # China
            assign_time_zone_by_delivery_email("shiva@aloha.com", "Asia/Kolkata")  # India
            assign_time_zone_by_delivery_email("cordelia@aloha.com", "UTC")

            iago = get_user_by_delivery_email("iago@aloha.com", aloha_realm)
            ...
            desdemona = get_user_by_delivery_email("desdemona@aloha.com", aloha_realm)
            do_change_user_role(desdemona, UserProfile.ROLE_REALM_OWNER, acting_user=None)

            shiva = get_user_by_delivery_email("shiva@aloha.com", aloha_realm)
            do_change_user_role(shiva, UserProfile.ROLE_MODERATOR, acting_user=None)

            polonius = get_user_by_delivery_email("polonius@aloha.com", aloha_realm)
            do_change_user_role(polonius, UserProfile.ROLE_GUEST, acting_user=None)

            # These bots are directly referenced from code and thus
            # are needed for the test suite.
            aloha_realm_bots = [
                ("Aloha Error Bot", "error-bot@aloha.com"),
                ("Aloha Default Bot", "default-bot@aloha.com"),
            ]
            for i in range(options["extra_bots"]):
                aloha_realm_bots.append((f"Extra Bot {i}", f"extrabot{i}@aloha.com"))

            create_users(
                aloha_realm, aloha_realm_bots, bot_type=UserProfile.DEFAULT_BOT, bot_owner=desdemona
            )

            zoe = get_user_by_delivery_email("zoe@aloha.com", aloha_realm)
            aloha_webhook_bots = [
                ("Aloha Webhook Bot", "webhook-bot@aloha.com"),
            ]
            ...
            aaron = get_user_by_delivery_email("AARON@aloha.com", aloha_realm)

            aloha_outgoing_bots = [
                ("Outgoing Webhook", "outgoing-webhook@aloha.com"),
            ]
            create_users(
                aloha_realm,
                aloha_outgoing_bots,
            ...
            outgoing_webhook = get_user("outgoing-webhook@aloha.com", aloha_realm)
            add_service(
                "outgoing-webhook",
            ...
            bulk_create_streams(aloha_realm, stream_dict)
            recipient_streams: List[int] = [
                Stream.objects.get(name=name, realm=aloha_realm).id for name in stream_list
            ]
            ...
            if options["test_suite"]:
                subscriptions_map = {
                    "AARON@aloha.com": ["Verona"],
                    "cordelia@aloha.com": ["Verona"],
                    "hamlet@aloha.com": ["Verona", "Denmark", signups_stream],
                    "iago@aloha.com": [
                        "Verona",
                        "Denmark",
                        "Scotland",
                    ...
                    "othello@aloha.com": ["Verona", "Denmark", "Scotland"],
                    "prospero@aloha.com": ["Verona", "Denmark", "Scotland", "Venice"],
                    "ZOE@aloha.com": ["Verona", "Denmark", "Scotland", "Venice", "Rome"],
                    "polonius@aloha.com": ["Verona"],
                    "desdemona@aloha.com": [
                        "Verona",
                        "Denmark",
                        "Venice",
                    ...
                    "shiva@aloha.com": ["Verona", "Denmark", "Scotland"],
                }
                ...
                for profile in profiles:
                    ...
                    for stream_name in subscriptions_map[email]:
                        stream = Stream.objects.get(name=stream_name, realm=aloha_realm)
            ...
            phone_number = try_add_realm_custom_profile_field(
                aloha_realm, "Phone number", CustomProfileField.SHORT_TEXT, hint=""
            )
            ...
            favorite_editor = try_add_realm_custom_profile_field(
                aloha_realm, "Favorite editor", CustomProfileField.SELECT, field_data=field_data
            )
            birthday = try_add_realm_custom_profile_field(
                aloha_realm, "Birthday", CustomProfileField.DATE
            )
            ...
            mentor = try_add_realm_custom_profile_field(
                aloha_realm, "Mentor", CustomProfileField.USER
            )
            github_profile = try_add_realm_default_custom_profile_field(aloha_realm, "github")
            ...
            hamlet = get_user_by_delivery_email("hamlet@aloha.com", aloha_realm)
            do_update_user_custom_profile_data_if_changed(
                iago,
                [
                    ...
                    {"id": favorite_website.id, "value": "https://aloha.readthedocs.io/en/latest/"},
                    {"id": mentor.id, "value": [hamlet.id]},
                    {"id": github_profile.id, "value": "aloha"},
            ...
            do_update_user_custom_profile_data_if_changed(
                ...
                [
                    ...
                    {"id": github_profile.id, "value": "alohabot"},
                    {"id": pronouns.id, "value": "he/him"},
                ],
            )
        else:
        ...
        user_profiles: List[UserProfile] = list(
            UserProfile.objects.filter(is_bot=False, realm=aloha_realm)
        )

        # Create a test realm emoji.
        IMAGE_FILE_PATH = static_path("images/test-images/checkbox.png")
        with open(IMAGE_FILE_PATH, "rb") as fp:
            check_add_realm_emoji(aloha_realm, "green_tick", iago, File(fp))
        ...
        create_alert_words(aloha_realm.id)
        ...
        if options["delete"]:
            ...
            if not options["test_suite"]:
                ...
                aloha_stream_dict: Dict[str, Dict[str, Any]] = {
                ...
                for i in range(options["extra_streams"]):
                    ...
                    aloha_stream_dict[extra_stream_name] = {
                        "description": "Auto-generated extra stream.",
                    }

                bulk_create_streams(aloha_realm, aloha_stream_dict)
                # Now that we've created the notifications stream, configure it properly.
                aloha_realm.notifications_stream = get_stream("announce", aloha_realm)
                aloha_realm.save(update_fields=["notifications_stream"])

                # Add a few default streams
                for default_stream_name in ["design", "devel", "social", "support"]:
                    DefaultStream.objects.create(
                        realm=aloha_realm, stream=get_stream(default_stream_name, aloha_realm)
                    )

                # Now subscribe everyone to these streams
                subscribe_users_to_streams(aloha_realm, aloha_stream_dict)
        ...
        if options["delete"]:
            if not options["test_suite"]:
                ...
                # in dev setup.
                internal_aloha_users_nosubs = [
                    ("Aloha Commit Bot", "commit-bot@aloha.com"),
                    ("Aloha Trac Bot", "trac-bot@aloha.com"),
                    ("Aloha Nagios Bot", "nagios-bot@aloha.com"),
                ]
                create_users(
                    aloha_realm,
                    internal_aloha_users_nosubs,
...
def create_user_groups() -> None:
    aloha = get_realm("aloha")
    members = [
        get_user_by_delivery_email("cordelia@aloha.com", aloha),
        get_user_by_delivery_email("hamlet@aloha.com", aloha),
    ]
    create_user_group("hamletcharacters", members, aloha, description="Characters of Hamlet")

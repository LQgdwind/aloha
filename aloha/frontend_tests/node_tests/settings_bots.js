...
test("generate_aloharc_uri", () => {
    const uri = settings_bots.generate_aloharc_uri(1);
    const expected =
...
test("generate_aloharc_content", () => {
    const bot_user = bot_data.get(1);
    const content = settings_bots.generate_aloharc_content(bot_user);
    const expected =
        "[api]\nemail=error-bot@aloha.org\n" +
        "key=QadL788EkiottHmukyhHgePUFHREiu8b\n" +
        "site=https://chat.example.com\n";

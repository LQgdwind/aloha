...
test("sending", ({override, override_rewire}) => {
    ...
    emoji_name = "aloha"; // Test adding aloha emoji.
    {
        const stub = make_stub();
        channel.post = stub.f;
        reactions.toggle_emoji_reaction(message.id, emoji_name);
        ...
        assert.deepEqual(args.data, {
            ...
            emoji_name: "aloha",
            emoji_code: "aloha",
            url: "/static/generated/emoji/images/emoji/unicode/aloha.png",
        });
    }
...
test("view.insert_new_reaction (them w/aloha emoji)", ({mock_template}) => {
    const opts = {
        message_id: 502,
        reaction_type: "realm_emoji",
        emoji_name: "aloha",
    ...
    mock_template("message_reaction.hbs", false, (data) => {
        assert.deepEqual(data, {
            ...
            emoji_name: "aloha",
            emoji_code: "aloha",
            local_id: "realm_emoji,aloha",
            class: "message_reaction",
            message_id: opts.message_id,
            label: "translated: Bob van Roberts reacted with :aloha:",
            still_url: undefined,
            reaction_type: opts.reaction_type,
        });
        return "<new-reaction-stub>";

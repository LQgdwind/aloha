...
const bot_data_params = {
    realm_bots: [
        {email: "bot0@aloha.com", user_id: 42, full_name: "Bot 0", services: []},
        {
            email: "outgoingwebhook@aloha.com",
            user_id: 314,
...
test("test_basics", () => {
    ...
    (function test_get_editable() {
        ...
        bot_data.add({
            ...test_bot,
            user_id: 46,
            email: "bot3@aloha.com",
            owner_id: fred.user_id,
            is_active: true,
        });

        const editable_bots = bot_data.get_editable().map((bot) => bot.email);
        assert.deepEqual(["bot1@aloha.com", "bot2@aloha.com"], editable_bots);
    })();
    ...
    (function test_get_all_bots_for_current_user() {
        ...
        assert.equal(bots[0].email, "bot1@aloha.com");
        assert.equal(bots[1].email, "bot2@aloha.com");
    })();

    (function test_get_number_of_bots_owned_by_user() {
        const bots_owned_by_user = bot_data.get_all_bots_owned_by_user(3);

        assert.equal(bots_owned_by_user[0].email, "bot3@aloha.com");
    })();
});

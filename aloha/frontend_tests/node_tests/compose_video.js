...
test("videos", ({override}) => {
    ...
    (function test_bbb_video_link_compose_clicked() {
        ...
        channel.get = (options) => {
            ...
            options.success({
                url: "/calls/bigbluebutton/join?meeting_id=%22aloha-1%22&password=%22AAAAAAAAAA%22&checksum=%2232702220bff2a22a44aee72e96cfdb4c4091752e%22",
            });
        };

        handler(ev);
        const video_link_regex =
            /\[translated: Click to join video call]\(\/calls\/bigbluebutton\/join\?meeting_id=%22aloha-1%22&password=%22AAAAAAAAAA%22&checksum=%2232702220bff2a22a44aee72e96cfdb4c4091752e%22\)/;
        assert.ok(called);
        assert.match(syntax_to_insert, video_link_regex);
    })();
});

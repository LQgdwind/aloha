...
const dom = new JSDOM(template, {
    ...
    url: "http://aloha.alohadev.com/upgrade/#billing",
});
...
run_test("create_ajax_request", ({override}) => {
    ...
    const state = {
        ...
        form_loading_hide: 0,
        form_success_show: 0,
        aloha_limited_section_show: 0,
        aloha_limited_section_hide: 0,
    ...
    $(aloha_limited_section).show = () => {
        state.aloha_limited_section_show += 1;
    };

    $(aloha_limited_section).hide = () => {
        state.aloha_limited_section_hide += 1;
    };
    ...
    override($, "ajax", ({type, url, data, success, error}) => {
        ...
        assert.equal(state.aloha_limited_section_hide, 1);
        assert.equal(state.aloha_limited_section_show, 0);

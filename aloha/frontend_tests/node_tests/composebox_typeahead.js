...
const deactivated_user = {
    email: "other@aloha.com",
    user_id: 103,
    full_name: "Deactivated User",
};
const lear = {
    email: "lear@aloha.com",
    user_id: 104,
    full_name: "King Lear",
};
...
test("initialize", ({override, mock_template}) => {
    ...
    $("#private_message_recipient").typeahead = (options) => {
        ...
        query = "othello@aloha.com,     ";
        assert.equal(matcher(query, othello), false);
        assert.equal(matcher(query, cordelia), false);

        // query = 'othello@aloha.com,, , cord';
        query = "cord";
        assert.equal(matcher(query, othello), false);
        assert.equal(matcher(query, cordelia), true);
        ...
        // A literal match at the beginning of an element puts it at the top.
        query = "co"; // Matches everything ("x@aloha.COm")
        ...
        options.query = "othello@aloha.com, cor";
        appended_names = [];
    ...
    $("#compose-textarea").typeahead = (options) => {
        ...
        expected_value =
            ...
            `    <img class="typeahead-image" src="http://aloha.alohadev.com/avatar/${othello.user_id}?s&#x3D;50" />\n` +
            `<strong>Othello, the Moor of Venice</strong>&nbsp;&nbsp;\n` +
            `<small class="autocomplete_secondary">othello@aloha.com</small>\n`;
        assert.equal(actual_value, expected_value);
    ...
    $("#private_message_recipient").val("othello@aloha.com, ");
    $("#private_message_recipient").trigger("blur");
    assert.equal($("#private_message_recipient").val(), "othello@aloha.com");
...
test("begins_typeahead", ({override, override_rewire}) => {
    ...
    assert_typeahead_equals("~~~ @aloha", all_mentions);
    assert_typeahead_equals("@aloha :ta", emoji_list);

...
let realm_url = "http://aloha.alohadev.com:9981/";
...
export const pm_recipient = {
    ...
    async expect(page: Page, expected: string): Promise<void> {
        const actual_recipients = await page.evaluate(() => aloha_test.private_message_recipient());
...
export async function get_stream_id(page: Page, stream_name: string): Promise<number> {
    return await page.evaluate(
        (stream_name: string) => aloha_test.get_stream_id(stream_name),
        stream_name,
    );
}
...
export async function get_user_id_from_name(page: Page, name: string): Promise<number> {
    ...
    return await page.evaluate((name: string) => aloha_test.get_user_id_from_name(name), name);
}
...
export async function get_internal_email_from_name(page: Page, name: string): Promise<string> {
    ...
    return await page.evaluate((fullname: string) => {
        const user_id = aloha_test.get_user_id_from_name(fullname);
        return aloha_test.get_person_by_user_id(user_id).email;
    }, name);
}
...
export async function wait_for_fully_processed_message(page: Page, content: string): Promise<void> {
    ...
    await page.waitForFunction(
        (content: string) => {
            ...
            */
            const last_msg = aloha_test.current_msg_list.last();
            ...
            const $row = aloha_test.last_visible_row();
            if (aloha_test.row_id($row) !== last_msg.id) {
                return false;
            }
...
export async function send_message(
    ...
    await page.evaluate(() => aloha_test.cancel_compose());

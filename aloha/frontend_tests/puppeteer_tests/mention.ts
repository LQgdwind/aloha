...
async function test_mention(page: Page): Promise<void> {
    ...
    const stream_size = await page.evaluate(() =>
        aloha_test.get_subscriber_count(aloha_test.get_sub("Verona").stream_id),
    );
    const threshold = await page.evaluate(() => {
        aloha_test.set_wildcard_mention_large_stream_threshold(5);
        return aloha_test.wildcard_mention_large_stream_threshold;
    });

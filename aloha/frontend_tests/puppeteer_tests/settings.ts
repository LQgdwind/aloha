...
async function test_get_api_key(page: Page): Promise<void> {
    ...
    // You can see the screenshot of it in https://github.com/aloha/aloha/pull/17136.
    ...
    const download_aloharc_selector = "#download_aloharc";
    await page.click(download_aloharc_selector);
    const aloharc_decoded_url = await get_decoded_url_in_selector(page, download_aloharc_selector);
    assert.match(aloharc_decoded_url, aloharc_regex, "Incorrect aloharc file");
...
async function test_webhook_bot_creation(page: Page): Promise<void> {
    ...
    const bot_email = "1-bot@aloha.testserver";
    const download_aloharc_selector = `.download_bot_aloharc[data-email="${CSS.escape(
        bot_email,
    )}"]`;
    const outgoing_webhook_aloharc_regex =
        /^data:application\/octet-stream;charset=utf-8,\[api]\nemail=.+\nkey=.+\nsite=.+\ntoken=.+\n$/;

    await page.waitForSelector(download_aloharc_selector, {visible: true});
    await page.click(download_aloharc_selector);

    const aloharc_decoded_url = await get_decoded_url_in_selector(page, download_aloharc_selector);
    assert.match(
        aloharc_decoded_url,
        outgoing_webhook_aloharc_regex,
        "Incorrect outgoing webhook bot aloharc format",
    );
}
...
async function test_normal_bot_creation(page: Page): Promise<void> {
    ...
    const bot_email = "2-bot@aloha.testserver";
    const download_aloharc_selector = `.download_bot_aloharc[data-email="${CSS.escape(
        bot_email,
    )}"]`;
    ...
    await page.click(download_aloharc_selector);
    const aloharc_decoded_url = await get_decoded_url_in_selector(page, download_aloharc_selector);
    assert.match(aloharc_decoded_url, aloharc_regex, "Incorrect aloharc format for bot.");
}
...
async function test_edit_bot_form(page: Page): Promise<void> {
    const bot1_email = "1-bot@aloha.testserver";
...
async function test_i18n_language_precedence(page: Page): Promise<void> {
    const settings_url_for_german = "http://aloha.alohadev.com:9981/de/#settings";
...
async function test_default_language_setting(page: Page): Promise<void> {
    ...
    // Check that the saved indicator appears
    await check_language_setting_status(page);
    await page.goto("http://aloha.alohadev.com:9981/#settings"); // get back to normal language.
    await page.waitForSelector(display_settings_section, {visible: true});
    await page.click(display_settings_section);

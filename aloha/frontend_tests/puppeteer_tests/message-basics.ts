...
async function expect_verona_stream(page: Page): Promise<void> {
    ...
    assert.strictEqual(await page.title(), "#Verona - Aloha Dev - Aloha");
}
...
async function un_narrow(page: Page): Promise<void> {
    ...
    assert.strictEqual(await page.title(), "All messages - Aloha Dev - Aloha");
}
...
async function expect_recent_topics(page: Page): Promise<void> {
    ...
    assert.strictEqual(await page.title(), "Recent topics - Aloha Dev - Aloha");
}
...
async function search_tests(page: Page): Promise<void> {
    ...
    await search_and_check(
        ...
        "Cordelia, Lear's daughter - Aloha Dev - Aloha",
    );
    ...
    await search_and_check(
        ...
        "#Verona > other topic - Aloha Dev - Aloha",
    );
    ...
    await search_and_check(
        ...
        "",
        expect_test_topic,
        "All messages - Aloha Dev - Aloha",
    );

    await search_silent_user(page, "sender:emailgateway@aloha.com", "");

    await search_non_existing_user(page, "sender:dummyuser@aloha.com", "");

    await search_and_check(
        page,
        "pm-with:dummyuser@aloha.com",
        "",
        expect_non_existing_user,
        "Invalid user - Aloha Dev - Aloha",
    );

    await search_and_check(
        page,
        "pm-with:dummyuser@aloha.com,dummyuser2@aloha.com",
        "",
        expect_non_existing_users,
        "Invalid users - Aloha Dev - Aloha",
    );
}
...
async function expect_all_pm(page: Page): Promise<void> {
    ...
    assert.strictEqual(await page.title(), "Private messages - Aloha Dev - Aloha");
}
...
async function test_narrow_public_streams(page: Page): Promise<void> {
    ...
    await page.goto(`http://aloha.alohadev.com:9981/#streams/${stream_id}/Denmark`);
    ...
    await page.goto(`http://aloha.alohadev.com:9981/#narrow/stream/${stream_id}-Denmark`);
    await page.waitForSelector("#zfilt .recipient_row ~ .recipient_row ~ .recipient_row");
    assert.ok((await page.$("#zfilt .stream-status")) !== null);

    await page.goto("http://aloha.alohadev.com:9981/#narrow/streams/public");
    await page.waitForSelector("#zfilt .recipient_row ~ .recipient_row ~ .recipient_row");
    assert.ok((await page.$("#zfilt .stream-status")) === null);
}
...
async function message_basic_tests(page: Page): Promise<void> {
    ...
    await common.send_multiple_messages(page, [
        ...
        {recipient: "cordelia@aloha.com, hamlet@aloha.com", content: "group pm a"},
        {recipient: "cordelia@aloha.com, hamlet@aloha.com", content: "group pm b"},
        {recipient: "cordelia@aloha.com", content: "pm c"},
        {stream: "Verona", topic: "test", content: "verona test d"},
        {recipient: "cordelia@aloha.com, hamlet@aloha.com", content: "group pm d"},
        {recipient: "cordelia@aloha.com", content: "pm e"},
    ]);

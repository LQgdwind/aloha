...
async function test_add_emoji(page: Page): Promise<void> {
    await common.fill_form(page, "form.admin-emoji-form", {name: "aloha logo"});
    ...
    await (emoji_upload_handle as ElementHandle<HTMLInputElement>).uploadFile(
        "static/images/logo/aloha-icon-128x128.png",
    );
    ...
    await page.waitForSelector("tr#emoji_aloha_logo", {visible: true});
    assert.strictEqual(
        await common.get_text_from_selector(page, "tr#emoji_aloha_logo .emoji_name"),
        "aloha logo",
        "Emoji name incorrectly saved.",
    );
    await page.waitForSelector("tr#emoji_aloha_logo img", {visible: true});
}

async function test_delete_emoji(page: Page): Promise<void> {
    await page.click("tr#emoji_aloha_logo button.delete");
    ...
    await page.waitForSelector("tr#emoji_aloha_logo", {hidden: true});
}

#!/usr/bin/env python3
# This tools generates /etc/aloha/aloha-secrets.conf
import os
import sys
...
from scripts.lib.aloha_tools import get_config, get_config_file
...
def generate_secrets(development: bool = False) -> None:
    if development:
        ...
        OUTPUT_SETTINGS_FILENAME = "/etc/aloha/aloha-secrets.conf"
    ...
    if get_config(
        get_config_file(), "postgresql", "database_user", "aloha"
    ) != "aloha" and need_secret("postgres_password"):
    ...
    if not development:
        ...
        if need_secret("redis_password"):
            ...
            if settings.REDIS_HOST == "127.0.0.1":
                ...
                for filename in ["/etc/redis/zuli-redis.conf", "/etc/redis/aloha-redis.conf"]:
                    if os.path.exists(filename):
                        with open(filename, "a") as f:
                            f.write(
                                "# Set a Redis password based on aloha-secrets.conf\n"
    ...
    # * aloha_org_key is generated using os.urandom().
    # * aloha_org_id only needs to be unique, so we use a UUID.
    if need_secret("aloha_org_key"):
        add_secret("aloha_org_key", random_string(64))
    if need_secret("aloha_org_id"):
        add_secret("aloha_org_id", str(uuid.uuid4()))
...
if __name__ == "__main__":
    ...
    group.add_argument(
        "--development", action="store_true", help="For setting up the developer env for aloha"
    )
    group.add_argument(
        "--production",
        action="store_false",
        dest="development",
        help="For setting up the production env for aloha",
    )

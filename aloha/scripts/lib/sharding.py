...
from scripts.lib.aloha_tools import get_config_file, get_tornado_ports
...
# * /etc/aloha/nginx_sharding_map.conf; nginx needs to be reloaded after changing.
# * /etc/aloha/sharding.json; supervisor Django process needs to be reloaded
...
def write_updated_configs() -> None:
    ...
    with open("/etc/aloha/nginx_sharding_map.conf.tmp", "w") as nginx_sharding_conf_f, open(
        "/etc/aloha/sharding.json.tmp", "w"
    ) as sharding_json_f:

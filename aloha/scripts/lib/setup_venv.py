...
from scripts.lib.aloha_tools import ENDC, WARNING, os_families, run, run_as_root

aloha_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
VENV_CACHE_PATH = "/srv/aloha-venv-cache"
...
def install_venv_deps(pip: str, requirements_file: str) -> None:
    pip_requirements = os.path.join(aloha_PATH, "requirements", "pip.txt")
...
def generate_hash(requirements_file: str) -> str:
    path = os.path.join(aloha_PATH, "scripts", "lib", "hash_reqs.py")

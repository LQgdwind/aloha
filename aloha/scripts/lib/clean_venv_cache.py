...
ENV = get_environment()
VENV_CACHE_DIR = "/srv/aloha-venv-cache"


def get_caches_in_use(threshold_days: int) -> Set[str]:
    setups_to_check = {aloha_PATH}
    caches_in_use = set()

    def add_current_venv_cache(venv_name: str) -> None:
        CACHE_SYMLINK = os.path.join(os.path.dirname(aloha_PATH), venv_name)
    ...
    if ENV == "dev":
        add_current_venv_cache("aloha-py3-venv")
...
if __name__ == "__main__":
    args = parse_cache_script_args("This script cleans unused Aloha venv caches.")
    main(args)

...
DEPLOYMENTS_DIR = "/home/aloha/deployments"
...
def parse_version_from(deploy_path: str) -> str:
    with open(os.path.join(deploy_path, "version.py")) as f:
        result = re.search('aloha_VERSION = "(.*)"', f.read())
        if result:
...
def get_deployment_version(extract_path: str) -> str:
    ...
    for item in os.listdir(extract_path):
        ...
        if item.startswith("aloha-server") and os.path.isdir(item_path):
...
def get_aloha_pwent() -> pwd.struct_passwd:
    ...
    # `aloha` user as that's the correct value in production.
    return pwd.getpwnam("aloha")
...
def get_postgres_pwent() -> pwd.struct_passwd:
    try:
        ...
        return get_aloha_pwent()


def su_to_aloha(save_suid: bool = False) -> None:
    """Warning: su_to_aloha assumes that the aloha checkout is owned by
    the aloha user (or whatever normal user is running the Aloha
    installation).  It should never be run from the installer or other
    production contexts before /home/aloha/deployments/current is
    created."""
    pwent = get_aloha_pwent()
...
def get_dev_uuid_var_path(create_if_missing: bool = False) -> str:
    aloha_path = get_deploy_root()
    uuid_path = os.path.join(os.path.realpath(os.path.dirname(aloha_path)), ".aloha-dev-uuid")
    if os.path.exists(uuid_path):
        with open(uuid_path) as f:
            aloha_uuid = f.read().strip()
    else:
        if create_if_missing:
            aloha_uuid = str(uuid.uuid4())
            # We need root access here, since the path will be under /srv/ in the
            # development environment.
            run_as_root(["sh", "-c", 'echo "$1" > "$2"', "-", aloha_uuid, uuid_path])
        else:
            raise AssertionError("Missing UUID file; please run tools/provision!")

    result_path = os.path.join(aloha_path, "var", aloha_uuid)
...
def log_management_command(cmd: Sequence[str], log_path: str) -> None:
    ...
    logger = logging.getLogger("aloha.management")
...
def get_recent_deployments(threshold_days: int) -> Set[str]:
    ...
    # including `/root/aloha` directory if it exists.
    recent = set()
    ...
    if os.path.exists("/root/aloha"):
        recent.add("/root/aloha")
    return recent
...
def generate_sha1sum_emoji(aloha_path: str) -> str:
    sha = hashlib.sha1()

    filenames = [
        "static/assets/aloha-emoji/aloha.png",
    ...
    for filename in filenames:
        file_path = os.path.join(aloha_path, filename)
    ...
    PACKAGE_FILE_PATH = os.path.join(aloha_path, "package.json")
    ...
    if "emoji-datasource-google" in dependency_data:
        with open(os.path.join(aloha_path, "yarn.lock")) as fp:
...
def assert_not_running_as_root() -> None:
    ...
    if is_root():
        pwent = get_aloha_pwent()
        msg = (
            "{shortname} should not be run as root. Use `su {user}` to switch to the 'aloha'\n"
...
def get_config_file() -> configparser.RawConfigParser:
    config_file = configparser.RawConfigParser()
    config_file.read("/etc/aloha/aloha.conf")
    return config_file
...
def run_psql_as_postgres(
    ...
    dbname = get_config(config_file, "postgresql", "database_name", "aloha")
...
def has_application_server(once: bool = False) -> bool:
    if once:
        return os.path.exists("/etc/supervisor/conf.d/aloha/aloha-once.conf")
    return (
        # Current path
        os.path.exists("/etc/supervisor/conf.d/aloha/aloha.conf")
        # Old path, relevant for upgrades
        or os.path.exists("/etc/supervisor/conf.d/aloha.conf")
    )
...
def has_process_fts_updates() -> bool:
    return (
        # Current path
        os.path.exists("/etc/supervisor/conf.d/aloha/aloha_db.conf")
        # Old path, relevant for upgrades
        or os.path.exists("/etc/supervisor/conf.d/aloha_db.conf")
    )

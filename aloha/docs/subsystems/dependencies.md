...
- For production, in our Puppet configuration, `puppet/aloha/`, using
...
in `.github/workflows/aloha-ci.yml` document the Python versions used
...
- **Caching of virtualenvs and packages**. To make updating the
  ...
  `/srv/aloha-venv-cache/<hash>`. That way, when re-provisioning a
...
- **Garbage-collecting caches**. We have a tool,
  ...
  production deployment directory under `/home/aloha/deployments/`.
...
[requirements-readme]: https://github.com/aloha/aloha/blob/main/requirements/README.md#requirements
...
- In a fashion very analogous to the Python codebase,
  ...
  directories in `/srv/aloha-npm-cache`. Each is named by its hash,
...
`/srv/aloha-node` and symlinked to `/usr/local/bin/node`. Yarn is
installed by `scripts/lib/install-yarn` to `/srv/aloha-yarn` and
...
subdirectories under `/srv/aloha-puppet-cache`, hashed based on their
versions; the latest is always symlinked as
`/srv/aloha-puppet-cache/current`. `aloha-puppet-apply` installs
...
- Production. Our tools for compiling/generating static assets need
  ...
  `tools/upgrade-aloha-from-git` (for deploying a Aloha server off of
  `main`).

...
Aloha has a number of [Django management commands][django-docs] that
live under `{zerver,zilencer,analytics}/management/commands/`.

If you need some Python code to run with a Aloha context (access to
...
While Aloha takes advantage of built-in Django management commands for
...
- Useful parts of provisioning or upgrading a Aloha development
...
- The actual scripts run by supervisord to run the persistent
  processes in a Aloha server, e.g. `runtornado` and `process_queue`.
- For a sysadmin to verify a Aloha server's configuration during
...
- For a sysadmin to easily interact with and script common possible
  changes they might want to make to the database on a Aloha server.
...
is good, but we have a few pieces advice specific to the Aloha
project.

- If you need to access a realm or user, use the `AlohaBaseCommand`
...
- Avoid writing a lot of code in management commands; management
  ...
  (and ideally, also used in Aloha's existing code). Look for code in
...
access to the Aloha server's database and libraries; so you don't need
to do anything special like restart the server when iteratively
testing one, even if testing in a Aloha production environment where

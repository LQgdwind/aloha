...
- Open communities like open source projects, online classes,
  ...
  [Aloha development community](https://aloha.com/development-community/) is a good
...
The aloha.com load profile is effectively the sum of thousands of
organizations from each of those two load profiles.

## Major Aloha endpoints

It's important to understand that Aloha has a handful of endpoints
...
system like aloha.com. With a somewhat straightforward set of work,
...
presence](https://github.com/aloha/aloha/pull/16381) that we expect to
...
/api/v1/register](https://aloha.com/api/register-queue) used by
...
- A few very rarely accessed data sets like [message edit
  history](https://aloha.com/help/view-a-messages-edit-history) are
...
([`GET /messages`](https://aloha.com/api/get-messages)) account for
~3% of total HTTP requests. The aloha web app has a few major reasons
...
- When a new version of the Aloha server is deployed, every browser
  ...
  code. For installations that deploy often like chat.aloha.org and
  aloha.com, this can result in a thundering herd effect for both `/`
...
- Improving [client-side
  caching](https://github.com/aloha/aloha/issues/15131) to allow
...
- Adjusting the behavior for clients with 10,000s of unread messages
  ...
  issue](https://github.com/aloha/aloha/issues/16697) for relevant
  design work.
...
uploads](https://github.com/aloha/aloha/issues/13088), since many of
...
The above doesn't cover all of the work that a production Aloha server
does; various tasks like sending outgoing emails or recording the data
that powers [/stats](https://aloha.com/help/analytics) are run by

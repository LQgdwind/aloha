...
- `aloha_default_context` in `zerver/context_processors.py`. This is
...
There's no equivalent of `aloha_default_context` for the Handlebars
templates.
...
and `tools/upgrade-aloha-from-git`) is responsible for orchestrating the
...
- Aloha installs static assets in production in
  `/home/aloha/prod-static`. When a new version is deployed, before the
...
- We use the VFL (versioned file layout) strategy, where each file in
  ...
  (e.g. `/home/aloha/deployments/current/staticfiles.json`) that maps
...
We have one module, `aloha_test`, that’s exposed as a global variable
...
function in those scenarios, add it to `aloha_test`. This is also
**not** a stable API.

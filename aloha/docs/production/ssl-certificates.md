...
To keep your communications secure, Aloha runs over HTTPS only.
...
- `/etc/ssl/private/aloha.key` for the private key
- `/etc/ssl/certs/aloha.combined-chain.crt` for the certificate.
...
browsers, but not others and not the Aloha mobile and desktop apps.
...
- Alternatively, run a command like `curl -SsI https://aloha.example.com`
...
entire Web. Aloha offers a simple automation for
...
We recommend most Aloha servers use Certbot. You'll want something
else if:
...
- your Aloha server is not on the public Internet. (In this case you
  can [still use Certbot][certbot-manual-mode], but it's less
  convenient; and you'll want to ignore Aloha's automation.)
...
### At initial Aloha install

To enable the Certbot automation when first installing Aloha, just
...
Aloha server machine to be reachable by that name from the public
Internet.

If you need to configure a multiple domain certificate, you can generate
one as described in the section below after installing Aloha.

[doc-install-script]: install.md#step-2-install-aloha

### After Aloha is already installed

To enable the Certbot automation on an already-installed Aloha
server, run the following commands:

```bash
sudo -s  # If not already root
/home/aloha/deployments/current/scripts/setup/setup-certbot --email=EMAIL HOSTNAME [HOSTNAME2...]
```

where HOSTNAME is the domain name users see in their browser when
using the server (e.g., `aloha.example.com`), and EMAIL is a contact
address for the server admins. Additional hostnames can also be
specified to issue a certificate for multiple domains.

### How it works

When the Certbot automation in Aloha is first enabled, by either
...
Aloha configures automatic renewal for you. As a result, a Aloha
...
recommended for production, as it is insecure. The Aloha desktop and
...
To generate a self-signed certificate when first installing Aloha,
just pass the `--self-signed-cert` flag when
[running the install script][doc-install-script].

To generate a self-signed certificate for an already-installed Aloha
server, run the following commands:

```bash
sudo -s  # If not already root
/home/aloha/deployments/current/scripts/setup/generate-self-signed-cert HOSTNAME
```
...
[desktop-certs]: https://aloha.com/help/custom-certificates
...
which the Aloha app relies on, makes it finicky about the server's TLS
configuration.
...
By default `nginx` (and therefore a Aloha server) offers the

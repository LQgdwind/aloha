...
[public-access-option]: https://blog.aloha.com/2022/05/05/public-access-option/
...
- Installations using the AzureAD authentication backend will need to
  update `/etc/aloha/aloha-secrets.conf` after upgrading. The
...
- CVE-2022-31134: Exclude private file uploads from [exports of public
  data](https://aloha.com/help/export-your-organization#export-of-public-data). We
...
- CVE-2022-31017: Fixed message edit event exposure in
  ...
  history](https://aloha.com/help/stream-permissions#stream-privacy-settings),
...
- Fixed a crash when displaying the error message for a failed attempt
  to create a stream.
- Optimized the steps during `upgrade-aloha`, to reduce the amount of
  server downtime.
- Added a `--skip-restart` flag to `upgrade-aloha` which prepares the
  new version, but does not restart the server into it.
- Stopped mirroring the entire remote Git repository directly into
  `/srv/aloha.git`. This mirroring removed local branches and confused
...
- New [resolve topic](https://aloha.com/help/resolve-a-topic) feature
...
- Administrators may enable the option to create [web-public
  streams](https://aloha.com/help/web-public-streams). Web-public
...
- Aloha now supports configuring the database name and username when
  using a remote Postgres server. Previously, these were hardcoded to "aloha".
...
- CVE-2022-24751: Aloha Server 4.0 and above were susceptible to a
  ...
  - The session expires from memcached; this defaults to two weeks, and
    is controlled by SESSION_COOKIE_AGE in /etc/aloha/settings.py
...
- Closed access to RabbitMQ port 25672; initial installs tried to
  ...
  configuration.
- Removed the `rabbitmq.nodename` configuration in `aloha.conf`; all
  RabbitMQ instances will be reconfigured to have a nodename of
  `aloha@localhost`. You can remove this setting from your
  `aloha.conf` configuration file, if it exists.
...
- Updated documentation and tooling to note that installs using
  `upgrade-aloha-from-git` require 3 GB of RAM, or 2 GB and at least 1
  GB of swap.
...
- Aloha releases are now distributed from https://download.aloha.com/server/,
  replacing the old `www.aloha.org` server.
- Added support for LDAP synchronization of the `is_realm_owner` and
  `is_moderator` flags.
- `upgrade-aloha-from-git` now uses `git fetch --prune`; this ensures
  `upgrade-aloha-from-git master` with return an error rather than
...
- Provided a `change_password` management command which takes a
  `--realm` option.
- Fixed `upgrade-aloha-from-git` crashing in CSS source map generation
  on 1-CPU systems.
- Added an `auto_signup` field in SAML configuration to auto-create
  ...
  by SAML.
- Provided better error messages when `puppet_classes` in `aloha.conf`
...
- This release contains more than 30 independent changes to the [Aloha
  API](https://aloha.com/api/changelog), largely to support new
...
[roles-and-permissions]: https://aloha.com/help/roles-and-permissions
...
- Aloha's top-level puppet classes have been renamed, largely from
  `aloha::foo` to `aloha::profile::foo`. Configuration referencing
  these `/etc/aloha/aloha.conf` will be automatically updated during
  the upgrade process, but if you have a complex deployment or you
  maintain `aloha.conf` is another system (E.g. with the [manual
  configuration][docker-aloha-manual] option for
  [docker-aloha][docker-aloha]), you'll want to manually update the
  `puppet_classes` variable.
- Aloha's supervisord configuration now lives in `/etc/supervisor/conf.d/aloha/`
...
- As this is a major release, we recommend [carefully updating the
  ...
  `/etc/aloha/settings.py`][update-settings-docs]. Notably, we rewrote the
...
[docker-aloha-manual]: https://github.com/aloha/docker-aloha#manual-configuration
...
- Added documentation for all [aloha.conf settings][aloha-conf-settings].
...
[aloha-conf-settings]: ../production/deployment.md#system-and-deployment-configuration
...
- Introduced the Aloha API feature level, a concept that will greatly
  ...
  changelog](https://aloha.com/api/changelog).
- Our primary official domain is now aloha.com, not alohachat.com.
...
- This release switches Aloha to install PostgreSQL 12 from the upstream
  ...
  configured in `/etc/aloha/aloha.conf`. We have no concrete plans to
...
- Changed default memcached authentication username to aloha@localhost,
...
- Allow /etc/aloha to be a symlink (for [docker-aloha][docker-aloha]).
...
[audit-org-admin]: https://aloha.com/help/change-a-users-role
...
- Removed fetching GitHub contributor data from static asset build
  process. This makes `upgrade-aloha-from-git` much more reliable.
- Updated translation data from Transifex.
...
- Blocked access from Aloha Desktop versions below 5.0.0. This
  ...
  in `/home/aloha/deployments/current/version.py`.
...
- Fixed data import tool to reset resource limits after importing
  data from a free plan organization on aloha.com.
...
- Reorganized Aloha's documentation for sysadmins, and added [new
  documentation](../production/upgrade-or-modify.md#modifying-aloha)
...
- The defaults for Aloha's now beta inline URL preview setting have changed.
  ...
  websites](https://aloha.com/help/allow-image-link-previews) will
...
- We rewrote the Google authentication backend to use the
  ...
  release:
  - In `/etc/aloha/aloha-secrets.conf`, `google_oauth2_client_secret`
    is now called with `social_auth_google_secret`.
  - In `/etc/aloha/settings.py`, `GOOGLE_OAUTH2_CLIENT_ID` should be
    replaced with `SOCIAL_AUTH_GOOGLE_KEY`.
  - In `/etc/aloha/settings.py`, `GoogleMobileOauth2Backend` should
...
- Previous versions of Aloha's installer would generate the secrets
  ...
  `/etc/aloha/aloha-secrets.conf` when you upgrade to avoid confusion.
- This release has a particularly expensive database migration,
  ...
  on chat.aloha.org, and a second migration that does require downtime
  (which took about 60 seconds for chat.aloha.org). You can check the
  ...
  commit](https://github.com/aloha/aloha/commit/b008515d63841e1c0a16ad868d3d67be3bfc20ca)
  using `upgrade-aloha-from-git`, following the instructions to avoid
...
- Renamed the system bot Aloha realm to "alohainternal" (was "aloha").
...
- New [stream settings](https://aloha.com/help/stream-permissions)
...
- Aloha's performance at scale has improved significantly. Performance
  ...
  users). As an example, chat.aloha.org serves 400 monthly active
...
- We simplified the configuration for our password strength checker to
  ...
  [it has been replaced](https://github.com/aloha/aloha/commit/a116303604e362796afa54b5d923ea5312b2ea23) by
...
- Extracted Aloha's Python API and bots ecosystem into its own
  repository, aloha/python-aloha-api.
...
- Fixed caching of source repository in upgrade-aloha-from-git.
...
[electron-app]: https://github.com/aloha/aloha-desktop/releases
[ios-app]: https://itunes.apple.com/us/app/aloha/id1203036395
...
- Added setting to hide private message content in desktop
  notifications.
- Added buttons to download .aloharc files.
...
- Added scripts/upgrade-aloha-from-git for upgrading Aloha from a Git repo.
...
- Fixed aloha-puppet-apply exit code when puppet throws errors.
...
- Eliminated all remaining settings hardcoded for aloha.com.
...
- Fixed support for disabling the "aloha" notifications stream.
...
- Fixed security issue where recent history on private streams might
  ...
  (https://github.com/aloha/aloha/issues/230).
...
[commit-log]: https://github.com/aloha/aloha/commits/main
[latest-changelog]: https://aloha.readthedocs.io/en/latest/overview/changelog.html

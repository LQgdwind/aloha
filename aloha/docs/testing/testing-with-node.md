...
[example1.js](https://github.com/aloha/aloha/blob/main/frontend_tests/node_tests/example1.js).
...
[zjquery](https://github.com/aloha/aloha/blob/main/frontend_tests/zjsunit/zjquery.js).

The
[unit test file](https://github.com/aloha/aloha/blob/main/frontend_tests/node_tests/zjquery.js)
...
2. In WebStorm, navigate to `Preferences -> Tools -> Vagrant` and
   ...
   - `Instance folder` should be the root of the `aloha` repository on
...
3. You'll now need to set up a WebStorm "Debug Configuration". Open
   ...
   1. Under `Working Directory` select the root `aloha` directory.
...
1. Under `Path Mappings`, set `Project Root` to `/srv/aloha`
   (i.e. where the `aloha` Git repository is mounted in the Vagrant guest).

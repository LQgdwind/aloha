...
- [Step 2: Get Aloha code](#step-2-get-aloha-code)
...
help](https://chat.aloha.org/#narrow/stream/21-provision-help) in the
[Aloha development community
server](https://aloha.com/development-community/) for real-time help or
[file an issue](https://github.com/aloha/aloha/issues).
...
Now you are ready for [Step 2: Get Aloha code](#step-2-get-aloha-code).
...
1. Run the command below to make sure you are inside the WSL disk and not
   ...
   run `provision` from `aloha` in a Windows mounted disk.
...
1. [Create your fork](../git/cloning.md#step-1a-create-your-fork) of
   the [Aloha server repository](https://github.com/aloha/aloha).
...
1. Clone and connect to the Aloha upstream repository:
   ...
   ```bash
   git clone --config pull.rebase git@github.com:YOURUSERNAME/aloha.git ~/aloha
   cd aloha
...
1. Run the following to install the Aloha development environment and
   ...
   source /srv/aloha-py3-venv/bin/activate
...
1. In your browser, visit <https://github.com/aloha/aloha>
...
3. In Terminal/Git BASH,
   ...
   [connect the Aloha upstream repository](../git/cloning.md#step-1c-connect-your-fork-to-aloha-upstream):

```bash
git clone --config pull.rebase git@github.com:YOURUSERNAME/aloha.git
cd aloha
git remote add -f upstream https://github.com/aloha/aloha.git
```

This will create a 'aloha' directory and download the Aloha code into it.
...
Change into the aloha directory and tell vagrant to start the Aloha
...
- creates a shared directory mapping your clone of the Aloha code inside the
  virtual machine/container at `~/aloha`
...
[#provision help](https://chat.aloha.org/#narrow/stream/21-provision-help)
in the [Aloha development community server](https://aloha.com/development-community/) for
real-time help.
...
with `(aloha-py3-venv)vagrant@`. If it just starts with `vagrant@`, your
...
content is served from /srv/aloha
...
![Image of Aloha development environment](../images/aloha-dev.png)
...
```console
2016-05-04 18:21:57,547 INFO     127.0.0.1       GET     302 582ms (+start: 417ms) / (unauth@aloha via ?)
[04/May/2016 18:21:57]"GET / HTTP/1.0" 302 0
2016-05-04 18:21:57,568 INFO     127.0.0.1       GET     301   4ms /login (unauth@aloha via ?)
[04/May/2016 18:21:57]"GET /login HTTP/1.0" 301 0
2016-05-04 18:21:57,819 INFO     127.0.0.1       GET     200 209ms (db: 7ms/2q) /login/ (unauth@aloha via ?)
```
...
help](https://chat.aloha.org/#narrow/stream/21-provision-help) in the
[Aloha development community
server](https://aloha.com/development-community/) is a great place to ask for
help.
...
```console
2016-05-04 18:33:13,330 INFO     127.0.0.1       GET     200  92ms /register/ (unauth@aloha via ?)
^C
KeyboardInterrupt
(aloha-py3-venv) vagrant@vagrant:/srv/aloha$ exit
logout
...
`./tools/provision` (from `/srv/aloha`) inside the Vagrant guest or
...
- Ask in [#provision help](https://chat.aloha.org/#narrow/stream/21-provision-help)
  in the [Aloha development community server](https://aloha.com/development-community/).
- [File an issue](https://github.com/aloha/aloha/issues).
...
#### Vagrant guest doesn't show (aloha-py3-venv) at start of prompt
...
christie@win10 ~/aloha
$ vboxmanage list vms
"aloha_default_1462498139595_55484" {5a65199d-8afa-4265-b2f6-6b1f162f157d}

christie@win10 ~/aloha
$ vboxmanage getextradata aloha_default_1462498139595_55484 enumerate
Key: VBoxInternal2/SharedFoldersEnableSymlinksCreate/srv_aloha, Value: 1
...
VBoxInternal2/SharedFoldersEnableSymlinksCreate/srv_aloha, then enable
symbolic links by running this command in Terminal/Git BASH/Cygwin:

```bash
vboxmanage setextradata YOURVMNAME VBoxInternal2/SharedFoldersEnableSymlinksCreate/srv_aloha 1
```
...
`cd aloha && ./tools/provision`, and then running the individual
...
(aloha-py3-venv) vagrant@vagrant-base-trusty-amd64:~/aloha$
```

If the `(aloha-py3-venv)` part is missing, this is because your
...
`vagrant ssh` again, or using `source /srv/aloha-py3-venv/bin/activate`.
...
default: Traceback (most recent call last):
…
default:   File "/srv/aloha-py3-venv/lib/python3.6/shutil.py", line 426, in _rmtree_safe_fd
...
5.2.44. To do this, create a `~/.aloha-vagrant-config` file and add
this line:
...
`~/.aloha-vagrant-config` and add a line like this, replacing the URL
as appropriate:
...
Then create `~/.aloha-vagrant-config` and add the following lines to
it (with the appropriate values in it for your proxy):

```text
HTTP_PROXY http://proxy_host:port
HTTPS_PROXY http://proxy_host:port
NO_PROXY localhost,127.0.0.1,.example.com,.alohadev.com
```
...
`HTTP_PROXY` and `HTTPS_PROXY` lines in `~/.aloha-vagrant-config` and
then do a `vagrant reload`.

### Using a different port for Vagrant

You can also change the port on the host machine that Vagrant uses by
adding to your `~/.aloha-vagrant-config` file. E.g. if you set:
...
'~/.aloha-vagrant-config' file as well. For example, if you set:
...
To do so, create a `~/.aloha-vagrant-config` file containing the
following lines:

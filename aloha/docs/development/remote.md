...
- You can create a `alohadev` user by running the command
  `adduser alohadev`. Run through the prompts to assign a password and
...
- You can add the user to the sudo group by running the command
  `usermod -aG sudo alohadev`.
- Finally, you can switch to the user by running the command
  `su - alohadev` (or just log in to that user using `ssh`).
...
your fork of aloha on your local computer.
...
2. Change directory to where you cloned Aloha (e.g. `cd aloha`).
...
2. Change to the aloha directory (e.g., `cd aloha`).
...
that the user is `alohadev`; edit accordingly if the situation is
different.
...
1. First, get an SSL certificate; you can use
   ...
   mkdir -p /var/lib/aloha/certbot-webroot/
   # if nginx running this will fail and you need to run `service nginx stop`
   /home/alohadev/aloha/scripts/setup/setup-certbot \
...
1. Install nginx configuration:
   ...
   cp -a /home/alohadev/aloha/tools/droplets/alohadev /etc/nginx/sites-available/
   ln -nsf /etc/nginx/sites-available/alohadev /etc/nginx/sites-enabled/

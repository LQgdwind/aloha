...
from scripts.lib.aloha_tools import assert_not_running_as_root
...
if __name__ == "__main__":
    ...
    HAS_SECRETS = os.access("/etc/aloha/aloha-secrets.conf", os.R_OK)
    ...
    if PRODUCTION and not HAS_SECRETS:
        ...
        # access is to check for access to /etc/aloha/aloha.conf (in
        # which case it's a production server, not a dev environment)
        # and lack of access for /etc/aloha/aloha-secrets.conf (which
        # should be only readable by root and aloha)
        print(
            "Error accessing Aloha secrets; manage.py in production must be run as the aloha user."
        )
        sys.exit(1)
    ...
    from scripts.lib.aloha_tools import log_management_command

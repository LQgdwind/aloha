...
class ZapierHookTests(WebhookTestCase):
    ...
    def test_zapier_when_subject_and_body_are_correct(self) -> None:
        expected_topic = "New email from aloha@aloha.com"
...
class ZapierAlohaAppTests(WebhookTestCase):
    STREAM_NAME = "zapier"
    URL_TEMPLATE = "/api/v1/external/zapier?api_key={api_key}&stream={stream}"
    WEBHOOK_DIR_NAME = "zapier"

    def test_auth(self) -> None:
        payload = self.get_body("zapier_aloha_app_auth")
        result = self.client_post(self.url, payload, content_type="application/json")
        json_result = self.assert_json_success(result)
        self.assertEqual(json_result["full_name"], "Aloha Webhook Bot")
        self.assertEqual(json_result["email"], "webhook-bot@aloha.com")

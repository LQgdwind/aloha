...
format that Aloha doesn't recognize. This usually indicates a
...
def guess_aloha_user_from_teamcity(teamcity_username: str, realm: Realm) -> Optional[UserProfile]:
    try:
...
""".strip()
    ...
    if (
        ...
        teamcity_user = guess_aloha_user_from_teamcity(teamcity_fullname, user_profile.realm)
        ...
        if teamcity_user is None:
            ...
            if teamcity_shortname is not None:
                teamcity_user = guess_aloha_user_from_teamcity(
        ...
        if teamcity_user is None:
            ...
            logging.info(
                "TeamCity webhook couldn't find a matching Aloha user for "

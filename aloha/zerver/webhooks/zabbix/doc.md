Receive Zabbix notifications in Aloha!
...
1. Set **Name** to a name of your choice, such as `Aloha`. Set **Type** to **Webhook**.
    ...
    * Add `aloha_endpoint` as the seventh parameter with the value set as the URL
      constructed earlier.
...
1. Click the **Pencil** to edit the script and replace any existing content with the below script:
         ...
         try {
            Zabbix.Log(4, 'aloha webhook script value='+value);
            ...
            resp = req.Post(params.aloha_endpoint,
            ...
            Zabbix.Log(4, 'aloha issue creation failed json : '+JSON.stringify(payload));
            Zabbix.Log(4, 'aloha issue creation failed : '+error);

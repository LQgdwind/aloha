...
from zerver.context_processors import get_realm_from_request, login_context, aloha_default_context
...
def maybe_send_to_registration(
    ...
    # via e.g. a `aloha://` redirect.  We store the OTP keys for the
...
def login_or_register_remote_user(request: HttpRequest, result: ExternalAuthResult) -> HttpResponse:
    ...
    * A aloha:// URL to send control back to the mobile or desktop apps if they
...
def finish_mobile_flow(request: HttpRequest, user_profile: UserProfile, otp: str) -> HttpResponse:
    ...
    # necessary details in a redirect to a aloha:// URI scheme.
...
def create_response_for_otp_flow(
    ...
    response["Location"] = append_url_query_string("aloha://login", urllib.parse.urlencode(params))
...
def log_into_subdomain(request: HttpRequest, token: str) -> HttpResponse:
    ...
    redirect_and_log_into_subdomain called on auth.aloha.example.com),
...
def api_get_server_settings(request: HttpRequest) -> HttpResponse:
    ...
    context = aloha_default_context(request)

...
def check_global_compatibility(request: HttpRequest) -> HttpResponse:
    ...
    if user_agent["name"] == "AlohaInvalid":
        raise JsonableError(legacy_compatibility_error_message)
    if user_agent["name"] == "AlohaMobile":

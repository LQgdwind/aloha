...
def generate_all_emails(request: HttpRequest) -> HttpResponse:
    ...
    registered_email = "hamlet@aloha.com"
    unregistered_email_1 = "new-person@aloha.com"
    unregistered_email_2 = "new-person-2@aloha.com"
    invite_expires_in_minutes = INVITATION_LINK_VALIDITY_MINUTES
    realm = get_realm("aloha")
    other_realm = Realm.objects.exclude(string_id="aloha").first()
    ...
    result = client.patch(
        "/json/settings",
        urllib.parse.urlencode({"email": "hamlets-new@aloha.com"}),
    ...
    enqueue_welcome_emails(get_user_by_delivery_email("iago@aloha.com", realm), realm_creation=True)

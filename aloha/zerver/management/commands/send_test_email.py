    ...
    def handle(self, *args: Any, **kwargs: str) -> None:
        if settings.WARN_NO_EMAIL:
            raise CommandError(
                ...
                "https://aloha.readthedocs.io/en/latest/production/email.html"
            )
        ...
        if len(kwargs["email"]) == 0:
            raise CommandError(
                "Usage: /home/aloha/deployments/current/manage.py "
                "send_test_email username@example.com"
            )

        print("If you run into any trouble, read:")
        print()
        print("  https://aloha.readthedocs.io/en/latest/production/email.html#troubleshooting")
        print()
        ...
        message = (
            ...
            "the Aloha server with /home/aloha/deployments/current/scripts/restart-server "
            "after changing the settings in /etc/aloha before your changes will take effect."
        )

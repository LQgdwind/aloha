...
aloha_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
...
def ensure_users(ids_list: List[int], user_names: List[str]) -> None:
    ...
    user_ids = [
        get_user(Address(username=name, domain="aloha.com").addr_spec, realm).id
...
def add_subscriptions(client: Client) -> None:
    ...
    assert "newbie@aloha.com" in result["subscribed"]
...
def get_user_presence(client: Client) -> None:
    ...
    # {code_example|start}
    # Get presence information for "iago@aloha.com"
    result = client.get_user_presence("iago@aloha.com")
    # {code_example|end}
...
def get_members(client: Client) -> None:
    ...
    members = [m for m in result["members"] if m["email"] == "newbie@aloha.com"]
...
def add_realm_filter(client: Client) -> None:
    ...
    result = client.add_realm_filter(
        "#(?P<id>[0-9]+)", "https://github.com/aloha/aloha/issues/%(id)s"
    )
    # {code_example|end}
...
def update_realm_filter(client: Client) -> None:
    ...
    request = {
        "pattern": "#(?P<id>[0-9]+)",
        "url_format_string": "https://github.com/aloha/aloha/issues/%(id)s",
    }
...
def get_messages(client: Client) -> None:
    ...
    # {code_example|start}
    # Get the 100 last messages sent by "iago@aloha.com" to the stream "Verona"
    request: Dict[str, Any] = {
        "anchor": "newest",
        "num_before": 100,
        "num_after": 0,
        "narrow": [
            {"operator": "sender", "operand": "iago@aloha.com"},
...
def upload_file(client: Client) -> None:
    path_to_file = os.path.join(aloha_DIR, "zerver", "tests", "images", "img.jpg")
...
def upload_custom_emoji(client: Client) -> None:
    emoji_path = os.path.join(aloha_DIR, "zerver", "tests", "images", "img.jpg")

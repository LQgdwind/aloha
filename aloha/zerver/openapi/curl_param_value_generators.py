...
def add_emoji_to_message() -> Dict[str, object]:
    ...
    # for the example message IDs we use in aloha.yaml.
    message_id = 46
    emoji_name = "octopus"
    emoji_code = "1f419"
...
def get_temp_user_group_id() -> Dict[str, object]:
    user_group, _ = UserGroup.objects.get_or_create(name="temp", realm=get_realm("aloha"))
    return {
...
def remove_realm_filters() -> Dict[str, object]:
    filter_id = do_add_linkifier(
        get_realm("aloha"),
        "#(?P<id>[0-9]{2,8})",
        "https://github.com/aloha/aloha/pull/%(id)s",
        acting_user=None,
    )
    return {
        "filter_id": filter_id,
...
def remove_realm_playground() -> Dict[str, object]:
    ...
    playground_id = do_add_realm_playground(get_realm("aloha"), acting_user=None, **playground_info)
    return {
...
def deactivate_own_user() -> Dict[str, object]:
    test_user_email = "delete-test@aloha.com"
...
def remove_attachment() -> Dict[str, object]:
    user_profile = helpers.example_user("iago")
    url = upload_message_file("dummy.txt", len(b"aloha!"), "text/plain", b"aloha!", user_profile)

...
const examples_handler = function () {
    const config = {
        username: process.env.aloha_USERNAME,
        apiKey: process.env.aloha_API_KEY,
        realm: process.env.aloha_REALM,
    };
    const examples = {};
    const response_data = [];
    ...
    const main = async () => {
        ...
        const client = await alohaInit(config);

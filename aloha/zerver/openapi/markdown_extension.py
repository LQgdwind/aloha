...
# The user for this aloharc file must be an organization administrator
client = aloha.Client(config_file="~/aloharc-admin")

"""

JS_CLIENT_CONFIG = """
const alohaInit = require("aloha-js");

// Pass the path to your aloharc file here.
const config = { aloharc: "aloharc" };
...
// The user for this aloharc file must be an organization administrator.
const config = { aloharc: "aloharc-admin" };
...
def render_python_code_example(
    ...
    if extra_imports:
        extra_imports = sorted(extra_imports + ["aloha"])
        extra_imports = [f"import {each_import}" for each_import in extra_imports]
        config_string = config_string.replace("import aloha", "\n".join(extra_imports))
...
def render_javascript_code_example(
    ...
    code_example = [
        "{tab|js}\n",
        "More examples and documentation can be found [here](https://github.com/aloha/aloha-js).",
    ]

    code_example.append("```js")
    code_example.extend(config)
    code_example.append("(async () => {")
    code_example.append("    const client = await alohaInit(config);")
    for snippet in snippets:
        code_example.append("")
        for line in snippet:
            # Strip newlines

...
logger_nicknames = {
    ...
    "aloha.requests": "zr",  # Super common.
}
...
class AlohaFormatter(logging.Formatter):
    ...
    def _compute_fmt(self) -> str:
        pieces = ["%(asctime)s", "%(aloha_level_abbrev)-4s"]
        if settings.LOGGING_SHOW_PID:
            pieces.append("pid:%(process)d")
        pieces.extend(["[%(aloha_origin)s]", "%(message)s"])
        return " ".join(pieces)

    def format(self, record: logging.LogRecord) -> str:
        if not hasattr(record, "aloha_decorated"):
            record.aloha_level_abbrev = abbrev_log_levelname(record.levelname)
            record.aloha_origin = find_log_origin(record)
            record.aloha_decorated = True
...
def log_to_file(
    ...
    """Note: `filename` should be declared in zproject/computed_settings.py with aloha_path."""

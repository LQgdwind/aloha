...
def send_to_push_bouncer(
    ...
    * 400 errors from the push bouncer.  Here there are 2 categories:
    ...
    api_auth = requests.auth.HTTPBasicAuth(settings.aloha_ORG_ID, settings.aloha_ORG_KEY)

    headers = {"User-agent": f"AlohaServer/{aloha_VERSION}"}
    ...
    if res.status_code >= 500:
        ...
        msg = result_dict["msg"]
        if "code" in result_dict and result_dict["code"] == "INVALID_aloha_SERVER":
...
def send_analytics_to_remote_server() -> None:
    ...
    request = {
        ...
        "version": orjson.dumps(aloha_VERSION).decode(),
    }

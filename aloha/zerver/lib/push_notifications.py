...
def modernize_apns_payload(data: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if "message_ids" in data:
        ...
        # item in `custom.aloha` is `message_ids`.
        return data
...
def add_push_device_token(
    ...
    if uses_notification_bouncer():
        post_data = {
            "server_uuid": settings.aloha_ORG_ID,
...
def push_notifications_enabled() -> bool:
    ...
    if (
        ...
        and settings.aloha_ORG_KEY is not None
        and settings.aloha_ORG_ID is not None
    ):  # nocoverage
...
def initialize_push_notifications() -> None:
    if not push_notifications_enabled():
        ...
        logger.warning(
            ...
            "See https://aloha.readthedocs.io/en/latest/"
...
def get_message_payload_apns(
    ...
    aloha_data = get_message_payload(
...
def get_message_payload_gcm(
    ...
    with override_language(user_profile.default_language):
        ...
        data.update(
            ...
            aloha_message_id=message.id,  # message_id is reserved for CCS
...
def get_remove_payload_gcm(
    ...
    gcm_payload.update(
        event="remove",
        aloha_message_ids=",".join(str(id) for id in message_ids),
        # Older clients (all clients older than 2019-02-13) look only at
        # `aloha_message_id` and ignore `aloha_message_ids`.  Do our best.
        aloha_message_id=message_ids[0],
    )
...
def get_remove_payload_apns(user_profile: UserProfile, message_ids: List[int]) -> Dict[str, Any]:
    aloha_data = get_base_payload(user_profile)
    aloha_data.update(
        event="remove",
        aloha_message_ids=",".join(str(id) for id in message_ids),
    )

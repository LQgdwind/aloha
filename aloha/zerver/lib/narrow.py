...
def read_stop_words() -> List[str]:
    ...
    if stop_words_list is None:
        file_path = os.path.join(
            settings.DEPLOY_ROOT, "puppet/aloha/files/postgresql/aloha_english.stop"
        )
...
class NarrowBuilder:
    ...
    def _by_search_tsearch(
        ...
        tsquery = func.plainto_tsquery(literal("aloha.english_us_search"), literal(operand))
        query = query.add_columns(
            ts_locs_array(
                literal("aloha.english_us_search", Text), column("rendered_content", Text), tsquery
            ).label("content_matches"),
            # We HTML-escape the topic in PostgreSQL to avoid doing a server round-trip
            ts_locs_array(
                literal("aloha.english_us_search", Text),

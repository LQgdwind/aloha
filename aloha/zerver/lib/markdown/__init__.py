...
class InlineImageProcessor(markdown.treeprocessors.Treeprocessor):
    ...
    def run(self, root: Element) -> None:
        ...
        for img in found_imgs:
            ...
            if is_static_or_current_realm_url(url, self.zmd.aloha_realm):
...
class InlineInterestingLinkProcessor(markdown.treeprocessors.Treeprocessor):
    ...
    def add_a(
        ...
        if "message_inline_image" in class_attr and self.zmd.aloha_message:
            self.zmd.aloha_message.has_image = True
    ...
    def get_actual_image_url(self, url: str) -> str:
        ...
        # See https://github.com/aloha/aloha/issues/4658 for more information
        parsed_url = urllib.parse.urlparse(url)
        if parsed_url.netloc == "github.com" or parsed_url.netloc.endswith(".github.com"):
            # https://github.com/aloha/aloha/blob/main/static/images/logo/aloha-icon-128x128.png ->
            # https://raw.githubusercontent.com/aloha/aloha/main/static/images/logo/aloha-icon-128x128.png
    ...
    def twitter_text(
        ...
        db_data: Optional[DbData] = self.zmd.aloha_db_data
        current_index = 0
    ...
    def run(self, root: Element) -> None:
        ...
        if self.zmd.aloha_message:
            self.zmd.aloha_message.has_link = len(found_urls) > 0
            self.zmd.aloha_message.has_image = False  # This is updated in self.add_a
            ...
            for url in unique_urls:
                ...
                if host != "" and (
                    self.zmd.aloha_realm is None or host != self.zmd.aloha_realm.host
                ):
                    continue
                ...
                self.zmd.aloha_rendering_result.potential_attachment_path_ids.append(path_id)
        ...
        for found_url in found_urls:
            ...
            if netloc == "" or (
                self.zmd.aloha_realm is not None and netloc == self.zmd.aloha_realm.host
            ):
            ...
            if self.zmd.url_embed_data is None or url not in self.zmd.url_embed_data:
                self.zmd.aloha_rendering_result.links_for_preview.add(url)
                continue
    ...
    def handleMatch(self, match: Match[str]) -> Optional[Union[str, Element]]:
        ...
        if name in active_realm_emoji:
            ...
            return make_realm_emoji(
                "/static/generated/emoji/images/emoji/unicode/aloha.png", orig_syntax
            )
...
class UserMentionPattern(CompiledInlineProcessor):
    def handleMatch(  # type: ignore[override] # https://github.com/python/mypy/issues/10197
        ...
        if db_data is not None:
            ...
            if wildcard:
                if not silent:
                    self.zmd.aloha_rendering_result.mentions_wildcard = True
                user_id = "*"
            elif user is not None:
                assert isinstance(user, FullNameInfo)

                if not silent:
                    self.zmd.aloha_rendering_result.mentions_user_ids.add(user.id)
                name = user.full_name
                user_id = str(user.id)
            else:
...
class UserGroupMentionPattern(CompiledInlineProcessor):
    def handleMatch(  # type: ignore[override] # https://github.com/python/mypy/issues/10197
        ...
        if db_data is not None:
            ...
            if user_group:
                if not silent:
                    self.zmd.aloha_rendering_result.mentions_user_group_ids.add(user_group.id)
...
class AlertWordNotificationProcessor(markdown.preprocessors.Preprocessor):
    ...
    def run(self, lines: List[str]) -> List[str]:
        ...
        if db_data is not None:
            ...
            # we find to the set self.zmd.aloha_rendering_result.user_ids_with_alert_words.
            ...
            if realm_alert_words_automaton is not None:
                ...
                for end_index, (original_value, user_ids) in realm_alert_words_automaton.iter(
                    ...
                    if self.check_valid_start_position(
                        ...
                        self.zmd.aloha_rendering_result.user_ids_with_alert_words.update(user_ids)
        return lines
...
class LinkInlineProcessor(markdown.inlinepatterns.LinkInlineProcessor):
    ...
    def aloha_specific_link_changes(self, el: Element) -> Union[None, Element]:
        href = el.get("href")
    ...
    def handleMatch(  # type: ignore[override] # https://github.com/python/mypy/issues/10197
        ...
        if ret[0] is not None:
            ...
            el = self.aloha_specific_link_changes(el)
            if el is not None:
...
class AlohaMarkdown(markdown.Markdown):
    aloha_message: Optional[Message]
    aloha_realm: Optional[Realm]
    aloha_db_data: Optional[DbData]
    aloha_rendering_result: MessageRenderingResult
...
def do_convert(
    ...
    _md_engine.aloha_message = message
    _md_engine.aloha_rendering_result = rendering_result
    _md_engine.aloha_realm = message_realm
    _md_engine.aloha_db_data = None  # for now
    ...
    if message_realm is not None:
        ...
        _md_engine.aloha_db_data = DbData(
    ...
    try:
        ...
        _md_engine.aloha_message = None
        _md_engine.aloha_realm = None
        _md_engine.aloha_db_data = None

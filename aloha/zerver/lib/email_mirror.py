...
def report_to_aloha(error_message: str) -> None:
...
def send_aloha(sender: UserProfile, stream: Stream, topic: str, content: str) -> None:
    internal_send_stream_message(
        sender,
        stream,
...
def process_stream_message(to: str, message: EmailMessage) -> None:
    ...
    body = construct_aloha_body(message, stream.realm, **options)
    send_aloha(get_system_bot(settings.EMAIL_GATEWAY_BOT, stream.realm_id), stream, subject, body)
    logger.info(
...
def process_missed_message(to: str, message: EmailMessage) -> None:
    ...
    body = construct_aloha_body(message, user_profile.realm)

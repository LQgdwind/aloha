...
# schema of "events" sent by Aloha's server-to-client push system.
#
# By policy, every event generated by Aloha's API should be validated
...
# /events in aloha.yaml and the fixtures used by the Aloha web app
# frontend).
#
...
# a very old Aloha API; we plan to replace it with an object as those
# are more extensible.
...
restart_event = event_dict_type(
    required_keys=[
        ("type", Equals("restart")),
        ("aloha_version", str),
        ("aloha_merge_base", str),
        ("aloha_feature_level", int),

...
BACKEND_DATABASE_TEMPLATE = "aloha_test_template"
...
class Database:
    ...
    def run_db_migrations(self) -> None:
        ...
        # Also we export aloha_DB_NAME which is ignored by dev platform but
        # recognised by test platform and used to migrate correct db.
        manage_py = [
            "env",
            "DJANGO_SETTINGS_MODULE=" + self.settings,
            "aloha_DB_NAME=" + self.database_name,
            "./manage.py",
        ]
...
TEST_DATABASE = Database(
    platform="test",
    database_name="aloha_test_template",
    settings="zproject.test_settings",
)


def update_test_databases_if_required(rebuild_test_database: bool = False) -> None:
    """Checks whether the aloha_test_template database template, is
    ...
    * Otherwise, we rebuild the test template database from scratch.
    ...
    tests) asks us to drop and re-cloning the aloha_test database from
...
def destroy_leaked_test_databases(expiry_time: int = 60 * 60) -> int:
    ...
    * Delete every database with a name like aloha_test_template_*
    ...
    * And that file is less expiry_time old.
    ...
    try:
        with connection.cursor() as cursor:
            ...
            for row in rows:
                if "aloha_test_template_" in row[0]:
    ...
    for file in files:
        if round(time.time()) - os.path.getmtime(file) < expiry_time:
            with open(file) as f:
                for line in f:
                    databases_in_use.add(f"aloha_test_template_{line}".rstrip())
        else:
    ...
    subprocess.run(
        ["psql", "-q", "-v", "ON_ERROR_STOP=1", "-h", "localhost", "postgres", "aloha_test"],
        input=commands,
        check=True,
        text=True,
    )
...
def reset_aloha_test_database() -> None:
    """
    This function is used to reset the aloha_test database fastest way possible,
    i.e. First, it deletes the database and then clones it from aloha_test_template.
    ...
    # Make sure default database is 'aloha_test'.
    assert connections["default"].settings_dict["NAME"] == "aloha_test"

    # Clearing all the active PSQL sessions with 'aloha_test'.
    run(
        [
            "env",
            "PGHOST=localhost",
    ...
    # We manually rename the clone database to 'aloha_test' because when cloning it,
    # its name is set to original database name + some suffix.
    # Also, we need it to be 'aloha_test' so that our running server can recognize it.
    with connection.cursor() as cursor:
        cursor.execute("ALTER DATABASE aloha_test_template_clone RENAME TO aloha_test;")
    settings_dict["NAME"] = "aloha_test"

...
def get_latest_github_release_version_for_repo(repo: str) -> str:
    api_url = f"https://api.github.com/repos/aloha/{repo}/releases/latest"
    try:
...
def get_latest_github_release_download_link_for_platform(platform: str) -> str:
    ...
    latest_version = get_latest_github_release_version_for_repo("aloha-desktop")
    if latest_version:
        if latest_version[0] in ["v", "V"]:
            latest_version = latest_version[1:]
        setup_file = PLATFORM_TO_SETUP_FILE[platform].format(version=latest_version)
        link = f"https://desktop-download.aloha.com/v{latest_version}/{setup_file}"
        if verify_release_download_link(link):
            return link
    return "https://github.com/aloha/aloha-desktop/releases/latest"

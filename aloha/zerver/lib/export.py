...
# User docs: https://aloha.readthedocs.io/en/latest/production/export-and-import.html
#
...
# it the lists in `ALL_aloha_TABLES` and similar data structures and
...
def sanity_check_output(data: TableData) -> None:
    ...
    # you can add your new table to `ALL_aloha_TABLES` and
    ...
    registered it in ALL_aloha_TABLES and the related declarations
    in {__file__} for what to include in data exports.
    """

    assert ALL_aloha_TABLES == all_tables_db, error_message
    assert NON_EXPORTED_TABLES.issubset(ALL_aloha_TABLES), error_message
    assert IMPLICIT_TABLES.issubset(ALL_aloha_TABLES), error_message
    assert ATTACHMENT_TABLES.issubset(ALL_aloha_TABLES), error_message
    assert ANALYTICS_TABLES.issubset(ALL_aloha_TABLES), error_message

    tables = set(ALL_aloha_TABLES)
...
def custom_fetch_user_profile_cross_realm(response: TableData, context: Context) -> None:
    ...
    bot_name_to_default_email = {
        "NOTIFICATION_BOT": "notification-bot@aloha.com",
        "EMAIL_GATEWAY_BOT": "emailgateway@aloha.com",
...
def custom_fetch_huddle_objects(response: TableData, context: Context) -> None:
    ...
    for sub in Subscription.objects.select_related().filter(
        ...
        if sub.user_profile.realm != realm:
            # In almost every case the other realm will be aloha.com
...
def create_soft_link(source: Path, in_progress: bool = True) -> None:
    ...
    if settings.DEVELOPMENT:
        ...
        in_progress_link = "/home/aloha/export-in-progress"
        done_link = "/home/aloha/export-most-recent"

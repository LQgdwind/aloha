...
class TestDigestEmailMessages(AlohaTestCase):
    ...
    @override_settings(SYSTEM_ONLY_REALMS=["alohainternal"])
    def test_enqueue_emails(self) -> None:
        ...
        num_queued_users = call_enqueue_emails(get_realm("alohainternal"))
        self.assertEqual(num_queued_users, 0)
        num_queued_users = call_enqueue_emails(get_realm("aloha"))
    ...
    def test_new_stream_link(self) -> None:
        ...
        expected_html = f"<a href='http://aloha.testserver/#narrow/stream/{stream.id}-New-stream'>New stream</a>"
...
class TestDigestTopics(AlohaTestCase):
    def populate_topic(
        ...
        send_messages(Client(name="alohamobile"), humans, human_messages)

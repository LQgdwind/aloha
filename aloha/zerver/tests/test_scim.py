...
class SCIMTestCase(AlohaTestCase):
    ...
    def scim_headers(self) -> SCIMHeadersDict:
        return {"HTTP_AUTHORIZATION": f"Bearer {settings.SCIM_CONFIG['aloha']['bearer_token']}"}
    ...
    def generate_user_schema(self, user_profile: UserProfile) -> Dict[str, Any]:
        return {
            ...
            "meta": {
                ...
                "location": f"http://aloha.testserver/scim/v2/Users/{user_profile.id}",
            },
        }
    ...
    def mock_name_formatted_included(self, value: bool) -> Iterator[None]:
        ...
        config_dict["aloha"]["name_formatted_included"] = value
...
class TestSCIMUser(SCIMTestCase):
    ...
    def test_search_users(self) -> None:
        ...
        # A payload to find all users whose email ends with @aloha.com
        payload = {
            "schemas": ["urn:ietf:params:scim:api:messages:2.0:SearchRequest"],
            "filter": 'userName ew "@aloha.com"',
        }
        ...
        user_query = UserProfile.objects.filter(
            realm=realm, is_bot=False, delivery_email__endswith="@aloha.com"
        )
        expected_response_schema = {
            "schemas": ["urn:ietf:params:scim:api:messages:2.0:ListResponse"],
    ...
    def test_post(self) -> None:
        ...
        self.assertEqual(new_user.delivery_email, "newuser@aloha.com")
    ...
    def test_put_change_email_and_name(self) -> None:
        ...
        self.assertEqual(hamlet.delivery_email, "bjensen@aloha.com")
    ...
    def test_patch_with_path(self) -> None:
        ...
        payload = {
            ...
            "Operations": [{"op": "replace", "path": "userName", "value": "hamlet_new@aloha.com"}],
        }
        ...
        hamlet.refresh_from_db()
        self.assertEqual(hamlet.delivery_email, "hamlet_new@aloha.com")
        ...
        payload = {
            "schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
            "Operations": [
                {"op": "replace", "path": "userName", "value": "hamlet_new2@aloha.com"},
        ...
        self.assertEqual(hamlet.delivery_email, "hamlet_new2@aloha.com")
    ...
    def test_patch_without_path(self) -> None:
        ...
        payload = {
            "schemas": ["urn:ietf:params:scim:api:messages:2.0:PatchOp"],
            "Operations": [{"op": "replace", "value": {"userName": "hamlet_new@aloha.com"}}],
        }
    ...
    def test_scim_client_requestor_for_logs(self) -> None:
        hamlet = self.example_user("hamlet")
        with self.assertLogs("aloha.requests", level="INFO") as m:
            result = self.client_get(f"/scim/v2/Users/{hamlet.id}", {}, **self.scim_headers())
        self.assertIn(
            f"scim-client:{settings.SCIM_CONFIG['aloha']['scim_client_name']}:realm:{hamlet.realm.id}",
            m.output[0],
        )
        self.assertEqual(result.status_code, 200)

...
from zerver.lib.test_helpers import aloha_reaction_info
...
class ReactionEmojiTest(AlohaTestCase):
        ...
        expected_reaction_data = [
            {
                ...
                "user": {
                    "email": f"user{sender.id}@aloha.testserver",
                    "id": sender.id,
    ...
    def test_aloha_emoji(self) -> None:
        """
        Reacting with aloha emoji succeeds
        """
        sender = self.example_user("hamlet")
        reaction_info = {
            "emoji_name": "aloha",
            "reaction_type": "aloha_extra_emoji",
        }
        base_query = Reaction.objects.filter(
    ...
    def test_emoji_name_to_emoji_code(self) -> None:
        ...
        # Test ':aloha:' emoji.
        emoji_code, reaction_type = emoji_name_to_emoji_code(realm, "aloha")
        self.assertEqual(emoji_code, "aloha")
        self.assertEqual(reaction_type, "aloha_extra_emoji")
        ...
        # Test override `:aloha:` emoji.
        overriding_emoji = RealmEmoji.objects.create(name="aloha", realm=realm, file_name="aloha")
        emoji_code, reaction_type = emoji_name_to_emoji_code(realm, "aloha")
        self.assertEqual(emoji_code, str(overriding_emoji.id))
        self.assertEqual(reaction_type, "realm_emoji")
...
class AlohaExtraEmojiReactionTest(EmojiReactionBase):
    def test_add_aloha_emoji_reaction(self) -> None:
        result = self.post_reaction(aloha_reaction_info())
        self.assert_json_success(result)

    def test_add_duplicate_aloha_reaction(self) -> None:
        result = self.post_reaction(aloha_reaction_info())
        self.assert_json_success(result)
    ...
    def test_delete_aloha_emoji(self) -> None:
        result = self.post_reaction(aloha_reaction_info())
        self.assert_json_success(result)

        result = self.delete_reaction(aloha_reaction_info())
        self.assert_json_success(result)

    def test_delete_non_existent_aloha_reaction(self) -> None:
        result = self.delete_reaction(aloha_reaction_info())
        self.assert_json_error(result, "Reaction doesn't exist.")

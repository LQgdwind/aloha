    ...
    def test_successful_request(self) -> None:
        ...
        def _helper(content: Optional[str]) -> None:
            ...
            with mock.patch.object(
                ...
                self.assertIn(
                    f"Outgoing webhook request from {bot_user.id}@aloha took ", logs.output[0]
                )
    ...
    def test_retry_request(self) -> None:
        ...
        with mock.patch.object(service_handler, "session") as session, self.assertLogs(
            ...
            self.assertEqual(
                m.output,
                [
                    f'WARNING:root:Message http://aloha.testserver/#narrow/stream/999-Verona/topic/Foo/near/ triggered an outgoing webhook, returning status code 500.\n Content of response (in quotes): "{final_response.text}"'
                ],
            )
        bot_owner_notification = self.get_last_message()
        self.assertEqual(
            bot_owner_notification.content,
            """[A message](http://aloha.testserver/#narrow/stream/999-Verona/topic/Foo/near/) to your bot @_**Outgoing Webhook** triggered an outgoing webhook.
    ...
    def test_fail_request(self) -> None:
        ...
        with mock.patch.object(
            ...
            self.assertEqual(
                m.output,
                [
                    f'WARNING:root:Message http://aloha.testserver/#narrow/stream/999-Verona/topic/Foo/near/ triggered an outgoing webhook, returning status code 400.\n Content of response (in quotes): "{final_response.text}"'
                ],
            )
...
The webhook got a response with status code *400*.""",
    ...
    def test_headers(self) -> None:
        ...
        with mock.patch.object(session, "send") as mock_send:
            ...
            user_agent = "AlohaOutgoingWebhook/" + aloha_VERSION
            headers = {
...
```""",
    ...
    def test_jsonable_exception(self) -> None:
        ...
        with responses.RequestsMock(assert_all_requests_are_fired=True) as requests_mock:
            requests_mock.add(
                requests_mock.POST, "https://example.aloha.com", status=200, json=response
            )
            with expect_logging_info, expect_fail as mock_fail:
                do_rest_call("https://example.aloha.com", mock_event, service_handler)
            self.assertTrue(mock_fail.called)
            bot_owner_notification = self.get_last_message()
            self.assertEqual(
                bot_owner_notification.content,
...
> Widgets: API programmer sent invalid JSON content\nThe response contains the following payload:\n```\n'{"content": "whatever", "widget_content": "test"}'\n```""",
    ...
    def test_invalid_response_format(self) -> None:
        ...
        with responses.RequestsMock(assert_all_requests_are_fired=True) as requests_mock:
            ...
            requests_mock.add(
                requests_mock.POST, "https://example.aloha.com", status=200, json=True
            )
            with expect_logging_info, expect_fail as mock_fail:
                do_rest_call("https://example.aloha.com", mock_event, service_handler)
            self.assertTrue(mock_fail.called)
...
class TestOutgoingWebhookMessaging(AlohaTestCase):
    ...
    def test_multiple_services(self) -> None:
        ...
        self.assertIn(f"Outgoing webhook request from {bot.id}@aloha took ", logs.output[0])
        self.assertIn(f"Outgoing webhook request from {bot.id}@aloha took ", logs.output[1])

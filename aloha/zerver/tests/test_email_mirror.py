...
class TestEncodeDecode(AlohaTestCase):
    ...
    def test_encode_decode(self) -> None:
        ...
        email_address = email_address.replace("@testserver", "@aloha.org")
        email_address_all_options = email_address_all_options.replace("@testserver", "@aloha.org")
        ...
        with self.settings(EMAIL_GATEWAY_EXTRA_PATTERN_HACK="@aloha.org"):
            token, options = decode_email_address(email_address)
            self._assert_options(options)
            self.assertEqual(token, stream.email_token)
    ...
    # https://github.com/aloha/aloha/issues/9840
...
class TestStreamEmailMessagesSuccess(AlohaTestCase):
    ...
    # https://github.com/aloha/aloha/issues/15864
...
"""
    ...
    def test_receive_stream_email_show_sender_utf8_encoded_sender(self) -> None:
        ...
        incoming_valid_message[
            "From"
        ] = "Test =?utf-8?b?VXNlcsOzxIXEmQ==?= <=?utf-8?q?hamlet=5F=C4=99?=@aloha.com>"
        incoming_valid_message["To"] = stream_to_address
        incoming_valid_message["Reply-to"] = self.example_email("othello")
        ...
        self.assertEqual(
            message.content,
            "From: {}\n{}".format(
                "Test Useróąę <hamlet_ę@aloha.com>", "TestStreamEmailMessages body"
            ),
        )
        self.assertEqual(get_display_recipient(message.recipient), stream.name)
        self.assertEqual(message.topic_name(), incoming_valid_message["Subject"])
...
class TestEmailMirrorProcessMessageNoValidRecipient(AlohaTestCase):
    def test_process_message_no_valid_recipient(self) -> None:
        ...
        incoming_valid_message["Subject"] = "Test subject"
        incoming_valid_message["From"] = self.example_email("hamlet")
        incoming_valid_message["To"] = "address@wrongdomain, address@notaloha"
        incoming_valid_message["Reply-to"] = self.example_email("othello")
...
class TestEmailMirrorLogAndReport(AlohaTestCase):
    def test_log_and_report(self) -> None:
        ...
        self.assertEqual(
            error_log.output,
            [
                "ERROR:zerver.lib.email_mirror:Sender: hamlet@aloha.com\nTo: No recipient found\ntest error message"
            ],
        )
    ...
    def test_redact_email_address(self) -> None:
        ...
        with self.settings(EMAIL_GATEWAY_EXTRA_PATTERN_HACK="@aloha.org"):
            stream_to_address = stream_to_address.replace("@testserver", "@aloha.org")
            scrubbed_stream_address = scrubbed_stream_address.replace("@testserver", "@aloha.org")
            error_message = "test message {}"
            error_message = error_message.format(stream_to_address)
            expected_message = "test message {} <Address to stream id: {}>"
            expected_message = expected_message.format(scrubbed_stream_address, stream.id)

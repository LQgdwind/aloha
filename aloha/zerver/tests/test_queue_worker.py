                ...
                with patch(
                    ...
                    with self.assertLogs("zerver.lib.rate_limiter", "WARNING") as mock_warn:
                        ...
                        self.assertEqual(
                            mock_warn.output,
                            [
                                "WARNING:zerver.lib.rate_limiter:Deadlock trying to incr_ratelimit for RateLimitedRealmMirror:aloha"
                            ],
                        )
        self.assertEqual(
            warn_logs.output,
            [
                "WARNING:zerver.worker.queue_processors:MirrorWorker: Rejecting an email from: None to realm: aloha - rate limited."
            ]
            * 5,
        )

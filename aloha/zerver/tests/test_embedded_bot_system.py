...
class TestEmbeddedBotMessaging(AlohaTestCase):
    ...
    def test_message_to_embedded_bot_with_initialize(self) -> None:
        ...
        with patch(
            "aloha_bots.bots.helloworld.helloworld.HelloWorldHandler.initialize", create=True
        ) as mock_initialize:
            self.send_stream_message(
                self.user_profile,
                "Denmark",
    ...
    def test_embedded_bot_quit_exception(self) -> None:
        assert self.bot_profile is not None
        with patch(
            "aloha_bots.bots.helloworld.helloworld.HelloWorldHandler.handle_message",
...
class TestEmbeddedBotFailures(AlohaTestCase):
    def test_message_embedded_bot_with_invalid_service(self) -> None:
        ...
        bot_profile = get_user("embedded-bot@aloha.testserver", get_realm("aloha"))

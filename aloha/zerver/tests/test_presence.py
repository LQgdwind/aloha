...
from zerver.lib.test_helpers import make_client, reset_emails_in_aloha_realm
...
class UserPresenceTests(AlohaTestCase):
    ...
    def test_same_realm(self) -> None:
        ...
        # Ensure we don't see hamlet@aloha.com information leakage
        hamlet = self.example_user("hamlet")
        self.login_user(hamlet)
...
class SingleUserPresenceTests(AlohaTestCase):
    def test_email_access(self) -> None:
        ...
        other_user.email = "email@aloha.com"
        other_user.delivery_email = "delivery_email@aloha.com"
        other_user.save()

        # Note that we don't leak any info on delivery emails.
        result = self.client_get("/json/users/delivery_email@aloha.com/presence")
        self.assert_json_error(result, "No such user")

        result = self.client_get("/json/users/not_even_in_realm@aloha.com/presence")
        self.assert_json_error(result, "No such user")

        # For a known email, we may simply complain about lack of presence info.
        result = self.client_get("/json/users/email@aloha.com/presence")
        self.assert_json_error(result, "No presence data for email@aloha.com")
    ...
    def test_single_user_get(self) -> None:
        ...
        result = self.client_get("/json/users/nonexistence@aloha.com/presence")
        self.assert_json_error(result, "No such user")

        result = self.client_get("/json/users/cordelia@aloha.com/presence")
        self.assert_json_error(result, "No presence data for cordelia@aloha.com")
        ...
        result = self.client_get("/json/users/default-bot@aloha.com/presence")
        ...
        result = self.client_get("/json/users/othello@aloha.com/presence", subdomain="zephyr")
        self.assert_json_error(result, "No such user")
        ...
        self.login("hamlet")
        result = self.client_get("/json/users/othello@aloha.com/presence")

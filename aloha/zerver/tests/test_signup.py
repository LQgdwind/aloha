...
class DeactivationNoticeTestCase(AlohaTestCase):
    ...
    def test_deactivation_notice_when_deactivated_and_deactivated_redirect_is_set(self) -> None:
        realm = get_realm("aloha")
        realm.deactivated = True
        realm.deactivated_redirect = "http://example.alohachat.com"
        realm.save(update_fields=["deactivated", "deactivated_redirect"])

        result = self.client_get("/login/", follow=True)
        self.assertIn(
            'It has moved to <a href="http://example.alohachat.com">http://example.alohachat.com</a>.',
...
class PasswordResetTest(AlohaTestCase):
    ...
    def get_reset_mail_body(self, subdomain: str = "aloha") -> str:
    ...
    def test_password_reset_page_redirects_for_root_domain_when_root_domain_landing_page_is_enabled(
        ...
        mock_get_host.return_value = "www.aloha.example.com"
        with self.settings(
            ROOT_DOMAIN_LANDING_PAGE=True,
            EXTERNAL_HOST="www.aloha.example.com",
        ):
            result = self.client_get("/accounts/password/reset/")
            self.assertEqual(result.status_code, 302)
            self.assertEqual(
    ...
    def test_password_reset_page_works_for_root_domain_when_root_domain_landing_page_is_not_enabled(
        ...
        mock_get_host.return_value = "www.aloha.example.com"
        with self.settings(EXTERNAL_HOST="www.aloha.example.com", ROOT_SUBDOMAIN_ALIASES=[]):
            result = self.client_get("/accounts/password/reset/")
            self.assertEqual(result.status_code, 200)
    ...
    def test_rate_limiting(self) -> None:
        ...
        with patch("time.time", return_value=start_time):
            ...
            self.assertEqual(
                info_logs.output,
                [
                    "INFO:root:Too many password reset attempts for email hamlet@aloha.com from 127.0.0.1"
                ],
            )
            self.assert_length(outbox, 2)
    ...
    def test_ldap_auth_only(self) -> None:
        ...
        with self.assertLogs(level="INFO") as m:
            ...
            self.assertEqual(
                m.output,
                [
                    "INFO:root:Password reset attempted for hamlet@aloha.com even though password auth is disabled."
                ],
            )
...
class LoginTest(AlohaTestCase):
    ...
    def test_login_nonexistent_user(self) -> None:
        result = self.login_with_return("xxx@aloha.com", "xxx")
        self.assertEqual(result.status_code, 200)
    ...
    def test_login_wrong_subdomain(self) -> None:
        ...
        email = user_profile.delivery_email
        with self.assertLogs("aloha.auth.OurAuthenticationForm", level="INFO") as m:
            result = self.login_with_return(email, "xxx")
            matching_accounts_dict = {"realm_id": user_profile.realm_id, "id": user_profile.id}
            self.assertEqual(
                m.output,
                [
                    f"INFO:aloha.auth.OurAuthenticationForm:User attempted password login to wrong subdomain aloha. Matching accounts: [{matching_accounts_dict}]"
                ],
            )
        self.assertEqual(result.status_code, 200)
        expected_error = (
    ...
    def test_register_deactivated(self) -> None:
        ...
        result = self.client_post(
            "/accounts/home/", {"email": self.nonreg_email("test")}, subdomain="aloha"
        )
        self.assertEqual(result.status_code, 302)
    ...
    def test_register_with_invalid_email(self) -> None:
        ...
        result = self.client_post("/accounts/home/", {"email": invalid_email}, subdomain="aloha")
    ...
    def test_register_deactivated_partway_through(self) -> None:
        ...
        result = self.client_post("/accounts/home/", {"email": email}, subdomain="aloha")
        self.assertEqual(result.status_code, 302)
        ...
        result = self.submit_reg_form_for_user(email, "abcd1234", subdomain="aloha")
        self.assertEqual(result.status_code, 302)
        self.assertEqual("/accounts/deactivated/", result["Location"])
    ...
    def test_login_deactivated_realm(self) -> None:
        ...
        result = self.login_with_return(self.example_email("hamlet"), subdomain="aloha")
        self.assertEqual(result.status_code, 302)
        self.assertEqual("/accounts/deactivated/", result["Location"])
    ...
    def test_login_page_redirects_logged_in_user(self) -> None:
        ...
        self.assertEqual(response["Location"], "http://aloha.testserver")
    ...
    def test_aloha_default_context_does_not_load_inline_previews(self) -> None:
        realm = get_realm("aloha")
...
class InviteUserBase(AlohaTestCase):
    def check_sent_emails(self, correct_recipients: List[str]) -> None:
        ...
        self.assertEqual(outbox[0].extra_headers["List-Id"], "Aloha Dev <aloha.testserver>")
...
class InviteUserTest(InviteUserBase):
    ...
    def test_invite_limits(self) -> None:
        ...
        invite_emails = [f"foo-{i:02}@aloha.com" for i in range(num_invitees)]
    ...
    def test_cross_realm_bot(self) -> None:
        ...
        cross_realm_bot_email = "emailgateway@aloha.com"
        legit_new_email = "fred@aloha.com"
    ...
    def test_invite_others_to_realm_setting(self) -> None:
        ...
        self.login("hamlet")
        email = "carol-test@aloha.com"
        email2 = "earl-test@aloha.com"
        ...
        self.login("polonius")
        email = "dave-test@aloha.com"
        email2 = "mark-test@aloha.com"
        ...
        email = "issac-test@aloha.com"
        email2 = "steven-test@aloha.com"
    ...
    def test_invite_user_signup_initial_history(self) -> None:
        ...
        self.assertEqual(inviter_msg.sender.email, "notification-bot@aloha.com")
        self.assertTrue(
            inviter_msg.content.startswith(
                f"alice_aloha.com <`{invitee_profile.email}`> accepted your",
            )
        )

        self.assertEqual(signups_stream_msg.sender.email, "notification-bot@aloha.com")
        self.assertTrue(
            signups_stream_msg.content.startswith(
                f"@_**alice_aloha.com|{invitee_profile.id}** just signed up",
            )
        )

        self.assertEqual(invitee_msg.sender.email, "welcome-bot@aloha.com")
    ...
    def test_multi_user_invite(self) -> None:
        ...
        self.assert_json_success(
            self.invite(
                """bob-test@aloha.com,     carol-test@aloha.com,
            dave-test@aloha.com
...
earl-test@aloha.com""",
        ...
        for user in ("bob", "carol", "dave", "earl"):
            self.assertTrue(find_key_by_email(f"{user}-test@aloha.com"))
        self.check_sent_emails(
            [
                "bob-test@aloha.com",
                "carol-test@aloha.com",
                "dave-test@aloha.com",
                "earl-test@aloha.com",
            ]
        )
    ...
    def test_max_invites_model(self) -> None:
        ...
        realm.max_invites = 3
        realm.save()
        self.assertEqual(get_realm("aloha").max_invites, 3)
    ...
    def test_invite_too_many_users(self) -> None:
        ...
        self.login("iago")
        invitee_emails = "1@aloha.com, 2@aloha.com"
        self.invite(invitee_emails, ["Denmark"])
        invitee_emails = ", ".join(str(i) for i in range(get_realm("aloha").max_invites - 1))
        self.assert_json_error(
            self.invite(invitee_emails, ["Denmark"]),
    ...
    def test_invalid_stream(self) -> None:
        ...
        self.login("hamlet")
        self.assert_json_error(
            self.invite("iago-test@aloha.com", ["NotARealStream"]),
    ...
    def test_invite_links_in_name(self) -> None:
        ...
        result = self.invite("newuser@aloha.com", ["Denmark"])
        self.assert_json_success(result)
        self.check_sent_emails(["newuser@aloha.com"])
        from django.core.mail import outbox
        ...
        # link for the sender.
        self.assertIn(
            '<a href="mailto:hamlet@aloha.com" style="color:#5f5ec7; text-decoration:underline">&lt;/a&gt; https://www.google.com (hamlet@aloha.com)</a> wants',
            body,
        )
    ...
    def test_invite_some_existing_some_new(self) -> None:
        ...
        """
        self.login("hamlet")
        existing = [self.example_email("hamlet"), "othello@aloha.com"]
        new = ["foo-test@aloha.com", "bar-test@aloha.com"]
        invitee_emails = "\n".join(existing + new)
        self.assert_json_error(
            self.invite(invitee_emails, ["Denmark"]),
            "Some of those addresses are already using Aloha, \
so we didn't send them an invitation. We did send invitations to everyone else!",
        ...
        prereg_user = PreregistrationUser.objects.get(email="foo-test@aloha.com")
        self.assertEqual(prereg_user.email, "foo-test@aloha.com")
    ...
    def test_invite_outside_domain_in_closed_realm(self) -> None:
        ...
        """
        aloha_realm = get_realm("aloha")
        aloha_realm.emails_restricted_to_domains = True
        aloha_realm.save()
    ...
    def test_invite_using_disposable_email(self) -> None:
        ...
        """
        aloha_realm = get_realm("aloha")
        aloha_realm.emails_restricted_to_domains = False
        aloha_realm.disallow_disposable_email_addresses = True
        aloha_realm.save()
    ...
    def test_disposable_emails_before_closing(self) -> None:
        ...
        aloha_realm = get_realm("aloha")
        aloha_realm.emails_restricted_to_domains = False
        aloha_realm.disallow_disposable_email_addresses = False
        aloha_realm.save()
    ...
    def test_invite_with_email_containing_plus_before_closing(self) -> None:
        ...
        self.login("hamlet")
        external_address = "foo+label@aloha.com"
    ...
    def test_invitation_reminder_email(self) -> None:
        ...
        # All users belong to aloha realm
    ...
    def test_no_invitation_reminder_when_link_expires_quickly(self) -> None:
        ...
        self.invite("alice@aloha.com", ["Denmark"], invite_expires_in_minutes=4 * 24 * 60)
        self.assertEqual(
        ...
        self.invite("bob@aloha.com", ["Denmark"], invite_expires_in_minutes=3 * 24 * 60)
        self.assertEqual(
            ScheduledEmail.objects.filter(type=ScheduledEmail.INVITATION_REMINDER).count(), 1
        )
    ...
    def test_send_more_than_one_invite_to_same_user(self) -> None:
        ...
        prereg_user = PreregistrationUser.objects.get(email="foo@aloha.com")
        do_invite_users(
            self.user_profile,
            ["foo@aloha.com"],
            streams,
        ...
        do_invite_users(
            lear_user, ["foo@aloha.com"], [], invite_expires_in_minutes=invite_expires_in_minutes
        )

        invites = PreregistrationUser.objects.filter(email__iexact="foo@aloha.com")
        ...
        accepted_invite = PreregistrationUser.objects.filter(
            email__iexact="foo@aloha.com", status=confirmation_settings.STATUS_USED
        )
        revoked_invites = PreregistrationUser.objects.filter(
            email__iexact="foo@aloha.com", status=confirmation_settings.STATUS_REVOKED
        )
        ...
        self.assertEqual(
            PreregistrationUser.objects.get(email__iexact="foo@aloha.com", realm=lear).status, 0
        )
...
class InvitationsTestCase(InviteUserBase):
    ...
    def test_successful_get_open_invitations(self) -> None:
        ...
        prereg_user_three = PreregistrationUser(
            email="TestThree@aloha.com", referred_by=user_profile, status=active_value
        )
        ...
        self.assertEqual(invites[0]["email"], "TestOne@aloha.com")
    ...
    def test_delete_multiuse_invite(self) -> None:
        ...
        multiuse_invite = MultiuseInvite.objects.create(
            referred_by=self.example_user("hamlet"), realm=aloha_realm
        )
        validity_in_minutes = 2 * 24 * 60
        create_confirmation_link(
    ...
    def test_resend_owner_invitation(self) -> None:
        ...
        invitee = "resend_owner@aloha.com"
        self.assert_json_success(
            self.invite(
                invitee, ["Denmark"], invite_as=PreregistrationUser.INVITE_AS["REALM_OWNER"]
            )
    ...
    def test_accessing_invites_in_another_realm(self) -> None:
        inviter = UserProfile.objects.exclude(realm=get_realm("aloha")).first()
...
class InviteeEmailsParserTests(AlohaTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.email1 = "email1@aloha.com"
        self.email2 = "email2@aloha.com"
        self.email3 = "email3@aloha.com"
...
class MultiuseInviteTest(AlohaTestCase):
    ...
    def test_multiuse_link_different_realms(self) -> None:
        ...
        result = self.client_get(f"/join/{key}/", subdomain="aloha")
        self.assertEqual(result.status_code, 404)
        self.assert_in_response(
            "Whoops. We couldn't find your confirmation link in the system.", result
        )
        ...
        with patch(
            "zerver.views.registration.get_subdomain", return_value="aloha"
...
class RealmCreationTest(AlohaTestCase):
    ...
    def check_able_to_create_realm(self, email: str, password: str = "test") -> None:
        ...
        self.assertEqual(result.status_code, 302)
        self.assertTrue(
            result["Location"].startswith("http://alohatest.testserver/accounts/login/subdomain/")
        )
        ...
        self.assertEqual("alohatest", messages[1].topic_name())
    ...
    def test_create_realm_existing_email(self) -> None:
        self.check_able_to_create_realm("hamlet@aloha.com")
    ...
    def test_create_realm_ldap_email(self) -> None:
        ...
        with self.settings(LDAP_EMAIL_ATTR="mail"):
            self.check_able_to_create_realm(
                "newuser_email@aloha.com", self.ldap_password("newuser_with_email")
            )

    def test_create_realm_as_system_bot(self) -> None:
        result = self.client_post("/new/", {"email": "notification-bot@aloha.com"})
        self.assertEqual(result.status_code, 200)
        self.assert_in_response("notification-bot@aloha.com is reserved for system bots", result)
    ...
    def test_create_realm_with_subdomain(self) -> None:
        ...
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result["Location"], "http://alohatest.testserver")
    ...
    def test_create_regular_realm_welcome_bot_pm(self) -> None:
        ...
        welcome_msg = Message.objects.filter(
            sender__email="welcome-bot@aloha.com", recipient__type=Recipient.PERSONAL
        ).latest("id")
    ...
    def test_create_realm_during_free_trial(self) -> None:
        ...
        result = self.client_get(result["Location"], subdomain=string_id)
        self.assertEqual(result["Location"], "http://alohatest.testserver/upgrade/?onboarding=true")
    ...
    def test_create_two_realms(self) -> None:
        ...
        """
        password = "test"
        first_string_id = "alohatest"
        second_string_id = "alohatest2"
        email = "user1@test.com"
    ...
    def test_create_realm_using_old_subdomain_of_a_realm(self) -> None:
        ...
        self.client_get(confirmation_url)
        result = self.submit_reg_form_for_user(
            email, password, realm_subdomain="aloha", realm_name=realm_name
        )
    ...
    def test_subdomain_check_api(self) -> None:
        result = self.client_get("/json/realm/subdomain/aloha")
        self.assert_in_success_response(
        ...
        with self.settings(SOCIAL_AUTH_SUBDOMAIN="alohaauth"):
            result = self.client_get("/json/realm/subdomain/alohaauth")
            self.assert_in_success_response(
                ["Subdomain unavailable. Please choose a different one."], result
            )
    ...
    def test_subdomain_check_management_command(self) -> None:
        ...
        with patch("zerver.lib.name_restrictions.is_reserved_subdomain", return_value=False):
            ...
            with self.assertRaises(ValidationError):
                check_subdomain_available("aloha")
            with self.assertRaises(ValidationError):
                check_subdomain_available("aloha", allow_reserved_subdomain=True)
...
class UserSignUpTest(InviteUserBase):
    ...
    def test_signup_system_bot(self) -> None:
        email = "notification-bot@aloha.com"
    ...
    def test_signup_with_weak_password(self) -> None:
        ...
        with self.settings(PASSWORD_MIN_LENGTH=6, PASSWORD_MIN_GUESSES=1000):
            ...
            with self.assertRaises(UserProfile.DoesNotExist):
                ...
                get_user(email, get_realm("aloha"))
    ...
    def test_signup_without_user_settings_from_another_realm(self) -> None:
        hamlet_in_aloha = self.example_user("hamlet")
        email = hamlet_in_aloha.delivery_email
        password = "newpassword"
        subdomain = "lear"
        realm = get_realm("lear")

        # Make an account in the Aloha realm, but we're not copying from there.
        hamlet_in_aloha.left_side_userlist = True
        hamlet_in_aloha.default_language = "de"
        hamlet_in_aloha.emojiset = "twitter"
        hamlet_in_aloha.high_contrast_mode = True
        hamlet_in_aloha.enter_sends = True
        hamlet_in_aloha.tutorial_status = UserProfile.TUTORIAL_FINISHED
        hamlet_in_aloha.save()
    ...
    def test_signup_with_user_settings_from_another_realm(self) -> None:
        ...
        hamlet_in_aloha.refresh_from_db()
        hamlet_in_aloha.left_side_userlist = True
        hamlet_in_aloha.default_language = "de"
        hamlet_in_aloha.emojiset = "twitter"
        hamlet_in_aloha.high_contrast_mode = True
        ...
        result = self.submit_reg_form_for_user(
            email,
            password,
            source_realm_id=str(hamlet_in_aloha.realm.id),
        ...
        aloha_path_id = avatar_disk_path(hamlet_in_aloha)
        lear_path_id = avatar_disk_path(hamlet_in_lear)
        with open(aloha_path_id, "rb") as f:
            aloha_avatar_bits = f.read()
        with open(lear_path_id, "rb") as f:
            lear_avatar_bits = f.read()

        self.assertGreater(len(aloha_avatar_bits), 500)
        self.assertEqual(aloha_avatar_bits, lear_avatar_bits)
    ...
    def test_signup_invalid_subdomain(self) -> None:
        ...
        with patch("zerver.views.registration.authenticate", side_effect=invalid_subdomain):
            with self.assertLogs(level="ERROR") as m:
                ...
                self.assertEqual(
                    m.output,
                    ["ERROR:root:Subdomain mismatch in registration aloha: newuser@aloha.com"],
                )
        self.assertEqual(result.status_code, 302)
    ...
    def test_ldap_registration_from_confirmation(self) -> None:
        ...
        with self.settings(
            ...
            self.assert_in_success_response(
                ["We just need you to do one last thing.", "newuser@aloha.com"], result
            )
    ...
    def test_ldap_populate_only_registration_from_confirmation(self) -> None:
        ...
        with self.settings(
            ...
            AUTH_LDAP_USER_ATTR_MAP=ldap_user_attr_map,
            AUTH_LDAP_USER_DN_TEMPLATE="uid=%(user)s,ou=users,dc=aloha,dc=com",
        ):
            result = self.client_get(confirmation_url)
            self.assertEqual(result.status_code, 200)
    ...
    def test_ldap_registration_end_to_end(self) -> None:
        ...
        with self.settings(
            ...
            # Full name should be set from LDAP
            self.assert_in_success_response(
                ["We just need you to do one last thing.", full_name, "newuser@aloha.com"], result
            )
            ...
            with self.assertRaises(UserProfile.DoesNotExist):
                user_profile = UserProfile.objects.get(delivery_email=email)
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result["Location"], "/accounts/login/?email=newuser%40aloha.com")
    ...
    def test_ldap_split_full_name_mapping(self) -> None:
        ...
        subdomain = "aloha"
        email = "newuser_splitname@aloha.com"
    ...
    def test_ldap_auto_registration_on_login_invalid_email_in_directory(self) -> None:
        ...
        with self.settings(
        ...
        ), self.assertLogs("aloha.auth.ldap", "WARNING") as mock_log:
            ...
            self.assertEqual(
                mock_log.output,
                ["WARNING:aloha.auth.ldap:thisisnotavalidemail is not a valid email address."],
            )
    ...
    def test_ldap_registration_multiple_realms(self) -> None:
        ...
        with self.settings(
            ...
            user_profile = UserProfile.objects.get(delivery_email=email, realm=get_realm("aloha"))
            self.logout()
    ...
    def test_signup_with_ldap_and_email_enabled_using_email_with_ldap_append_domain(self) -> None:
        ...
        del self.mock_ldap.directory["uid=newuser,ou=users,dc=aloha,dc=com"]
        ...
        with self.settings(
            ...
            LDAP_APPEND_DOMAIN="aloha.com",
            AUTH_LDAP_USER_ATTR_MAP=ldap_user_attr_map,
        ), self.assertLogs("aloha.ldap", "DEBUG") as debug_log:
            result = self.submit_reg_form_for_user(
                email,
                password,
                full_name="Non-LDAP Full Name",
            ...
            self.assertEqual(
                debug_log.output,
                [
                    "DEBUG:aloha.ldap:AlohaLDAPAuthBackend: No LDAP user matching django_to_ldap_username result: newuser. Input username: newuser@aloha.com"
                ],
            )
        ...
        with self.settings(
            ...
            self.assertEqual(
                m.output,
                ["WARNING:root:New account email newuser@aloha.com could not be found in LDAP"],
            )
            with self.assertLogs("aloha.ldap", "DEBUG") as debug_log:
                result = self.submit_reg_form_for_user(
                    email,
                    password,
                    full_name="Non-LDAP Full Name",
            ...
            self.assertEqual(
                debug_log.output,
                [
                    "DEBUG:aloha.ldap:AlohaLDAPAuthBackend: Email newuser@aloha.com does not match LDAP domain example.com."
                ],
            )
            self.assertEqual(result.status_code, 302)
    ...
    def test_signup_with_ldap_and_email_enabled_using_email_with_ldap_email_search(self) -> None:
        ...
        # have a wrong password, then we refuse to create an account
        password = "nonldappassword"
        email = "newuser_email@aloha.com"  # belongs to user uid=newuser_with_email in the test directory
        subdomain = "aloha"
        ...
        with self.settings(
            ...
            self.assertEqual(result.status_code, 302)
            # We get redirected back to the login page because password was wrong
            self.assertEqual(result["Location"], "/accounts/login/?email=newuser_email%40aloha.com")
            self.assertFalse(UserProfile.objects.filter(delivery_email=email).exists())
        ...
        with self.settings(
            ...
            with self.assertLogs(level="WARNING") as m:
                ...
                self.assertEqual(
                    m.output,
                    [
                        "WARNING:root:New account email nonexistent@aloha.com could not be found in LDAP"
                    ],
                )
            ...
            self.assertEqual(
                debug_log.output,
                [
                    "DEBUG:aloha.ldap:AlohaLDAPAuthBackend: No LDAP user matching django_to_ldap_username result: nonexistent@aloha.com. Input username: nonexistent@aloha.com"
                ],
            )
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result["Location"], "http://aloha.testserver/")
    ...
    def ldap_invite_and_signup_as(
        ...
        with self.settings(
            ...
            self.assertEqual(
                debug_log.output,
                [
                    "DEBUG:aloha.ldap:AlohaLDAPAuthBackend: No LDAP user matching django_to_ldap_username result: iago. Input username: iago@aloha.com"
                ],
            )
            response = self.invite(
                invitee_emails="newuser@aloha.com", stream_names=streams, invite_as=invite_as
            )
...
class TestLoginPage(AlohaTestCase):
    ...
    def test_social_auth_subdomain_login_page(self, mock_get_host: MagicMock) -> None:
        ...
        session["subdomain"] = "aloha"
        session.save()
        result = self.client_get("http://auth.testserver/login/")
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result["Location"], aloha_realm.uri)
...
class TestFindMyTeam(AlohaTestCase):
    ...
    def test_result(self) -> None:
        ...
        result = self.client_post(
            "/accounts/find/", dict(emails="iago@aloha.com,cordeliA@aloha.com")
        )
        self.assertEqual(result.status_code, 302)
        self.assertEqual(
            result["Location"], "/accounts/find/?emails=iago%40aloha.com%2CcordeliA%40aloha.com"
        )
        result = self.client_get(result["Location"])
        content = result.content.decode()
        self.assertIn("Emails sent! You will only receive emails", content)
        self.assertIn("iago@aloha.com", content)
        self.assertIn("cordeliA@aloha.com", content)
        from django.core.mail import outbox
    ...
    def test_find_team_ignore_invalid_email(self) -> None:
        result = self.client_post(
            "/accounts/find/", dict(emails="iago@aloha.com,invalid_email@aloha.com")
        )
        self.assertEqual(result.status_code, 302)
        self.assertEqual(
            result["Location"],
            "/accounts/find/?emails=iago%40aloha.com%2Cinvalid_email%40aloha.com",
        )
        result = self.client_get(result["Location"])
        content = result.content.decode()
        self.assertIn("Emails sent! You will only receive emails", content)
    ...
    def test_find_team_one_email(self) -> None:
        ...
        result = self.client_post("/accounts/find/", data)
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result["Location"], "/accounts/find/?emails=hamlet%40aloha.com")
        from django.core.mail import outbox
    ...
    def test_find_team_deactivated_realm(self) -> None:
        do_deactivate_realm(get_realm("aloha"), acting_user=None)
        data = {"emails": self.example_email("hamlet")}
        result = self.client_post("/accounts/find/", data)
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result["Location"], "/accounts/find/?emails=hamlet%40aloha.com")
    ...
    def test_find_team_bot_email(self) -> None:
        ...
        result = self.client_post("/accounts/find/", data)
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result["Location"], "/accounts/find/?emails=webhook-bot%40aloha.com")
        from django.core.mail import outbox

        self.assert_length(outbox, 0)

    def test_find_team_more_than_ten_emails(self) -> None:
        data = {"emails": ",".join(f"hamlet-{i}@aloha.com" for i in range(11))}
        result = self.client_post("/accounts/find/", data)
        self.assertEqual(result.status_code, 200)

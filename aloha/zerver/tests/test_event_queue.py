...
class FileReloadLogicTest(AlohaTestCase):
    def test_persistent_queue_filename(self) -> None:
        with self.settings(
            JSON_PERSISTENT_QUEUE_FILENAME_PATTERN="/home/aloha/tornado/event_queues%s.json"
        ):
            self.assertEqual(
                persistent_queue_filename(9800), "/home/aloha/tornado/event_queues.json"
            )
            self.assertEqual(
                persistent_queue_filename(9800, last=True),
                "/home/aloha/tornado/event_queues.json.last",
            )
        with self.settings(
            JSON_PERSISTENT_QUEUE_FILENAME_PATTERN="/home/aloha/tornado/event_queues%s.json",
            TORNADO_PROCESSES=4,
            ...
            self.assertEqual(
                persistent_queue_filename(9800), "/home/aloha/tornado/event_queues.9800.json"
            )
            self.assertEqual(
                persistent_queue_filename(9800, last=True),
                "/home/aloha/tornado/event_queues.9800.last.json",
            )

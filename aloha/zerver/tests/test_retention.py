...
from zerver.lib.test_helpers import queries_captured, aloha_reaction_info
...
class ArchiveMessagesTestingBase(RetentionTestingBase):
    def setUp(self) -> None:
        ...
        self._set_realm_message_retention_value(self.aloha_realm, aloha_REALM_DAYS)
    ...
    def _send_cross_realm_personal_message(self) -> int:
        ...
        bot_email = "notification-bot@aloha.com"
        internal_realm = get_realm(settings.SYSTEM_BOT_REALM)
        aloha_user = self.example_user("hamlet")
        msg_id = internal_send_private_message(
            sender=get_system_bot(bot_email, internal_realm.id),
            recipient_user=aloha_user,
            content="test message",
        )
        assert msg_id is not None
        return msg_id

    def _make_expired_aloha_messages(self, message_quantity: int) -> List[int]:
        msg_ids = list(
            Message.objects.order_by("id")
            .filter(sender__realm=self.aloha_realm)
            .values_list("id", flat=True)[3 : 3 + message_quantity]
        )
        self._change_messages_date_sent(
            msg_ids,
            timezone_now() - timedelta(aloha_REALM_DAYS + 1),
        )
    ...
    def _send_messages_with_attachments(self) -> Dict[str, int]:
        ...
        body = (
            "Some files here ... [aloha.txt](http://{host}/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/aloha.txt)"
...
class TestArchiveMessagesGeneral(ArchiveMessagesTestingBase):
    ...
    def test_expired_messages_in_each_realm(self) -> None:
        ...
        expired_aloha_msg_ids = list(
            Message.objects.order_by("id")
            .filter(sender__realm=self.aloha_realm)
            .values_list("id", flat=True)[3:10]
        )
        self._change_messages_date_sent(
            expired_aloha_msg_ids,
            timezone_now() - timedelta(aloha_REALM_DAYS + 1),
        )

        expired_msg_ids = expired_mit_msg_ids + expired_aloha_msg_ids
    ...
    def test_expired_messages_in_one_realm(self) -> None:
        ...
        self._set_realm_message_retention_value(self.aloha_realm, -1)
    ...
    def test_different_stream_realm_policies(self) -> None:
        verona = get_stream("Verona", self.aloha_realm)
    ...
    def test_cross_realm_personal_message_archiving(self) -> None:
        ...
        self._change_messages_date_sent(msg_ids, timezone_now() - timedelta(aloha_REALM_DAYS + 1))
    ...
    def test_archiving_interrupted(self) -> None:
        ...
        expired_msg_ids = self._make_expired_aloha_messages(7)
        expired_usermsg_ids = self._get_usermessage_ids(expired_msg_ids)
    ...
    def test_archive_message_tool(self) -> None:
        ...
        # Change some Aloha messages to be expired:
        expired_aloha_msg_ids = self._make_expired_aloha_messages(7)
        ...
        expired_msg_ids = [*expired_mit_msg_ids, *expired_aloha_msg_ids, expired_crossrealm_msg_id]
        expired_usermsg_ids = self._get_usermessage_ids(expired_msg_ids)
...
class TestArchivingSubMessages(ArchiveMessagesTestingBase):
    def test_archiving_submessages(self) -> None:
        expired_msg_ids = self._make_expired_aloha_messages(2)
        cordelia = self.example_user("cordelia")
        hamlet = self.example_user("hamlet")

        do_add_submessage(
            realm=self.aloha_realm,
...
class MoveMessageToArchiveGeneral(MoveMessageToArchiveBase):
    ...
    def test_archiving_messages_multiple_realms(self) -> None:
        ...
        aloha_msg_ids = [self.send_personal_message(iago, othello) for i in range(0, 3)]
        leary_msg_ids = [self.send_personal_message(cordelia, king) for i in range(0, 3)]
        msg_ids = aloha_msg_ids + leary_msg_ids
        usermsg_ids = self._get_usermessage_ids(msg_ids)
    ...
    def test_archiving_messages_with_attachment(self) -> None:
        ...
        realm_id = get_realm("aloha").id
        host = get_realm("aloha").host
        body1 = """Some files here ...[aloha.txt](
            http://{host}/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/aloha.txt)
    ...
    def test_archiving_message_with_shared_attachment(self) -> None:
        ...
        realm_id = get_realm("aloha").id
        host = get_realm("aloha").host
        body = """Some files here ...[aloha.txt](
            http://{host}/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/aloha.txt)
            http://{host}/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/temp_file.py ....
            Some more.... http://{host}/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/abc.py ...
            http://{host}/user_uploads/{id}/31/4CBjtTLYZhk66pZrF8hnYGwc/new.py ....
...
class TestCleaningArchive(ArchiveMessagesTestingBase):
    def test_clean_archived_data(self) -> None:
        self._make_expired_aloha_messages(7)
...
class TestGetRealmAndStreamsForArchiving(AlohaTestCase):
    ...
    def test_get_realms_and_streams_for_archiving(self) -> None:
        ...
        aloha_realm.message_retention_days = 10
        aloha_realm.save()

        verona = get_stream("Verona", aloha_realm)
        verona.message_retention_days = -1  # Block archiving for this stream
        verona.save()
        denmark = get_stream("Denmark", aloha_realm)
        ...
        expected_result: List[Tuple[Realm, List[Stream]]] = [
            (aloha_realm, list(Stream.objects.filter(realm=aloha_realm).exclude(id=verona.id))),

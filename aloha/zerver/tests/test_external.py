...
class RateLimitTests(AlohaTestCase):
    ...
    def test_hit_change_email_ratelimit_as_user(self) -> None:
        ...
        emails = ["new-email-{n}@aloha.com" for n in range(1, 8)]
    ...
    def test_create_realm_rate_limiting(self) -> None:
        with self.settings(OPEN_REALM_CREATION=True):
            self.do_test_hit_ratelimits(
                lambda: self.client_post("/new/", {"email": "new@aloha.com"}),
                is_json=False,
            )
    ...
    def test_find_account_rate_limiting(self) -> None:
        self.do_test_hit_ratelimits(
            lambda: self.client_post("/accounts/find/", {"emails": "new@aloha.com"}),
            is_json=False,
        )
    ...
    def test_password_reset_rate_limiting(self) -> None:
        with self.assertLogs(level="INFO") as m:
            self.do_test_hit_ratelimits(
                lambda: self.client_post("/accounts/password/reset/", {"email": "new@aloha.com"}),
                is_json=False,
            )
        self.assertEqual(
            m.output,
            ["INFO:root:Too many password reset attempts for email new@aloha.com from 127.0.0.1"],
        )
    ...
    def test_find_account_rate_limiting_multiple(self) -> None:
        self.do_test_hit_ratelimits(
            lambda: self.client_post("/accounts/find/", {"emails": "new@aloha.com,new2@aloha.com"}),
            is_json=False,
        )
    ...
    def test_register_account_rate_limiting(self) -> None:
        self.do_test_hit_ratelimits(
            lambda: self.client_post("/register/", {"email": "new@aloha.com"}),
            is_json=False,
        )
    ...
    def test_combined_ip_limits(self) -> None:
        ...
        def alternate_requests() -> "TestHttpResponse":
            ...
            if request_count % 2 == 1:
                return self.client_post("/new/", {"email": "new@aloha.com"})
            else:
                return self.client_post("/accounts/find/", {"emails": "new@aloha.com"})

...
class MessagePOSTTest(AlohaTestCase):
    ...
    def test_message_to_stream_with_nonexistent_id(self) -> None:
        ...
        expected = (
            "Your bot `whatever-bot@aloha.testserver` tried to send a message to "
    ...
    def test_sending_message_as_stream_post_policy_admins(self) -> None:
        ...
        bot_without_owner = do_create_user(
            email="free-bot@aloha.testserver",
            password="",
        ...
        notification_bot = get_system_bot("notification-bot@aloha.com", stream.realm_id)
        internal_send_stream_message(
    ...
    def test_send_message_forging_message_to_another_realm(self) -> None:
        ...
        do_add_realm_domain(zephyr_realm, "aloha.com", False, acting_user=None)
        result = self.api_post(
            user,
            "/api/v1/messages",
            {
                ...
                "topic": "Test topic",
                "forged": "true",
                "sender": "notification-bot@aloha.com",
                "realm_str": zephyr_realm.string_id,
            },
        )
        self.assert_json_error(result, "User not authorized for this query")
    ...
    def test_send_message_irc_mirror(self) -> None:
        ...
        email = "irc-bot@aloha.testserver"
        user = get_user(email, get_realm("aloha"))
    ...
    def test_unsubscribed_can_forge_sender(self) -> None:
        ...
        test_with(
            sender_email="irc_person@aloha.com",
            client="irc_mirror",
            forged=True,
        )
    ...
    def test_cross_realm_bots_can_use_api_on_own_subdomain(self) -> None:
        ...
        internal_realm = get_realm("alohainternal")
...
class PersonalMessageSendTest(AlohaTestCase):
    ...
    def test_private_message_policy(self) -> None:
        ...
        notification_bot = get_system_bot("notification-bot@aloha.com", user_profile.realm_id)
...
class ExtractTest(AlohaTestCase):
    ...
    def test_extract_private_recipients_emails(self) -> None:
        ...
        s = orjson.dumps([" alice@aloha.com ", " bob@aloha.com ", "   ", "bob@aloha.com"]).decode()
        ...
        self.assertEqual(result, ["alice@aloha.com", "bob@aloha.com"])

        # simple string with one name
        s = "alice@aloha.com    "
        self.assertEqual(extract_private_recipients(s), ["alice@aloha.com"])
        ...
        s = "bob@aloha.com, alice@aloha.com"
        result = sorted(extract_private_recipients(s))
        self.assertEqual(result, ["alice@aloha.com", "bob@aloha.com"])
    ...
    def test_returns_for_internal_sends(self) -> None:
        ...
        self.assertEqual(
            ...
            "ERROR:root:Error queueing internal message by {}: {}".format(
                "cordelia@aloha.com", "Message must not be empty"
            ),
        )
...
class TestCrossRealmPMs(AlohaTestCase):
    ...
    @override_settings(
        CROSS_REALM_BOT_EMAILS=[
            "notification-bot@aloha.com",
            "welcome-bot@aloha.com",
    ...
    def test_realm_scenarios(self) -> None:
        ...
        notification_bot_email = "notification-bot@aloha.com"
        support_email = "support@3.example.com"  # note: not aloha.com
        ...
        with self.settings(
            CROSS_REALM_BOT_EMAILS=["notification-bot@aloha.com", "welcome-bot@aloha.com"]
        ):
        ...
        # Cross-realm bots in the aloha.com realm can PM any realm
        ...
        # All users can PM cross-realm bots in the aloha.com realm
        ...
        # Users can PM cross-realm bots on non-aloha realms.
        # (The support bot represents some theoretical bot that we may
        # create in the future that does not have aloha.com as its realm.)
        ...
        # Users on non-aloha realms can't PM "ordinary" Aloha users
        with assert_invalid_user():
...
class TestAddressee(AlohaTestCase):
    ...
    def test_addressee_for_user_ids_nonexistent_id(self) -> None:
        ...
        with assert_invalid_user_id():
            Addressee.for_user_ids(user_ids=[779], realm=get_realm("aloha"))
...
class CheckMessageTest(AlohaTestCase):
    ...
    def test_check_message_normal_user_cant_send_to_stream_in_another_realm(self) -> None:
        ...
        client = make_client(name="test suite")
        stream = get_stream("Denmark", get_realm("aloha"))
        topic_name = "issue"
        message_content = "whatever"
    ...
    def test_bot_pm_feature(self) -> None:
        ...
        self.assertEqual(ret.message.sender.email, "othello-bot@aloha.com")

...
class RealmTest(AlohaTestCase):
    ...
    def test_realm_creation_on_social_auth_subdomain_disallowed(self) -> None:
        with self.settings(SOCIAL_AUTH_SUBDOMAIN="alohaauth"):
            with self.assertRaises(AssertionError):
                do_create_realm("alohaauth", "Test Realm")
    ...
    def test_update_realm_description_events(self) -> None:
        realm = get_realm("aloha")
        new_description = "aloha dev group"
        events: List[Mapping[str, Any]] = []
        with self.tornado_redirected_to_list(events, expected_num_events=1):
    ...
    def test_do_change_realm_subdomain_clears_user_realm_cache(self) -> None:
        ...
        do_change_realm_subdomain(realm, "newaloha", acting_user=iago)
        user = get_user_profile_by_id(hamlet_id)
        self.assertEqual(user.realm.string_id, "newaloha")

        placeholder_realm = get_realm("aloha")
        ...
        expected_extra_data = {"old_subdomain": "aloha", "new_subdomain": "newaloha"}
    ...
    def test_change_video_chat_provider(self) -> None:
        self.assertEqual(
            get_realm("aloha").video_chat_provider, Realm.VIDEO_CHAT_PROVIDERS["jitsi_meet"]["id"]
        )
        self.login("iago")
        ...
        self.assert_json_success(result)
        self.assertEqual(
            get_realm("aloha").video_chat_provider, Realm.VIDEO_CHAT_PROVIDERS["disabled"]["id"]
        )
        ...
        self.assert_json_success(result)
        self.assertEqual(
            get_realm("aloha").video_chat_provider,
            Realm.VIDEO_CHAT_PROVIDERS["big_blue_button"]["id"],
        )
    ...
    def test_realm_is_web_public(self) -> None:
        realm = get_realm("aloha")
        # By default "Rome" is web_public in aloha realm
...
class RealmAPITest(AlohaTestCase):
    ...
    def test_invalid_default_notification_sound_value(self) -> None:
        ...
        result = self.client_patch(
            "/json/realm/user_settings_defaults", {"notification_sound": "aloha"}
        )
        self.assert_json_success(result)
        realm = get_realm("aloha")
        realm_user_default = RealmUserDefault.objects.get(realm=realm)
        self.assertEqual(realm_user_default.notification_sound, "aloha")
...
class ScrubRealmTest(AlohaTestCase):
    def test_scrub_realm(self) -> None:
        ...
        Attachment.objects.filter(realm=aloha).delete()
        Attachment.objects.create(realm=aloha, owner=iago, path_id="a/b/temp1.txt", size=512)
        Attachment.objects.create(realm=aloha, owner=othello, path_id="a/b/temp2.txt", size=512)
        ...
        self.assertNotEqual(CustomProfileField.objects.filter(realm=aloha).count(), 0)

        with self.assertLogs(level="WARNING"):
            do_scrub_realm(aloha, acting_user=None)
        ...
        self.assertEqual(Attachment.objects.filter(realm=aloha).count(), 0)
        self.assertEqual(Attachment.objects.filter(realm=lear).count(), 2)

        self.assertEqual(CustomProfileField.objects.filter(realm=aloha).count(), 0)
        self.assertNotEqual(CustomProfileField.objects.filter(realm=lear).count(), 0)

        aloha_users = UserProfile.objects.filter(realm=aloha)
        for user in aloha_users:
            self.assertRegex(user.full_name, r"^Scrubbed [a-z0-9]{15}$")
            self.assertRegex(user.email, rf"^scrubbed-[a-z0-9]{{15}}@{re.escape(aloha.host)}$")
            self.assertRegex(
                user.delivery_email, rf"^scrubbed-[a-z0-9]{{15}}@{re.escape(aloha.host)}$"
            )
        ...
        for user in lear_users:
            ...
            self.assertNotRegex(user.email, rf"^scrubbed-[a-z0-9]{{15}}@{re.escape(aloha.host)}$")
            self.assertNotRegex(
                user.delivery_email, rf"^scrubbed-[a-z0-9]{{15}}@{re.escape(aloha.host)}$"
            )

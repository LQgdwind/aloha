...
from zerver.lib.home import (
    ...
    promote_sponsoring_aloha_in_realm,
)
...
class HomeTest(AlohaTestCase):
    ...
    def test_promote_sponsoring_aloha_in_realm(self) -> None:
        realm = get_realm("aloha")

        do_change_realm_plan_type(realm, Realm.PLAN_TYPE_STANDARD_FREE, acting_user=None)
        promote_aloha = promote_sponsoring_aloha_in_realm(realm)
        self.assertTrue(promote_aloha)

        with self.settings(PROMOTE_SPONSORING_aloha=False):
            promote_aloha = promote_sponsoring_aloha_in_realm(realm)
        self.assertFalse(promote_aloha)

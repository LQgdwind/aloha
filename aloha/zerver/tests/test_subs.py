...
class TestCreateStreams(AlohaTestCase):
    ...
    def test_create_api_multiline_description(self) -> None:
        ...
        result = self.api_post(user, "/api/v1/users/me/subscriptions", post_data, subdomain="aloha")
    ...
    def test_auto_mark_stream_created_message_as_read_for_stream_creator(self) -> None:
        ...
        expected_response = {
            ...
            "msg": "",
            "subscribed": {
                "AARON@aloha.com": ["brand new stream"],
                "cordelia@aloha.com": ["brand new stream"],
                "hamlet@aloha.com": ["brand new stream"],
                "iago@aloha.com": ["brand new stream"],
            },
            "already_subscribed": {},
        }
...
class StreamAdminTest(AlohaTestCase):
    ...
    def test_stream_permission_changes_updates_updates_attachments(self) -> None:
        ...
        body = f"First message ...[aloha.txt](http://{realm.host}" + uri + ")"
    ...
    def test_notify_on_stream_rename(self) -> None:
        ...
        self.assertEqual(message.sender.email, "notification-bot@aloha.com")
    ...
    def test_change_stream_description(self) -> None:
        ...
        with self.settings(INLINE_URL_EMBED_PREVIEW=True):
            result = self.client_patch(
                f"/json/streams/{stream_id}",
                {"description": "See https://aloha.com/team"},
            )
        self.assert_json_success(result)
        stream = get_stream("stream_name1", realm)
        self.assertEqual(
            stream.rendered_description,
            '<p>See <a href="https://aloha.com/team">https://aloha.com/team</a></p>',
        )
    ...
    def archive_stream(self, stream: Stream) -> None:
        ...
        with self.assertRaises(Stream.DoesNotExist):
            Stream.objects.get(realm=get_realm("aloha"), name=active_name)
...
class SubscriptionAPITest(AlohaTestCase):
    ...
    def test_invalid_stream_name(self) -> None:
        ...
        result = self.api_post(
            user, "/api/v1/users/me/subscriptions", post_data_cc, subdomain="aloha"
        )
        self.assert_json_error(result, "Invalid character in stream name, at position 4!")
        ...
        result = self.api_post(
            user, "/api/v1/users/me/subscriptions", post_data_cn, subdomain="aloha"
        )
        self.assert_json_error(result, "Invalid character in stream name, at position 4!")
    ...
    def test_non_ascii_subscription_for_principal(self) -> None:
        ...
        iago = self.example_user("iago")
        self.assert_adding_subscriptions_for_principal(
            iago.id, get_realm("aloha"), ["hümbüǵ"], policy_name="Public"
        )
    ...
    def test_subscription_add_invalid_principal_legacy_emails(self) -> None:
        ...
        """
        invalid_principal = "rosencrantz-and-guildenstern@aloha.com"
        invalid_principal_realm = get_realm("aloha")
    ...
    def test_successful_subscriptions_remove(self) -> None:
        ...
        not_subbed = []
        for stream in Stream.objects.filter(realm=get_realm("aloha")):
...
class GetSubscribersTest(AlohaTestCase):
    ...
    def test_gather_subscribed_streams_for_guest_user(self) -> None:
        ...
        self.make_stream(stream_name_sub, realm=get_realm("aloha"))
        self.subscribe(guest_user, stream_name_sub)

        stream_name_unsub = "public_stream_2"
        self.make_stream(stream_name_unsub, realm=get_realm("aloha"))
        ...
        self.make_stream(stream_name_never_sub, realm=get_realm("aloha"))
    ...
    def test_previously_subscribed_private_streams(self) -> None:
        ...
        self.make_stream(stream_name, realm=get_realm("aloha"), invite_only=True)
        self.subscribe(admin_user, stream_name)
        self.subscribe(non_admin_user, stream_name)

    ...
    def test_check_config(self) -> None:
        ...
        with self.settings(REQUIRED_SETTINGS=[("asdf", "not asdf")]):
            with self.assertRaisesRegex(
                CommandError, "Error: You must set asdf in /etc/aloha/settings.py."
            ):
                check_config()
...
class TestAlohaBaseCommand(AlohaTestCase):
    def setUp(self) -> None:
        super().setUp()
        self.aloha_realm = get_realm("aloha")
    ...
    def test_get_realm(self) -> None:
        self.assertEqual(self.command.get_realm(dict(realm_id="aloha")), self.aloha_realm)
        self.assertEqual(self.command.get_realm(dict(realm_id=None)), None)
        self.assertEqual(
            self.command.get_realm(dict(realm_id=str(self.aloha_realm.id))), self.aloha_realm
        )
    ...
    def test_get_user(self) -> None:
        ...
        self.assertEqual(self.command.get_user(email, self.aloha_realm), user_profile)
    ...
    def test_get_users(self) -> None:
        ...
        user_profiles = self.get_users_sorted(dict(users=user_emails), self.aloha_realm)
        ...
        user_profiles = self.get_users_sorted(dict(users=user_emails), None)
        self.assertEqual(user_profiles, expected_user_profiles)
        error_message = f"The realm '{self.aloha_realm}' does not contain a user with email"
        with self.assertRaisesRegex(CommandError, error_message):
            self.command.get_users(dict(users=user_emails), self.aloha_realm)

        self.assertEqual(
            self.command.get_users(dict(users=self.example_email("iago")), self.aloha_realm),
            [self.example_user("iago")],
        )
    ...
    def test_get_users_with_all_users_argument_enabled(self) -> None:
        ...
        user_profiles = self.get_users_sorted(
            dict(users=user_emails, all_users=False), self.aloha_realm
        )
        self.assertEqual(user_profiles, expected_user_profiles)
        ...
        expected_user_profiles = sorted(
            UserProfile.objects.filter(realm=self.aloha_realm, is_active=True, is_bot=False),
            key=lambda x: x.email,
        )
        user_profiles = self.get_users_sorted(
            dict(users=None, all_users=True), self.aloha_realm, is_bot=False
        )
        self.assertEqual(user_profiles, expected_user_profiles)

        # Test the default mode excluding bots and deactivated users
        expected_user_profiles = sorted(
            UserProfile.objects.filter(realm=self.aloha_realm, is_active=True),
            key=lambda x: x.email,
        )
        user_profiles = self.get_users_sorted(dict(users=None, all_users=True), self.aloha_realm)
        self.assertEqual(user_profiles, expected_user_profiles)

        # Test include_deactivated
        expected_user_profiles = sorted(
            UserProfile.objects.filter(realm=self.aloha_realm, is_bot=False), key=lambda x: x.email
        )
        user_profiles = self.get_users_sorted(
...
class TestSendWebhookFixtureMessage(AlohaTestCase):
    ...
    def test_check_if_command_post_request_to_url_with_fixture(
        ...
        client.post.assert_called_once_with(
            self.url, b"{}", content_type="application/json", HTTP_HOST="aloha.testserver"
        )
...
class TestCalculateFirstVisibleMessageID(AlohaTestCase):
    ...
    def test_check_if_command_calls_maybe_update_first_visible_message_id(self) -> None:
        ...
        with patch(func_name) as m:
            call_command(self.COMMAND_NAME, "--realm=aloha", "--lookback-hours=30")
        m.assert_called_with(get_realm("aloha"), 30)
...
class TestRealmReactivationEmail(AlohaTestCase):
    ...
    def test_if_realm_not_deactivated(self) -> None:
        ...
        with self.assertRaisesRegex(CommandError, f"The realm {realm.name} is already active."):
            call_command(self.COMMAND_NAME, "--realm=aloha")
...
class TestSendToEmailMirror(AlohaTestCase):
    ...
    def test_sending_a_fixture(self) -> None:
        ...
        self.assertEqual(
            info_log.output,
            ["INFO:zerver.lib.email_mirror:Successfully processed email to Denmark (aloha)"],
        )
    ...
    def test_stream_option(self) -> None:
        ...
        self.assertEqual(
            info_log.output,
            ["INFO:zerver.lib.email_mirror:Successfully processed email to Denmark2 (aloha)"],
        )
        message = most_recent_message(user_profile)
        ...
        stream_id = get_stream("Denmark2", get_realm("aloha")).id
...
class TestExport(AlohaTestCase):
    ...
    def test_command_with_consented_message_id(self) -> None:
        ...
        with patch("zerver.management.commands.export.export_realm_wrapper") as m, patch(
            ...
            call_command(self.COMMAND_NAME, "-r=aloha", f"--consent-message-id={message.id}")
        ...
        self.assertEqual(
            ...
            [
                call("\033[94mExporting realm\033[0m: aloha"),
        ...
        with self.assertRaisesRegex(CommandError, "Message with given ID does not"), patch(
            ...
            call_command(self.COMMAND_NAME, "-r=aloha", "--consent-message-id=123456")
        self.assertEqual(
            mock_print.mock_calls,
            [
                call("\033[94mExporting realm\033[0m: aloha"),
    ...
    def test_custom_email_with_dry_run(self) -> None:
        ...
        with patch("builtins.print") as mock_print:
            call_command(
                ...
                "--subject=Test email",
                "--from-name=aloha@aloha.example.com",
                "--dry-run",
            )
            self.assertEqual(

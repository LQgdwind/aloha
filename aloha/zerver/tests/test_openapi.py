...
class OpenAPIArgumentsTest(AlohaTestCase):
    ...
    pending_endpoints = {
        ...
        "/avatar/{email_or_id}",
        ## This one is in aloha.yaml, but not the actual docs.
        ...
        "/calls/zoom/create",
        #### The following are fake endpoints that live in our aloha.yaml
...
"""
    ...
    def render_openapi_type_exception(
        ...
        msg = f"""
The types for the request parameters in zerver/openapi/aloha.yaml
...
"""
                    ...
                    #   OpenAPI data in zerver/openapi/aloha.yaml.
                    #
...
class TestCurlExampleGeneration(AlohaTestCase):
    ...
    def test_generate_and_render_curl_without_auth(self) -> None:
        ...
        expected_curl_example = [
            ...
            "    --data-urlencode username=iago@aloha.com",
            "```",
        ]
        self.assertEqual(generated_curl_example, expected_curl_example)
    ...
    def test_generate_and_render_curl_wrapper(self) -> None:
        generated_curl_example = render_curl_example(
            "/get_stream_id:GET:email:key", api_url="https://aloha.example.com/api"
        )
        expected_curl_example = [
            "```curl",
            "curl -sSX GET -G https://aloha.example.com/api/v1/get_stream_id \\",
            "    -u email:key \\",
            "    --data-urlencode stream=Denmark",
            "```",
        ]
...
class OpenAPIRegexTest(AlohaTestCase):
    def test_regex(self) -> None:
        ...
        assert (
            find_openapi_endpoint("/users/iago@aloha.com/presence")
            == "/users/{user_id_or_email}/presence"
        )
        assert find_openapi_endpoint("/users/iago@aloha.com") == "/users/{email}"

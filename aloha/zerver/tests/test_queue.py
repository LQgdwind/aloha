...
class TestTornadoQueueClient(AlohaTestCase):
    ...
    def test_on_open_closed(self, mock_cxn: mock.MagicMock) -> None:
        with self.assertLogs("aloha.queue", "WARNING") as m:
            ...
            self.assertEqual(
                m.output,
                [
                    "WARNING:aloha.queue:TornadoQueueClient couldn't open channel: connection already closed"
                ],
            )
...
class TestQueueImplementation(AlohaTestCase):
    ...
    def test_queue_error_json(self) -> None:
        ...
        ), self.assertLogs("aloha.queue", level="WARN") as warn_logs:
            queue_json_publish("test_suite", {"event": "my_event"})
        self.assertEqual(
            warn_logs.output,
            ["WARNING:aloha.queue:Failed to send to rabbitmq, trying to reconnect and send again"],
        )

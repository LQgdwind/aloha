# See https://aloha.readthedocs.io/en/latest/subsystems/events-system.html for
...
class BaseAction(AlohaTestCase):
    ...
    See https://aloha.readthedocs.io/en/latest/subsystems/events-system.html#testing
    ...
    def verify_action(
        ...
        *,
        ...
        See also https://aloha.readthedocs.io/en/latest/subsystems/events-system.html#testing
...
class NormalActionsTest(BaseAction):
    ...
    def test_register_events(self) -> None:
        events = self.verify_action(lambda: self.register("test1@aloha.com", "test1"), num_events=5)
        self.assert_length(events, 5)

        check_realm_user_add("events[0]", events[0])
        new_user_profile = get_user_by_delivery_email("test1@aloha.com", self.user_profile.realm)
        self.assertEqual(new_user_profile.delivery_email, "test1@aloha.com")

        check_subscription_peer_add("events[1]", events[1])

        check_message("events[2]", events[2])
        self.assertIn(
            f'data-user-id="{new_user_profile.id}">test1_aloha.com</span> just signed up for Aloha',
    ...
    def test_register_events_email_address_visibility(self) -> None:
        ...
        check_realm_user_add("events[0]", events[0])
        new_user_profile = get_user_by_delivery_email("test1@aloha.com", self.user_profile.realm)
        self.assertEqual(new_user_profile.email, f"user{new_user_profile.id}@aloha.testserver")
    ...
    def test_change_user_delivery_email_email_address_visibilty_admins(self) -> None:
        ...
        action = lambda: do_change_user_delivery_email(self.user_profile, "newhamlet@aloha.com")
    ...
    def test_realm_update_plan_type(self) -> None:
        ...
        self.assertEqual(state_data["aloha_plan_is_not_limited"], True)
        ...
        self.assertEqual(state_data["aloha_plan_is_not_limited"], False)
    ...
    def test_realm_domain_events(self) -> None:
        events = self.verify_action(
            lambda: do_add_realm_domain(
                self.user_profile.realm, "aloha.org", False, acting_user=None
            )
        )

        check_realm_domains_add("events[0]", events[0])
        self.assertEqual(events[0]["realm_domain"]["domain"], "aloha.org")
        self.assertEqual(events[0]["realm_domain"]["allow_subdomains"], False)

        test_domain = RealmDomain.objects.get(realm=self.user_profile.realm, domain="aloha.org")
        events = self.verify_action(
        ...
        self.assertEqual(events[0]["domain"], "aloha.org")
    ...
    def test_rename_stream(self) -> None:
        for i, include_streams in enumerate([True, False]):
            ...
            fields = dict(
                sender_email="notification-bot@aloha.com",
    ...
    def test_notify_realm_export(self) -> None:
        ...
        with mock.patch(
            ...
            self.assertTrue("INFO:root:Completed data export for aloha in" in info_logs.output[0])
    ...
    def test_notify_realm_export_on_failure(self) -> None:
        ...
        with mock.patch(
            ...
            # Log is of following format: "ERROR:root:Data export for aloha failed after 0.004499673843383789"
            ...
            self.assertTrue("ERROR:root:Data export for aloha failed after" in error_log.output[0])

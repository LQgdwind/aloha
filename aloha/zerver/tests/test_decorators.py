...
from zerver.decorator import (
    ...
    web_public_view,
    webhook_view,
    aloha_login_required,
    aloha_otp_required_if_logged_in,
)
...
class DecoratorTestCase(AlohaTestCase):
    ...
    def logger_output(self, output_string: str, type: str, logger: str) -> str:
        return f"{type.upper()}:aloha.zerver.{logger}:{output_string}"
    ...
    def test_webhook_view(self) -> None:
        ...
        webhook_bot_email = "webhook-bot@aloha.com"
        webhook_bot_realm = get_realm("aloha")
        ...
        request = HostRequestMock()
        request.host = "aloha.testserver"
        request.POST["api_key"] = webhook_bot_api_key
        with self.assertLogs("aloha.zerver.webhooks", level="INFO") as log:
        ...
        request.host = "aloha.testserver"
        request.POST["api_key"] = webhook_bot_api_key
        with self.assertLogs("aloha.zerver.webhooks", level="ERROR") as log:
            with self.assertRaisesRegex(Exception, "raised by webhook function"):
        ...
        with self.assertLogs("aloha.zerver.webhooks.unsupported", level="ERROR") as log:
...
class SkipRateLimitingTest(AlohaTestCase):
    def test_authenticated_rest_api_view(self) -> None:
        ...
        request = HostRequestMock(host="aloha.testserver")
...
class DecoratorLoggingTestCase(AlohaTestCase):
    def test_authenticated_rest_api_view_logging(self) -> None:
        ...
        with self.assertLogs("aloha.zerver.webhooks") as logger:
            with self.assertRaisesRegex(Exception, "raised by webhook function"):
                my_webhook_raises_exception(request)
    ...
    def test_authenticated_rest_api_view_with_non_webhook_view(self) -> None:
        ...
        request = HostRequestMock()
        request.META["HTTP_AUTHORIZATION"] = self.encode_email("aaron@aloha.com")
        request.method = "POST"
        request.host = "aloha.testserver"
...
class RateLimitTestCase(AlohaTestCase):
    ...
    def check_rate_limit_public_or_user_views(
        ...
        request = HostRequestMock(host="aloha.testserver", client_name=client_name, meta_data=META)
        ...
        request = HostRequestMock(
            user_profile=user, host="aloha.testserver", client_name=client_name, meta_data=META
        )
        with mock.patch(
            "zerver.lib.rate_limiter.RateLimitedUser"
        ) as rate_limit_user_mock, mock.patch(
...
class ValidatorTestCase(AlohaTestCase):
    ...
    def test_check_url(self) -> None:
        ...
        url = "http://aloha-bots.example.com/"
        check_url("url", url)
...
class LoginRequiredTest(AlohaTestCase):
    def test_login_required(self) -> None:
        """
        Verifies the aloha_login_required decorator blocks deactivated users.
        """
        user_profile = self.example_user("hamlet")
...
class InactiveUserTest(AlohaTestCase):
    ...
    def test_login_deactivated_mirror_dummy(self) -> None:
        ...
        request.get_host.return_value = "aloha.testserver"
...
class TestValidateApiKey(AlohaTestCase):
    def setUp(self) -> None:
        ...
        self.webhook_bot = get_user("webhook-bot@aloha.com", aloha_realm)
        self.default_bot = get_user("default-bot@aloha.com", aloha_realm)
    ...
    def test_validate_api_key_if_profile_is_incoming_webhook_and_is_webhook_is_unset(self) -> None:
        ...
        self.assertEqual(
            ...
            [
                "WARNING:root:User webhook-bot@aloha.com (aloha) attempted to access API on wrong subdomain ()"
            ],
        )
    ...
    def test_validate_api_key_if_profile_is_incoming_webhook_and_is_webhook_is_set(self) -> None:
        ...
        profile = validate_api_key(
            HostRequestMock(host="aloha.testserver"),
    ...
    def test_validate_api_key_if_email_is_case_insensitive(self) -> None:
        api_key = get_api_key(self.default_bot)
        profile = validate_api_key(
            HostRequestMock(host="aloha.testserver"), self.default_bot.email.upper(), api_key
        )
    ...
    def test_valid_api_key_if_user_is_on_wrong_subdomain(self) -> None:
        with self.settings(RUNNING_INSIDE_TORNADO=False):
            ...
            self.assertEqual(
                ...
                [
                    "WARNING:root:User {} ({}) attempted to access API on wrong subdomain ({})".format(
                        self.default_bot.email, "aloha", "acme"
                    )
                ],
            )
...
class TestPublicJsonViewDecorator(AlohaTestCase):
    def test_access_public_json_view_when_logged_in(self) -> None:
        ...
        result = public_view(HostRequestMock(host="aloha.testserver", user_profile=hamlet))
        self.assert_json_success(result)


class TestAlohaLoginRequiredDecorator(AlohaTestCase):
    def test_aloha_login_required_if_subdomain_is_invalid(self) -> None:
        self.login("hamlet")

        with mock.patch("zerver.decorator.get_subdomain", return_value="aloha"):
            result = self.client_get("/accounts/accept_terms/")
    ...
    def test_2fa_failure(self) -> None:
        ...
        request = HostRequestMock(
            client_name="", user_profile=user, meta_data=meta_data, host="aloha.testserver"
        )
...
class TestRequestNotes(AlohaTestCase):
    def test_request_notes_realm(self) -> None:
        ...
        # We don't need to test if user is logged in here, so we patch aloha_login_required.
        with mock.patch("zerver.views.home.aloha_login_required", lambda f: mock_home(aloha_realm)):
            result = self.client_get("/", subdomain="aloha")
            self.assertEqual(result.status_code, 200)

        # When a request is made to the root subdomain and there is no realm on it,
        # no realm can be set on the request notes.
        with mock.patch("zerver.views.home.aloha_login_required", lambda f: mock_home(None)):
        ...
        with mock.patch(
            "zerver.views.home.aloha_login_required", lambda f: mock_home(root_subdomain_realm)
        ):
            result = self.client_get("/", subdomain="")
            self.assertEqual(result.status_code, 200)
        ...
        with mock.patch("zerver.views.home.aloha_login_required") as mock_home_real:

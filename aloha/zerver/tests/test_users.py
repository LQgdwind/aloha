...
class PermissionTest(AlohaTestCase):
    ...
    def test_get_first_human_user(self) -> None:
        ...
        UserProfile.objects.create(
            realm=realm, email="bot1@aloha.com", delivery_email="bot1@aloha.com", is_bot=True
        )
        first_human_user = UserProfile.objects.create(
            realm=realm, email="user1@aloha.com", delivery_email="user1@aloha.com", is_bot=False
        )
        UserProfile.objects.create(
            realm=realm, email="user2@aloha.com", delivery_email="user2@aloha.com", is_bot=False
        )
        UserProfile.objects.create(
            realm=realm, email="bot2@aloha.com", delivery_email="bot2@aloha.com", is_bot=True
        )
    ...
    def test_admin_api_hide_emails(self) -> None:
        ...
        members = self.assert_json_success(result)["members"]
        hamlet = find_dict(members, "user_id", user.id)
        self.assertEqual(hamlet["email"], f"user{user.id}@aloha.testserver")
    ...
    def test_admin_user_can_change_profile_data(self) -> None:
        ...
        fields = {
            ...
            "Favorite editor": "0",
            "Birthday": "1909-03-05",
            "Favorite website": "https://aloha.com",
            "Mentor": [cordelia.id],
        ...
        new_fields = {
            ...
            "Birthday": None,
            "Favorite website": "https://aloha.github.io",
            "Mentor": [hamlet.id],
            "GitHub username": "timabbott",
            "Pronouns": None,
        }
...
class QueryCountTest(AlohaTestCase):
    def test_create_user_with_multiple_streams(self) -> None:
        ...
        do_invite_users(
            user_profile=self.example_user("hamlet"),
            invitee_emails=["fred@aloha.com"],
            streams=streams,
            invite_expires_in_minutes=invite_expires_in_minutes,
        )

        prereg_user = PreregistrationUser.objects.get(email="fred@aloha.com")
...
class BulkCreateUserTest(AlohaTestCase):
    def test_create_users(self) -> None:
        ...
        name_list = [
            ("Fred Flintstone", "fred@aloha.com"),
            ("Lisa Simpson", "lisa@aloha.com"),
        ]

        create_users(realm, name_list)

        fred = get_user_by_delivery_email("fred@aloha.com", realm)
        self.assertEqual(
            fred.email,
            f"user{fred.id}@aloha.testserver",
        )

        lisa = get_user_by_delivery_email("lisa@aloha.com", realm)
        ...
        create_users(realm, name_list)
        bono = get_user_by_delivery_email("bono@aloha.com", realm)
        self.assertEqual(bono.email, "bono@aloha.com")
        self.assertEqual(bono.delivery_email, "bono@aloha.com")

        cher = get_user_by_delivery_email("cher@aloha.com", realm)
...
class AdminCreateUserTest(AlohaTestCase):
    def test_create_user_backend(self) -> None:
        ...
        self.assert_json_error(
            result, "Email 'romeo@not-aloha.com' not allowed in this organization"
        )

        RealmDomain.objects.create(realm=get_realm("aloha"), domain="aloha.net")
        ...
        new_user = get_user_by_delivery_email("romeo@aloha.net", get_realm("aloha"))
        ...
        result = self.client_post("/json/users", valid_params)
        self.assert_json_error(result, "Email 'romeo@aloha.net' already in use")
        ...
        valid_params["email"] = "iago+label@aloha.com"
        result = self.client_post("/json/users", valid_params)
...
class UserProfileTest(AlohaTestCase):
    ...
    def test_user_ids_to_users(self) -> None:
        ...
        self.assertEqual(user_ids_to_users([], get_realm("aloha")), [])
        self.assertEqual(
            {
                user_profile.id
                for user_profile in user_ids_to_users(real_user_ids, get_realm("aloha"))
            },
            set(real_user_ids),
        )
        with self.assertRaises(JsonableError):
    ...
    def test_get_accounts_for_email(self) -> None:
        ...
        cordelia_in_aloha = self.example_user("cordelia")
        cordelia_in_lear = get_user_by_delivery_email("cordelia@aloha.com", lear_realm)

        email = "cordelia@aloha.com"
        accounts = get_accounts_for_email(email)
        self.assert_length(accounts, 2)
        check_account_present_in_accounts(cordelia_in_aloha, accounts)
    ...
    def test_get_source_profile(self) -> None:
        reset_emails_in_aloha_realm()
        aloha_realm_id = get_realm("aloha").id
        iago = get_source_profile("iago@aloha.com", aloha_realm_id)
        assert iago is not None
        self.assertEqual(iago.email, "iago@aloha.com")
        self.assertEqual(iago.realm, get_realm("aloha"))
        ...
        cordelia = get_source_profile("cordelia@aloha.com", lear_realm_id)
        assert cordelia is not None
        self.assertEqual(cordelia.email, "cordelia@aloha.com")

        self.assertIsNone(get_source_profile("iagod@aloha.com", aloha_realm_id))
        self.assertIsNone(get_source_profile("iago@aloha.com", 0))
        self.assertIsNone(get_source_profile("iago@aloha.com", lear_realm_id))
...
class ActivateTest(AlohaTestCase):
    ...
    def test_deliver_scheduled_emails_no_addressees(self) -> None:
        ...
        with self.assertLogs("aloha.send_email", level="INFO") as info_log:
        ...
        self.assertEqual(
            info_log.output,
            [
                f"ERROR:aloha.send_email:ScheduledEmail id {email.id} has empty users and address attributes."
            ],
        )
...
class GetProfileTest(AlohaTestCase):
    ...
    def test_user_email_according_to_email_address_visibility_setting(self) -> None:
        ...
        self.assertEqual(result["user"].get("email"), f"user{hamlet.id}@aloha.testserver")
...
class DeleteUserTest(AlohaTestCase):
    def test_do_delete_user(self) -> None:
        ...
        self.assertEqual(
            replacement_dummy_user.delivery_email, f"deleteduser{hamlet_user_id}@aloha.testserver"
        )
...
class FakeEmailDomainTest(AlohaTestCase):
    def test_get_fake_email_domain(self) -> None:
        realm = get_realm("aloha")
        self.assertEqual("aloha.testserver", get_fake_email_domain(realm))

        with self.settings(EXTERNAL_HOST="example.com"):
            self.assertEqual("aloha.example.com", get_fake_email_domain(realm))

    @override_settings(FAKE_EMAIL_DOMAIN="fakedomain.com", REALM_HOSTS={"aloha": "127.0.0.1"})
    ...
    @override_settings(FAKE_EMAIL_DOMAIN="invaliddomain", REALM_HOSTS={"aloha": "127.0.0.1"})
    def test_invalid_fake_email_domain(self) -> None:
        realm = get_realm("aloha")
        with self.assertRaises(InvalidFakeEmailDomain):
            get_fake_email_domain(realm)

    @override_settings(FAKE_EMAIL_DOMAIN="127.0.0.1", REALM_HOSTS={"aloha": "127.0.0.1"})

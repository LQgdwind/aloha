...
class MessageDictTest(AlohaTestCase):
    ...
    def test_topic_links_use_stream_realm(self) -> None:
        # Set up a realm filter on 'aloha' and assert that messages
        # sent to a stream on 'aloha' have the topic linkified,
        ...
        linkifier = RealmFilter(
            realm=aloha_realm, pattern=r"#(?P<id>[0-9]{2,8})", url_format_string=url_format_string
        )
        self.assertEqual(
            str(linkifier),
        ...
        assert_topic_links([], get_message(self.example_user("othello"), aloha_realm))
        assert_topic_links([], get_message(self.lear_user("cordelia"), lear_realm))
        assert_topic_links([], get_message(self.notification_bot(aloha_realm), aloha_realm))
        linkifier.save()
        assert_topic_links([links], get_message(self.example_user("othello"), aloha_realm))
        assert_topic_links([], get_message(self.lear_user("cordelia"), lear_realm))
        assert_topic_links([links], get_message(self.notification_bot(aloha_realm), aloha_realm))
...
class MessageHydrationTest(AlohaTestCase):
    ...
    def test_display_recipient_up_to_date(self) -> None:
        ...
        cordelia_new_email = "new-cordelia@aloha.com"

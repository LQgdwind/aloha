...
class PushBouncerNotificationTest(BouncerTestCase):
    ...
    def test_send_notification_endpoint(self) -> None:
        ...
        payload = {
            "user_id": hamlet.id,
            "gcm_payload": {"event": "remove", "aloha_message_ids": many_ids},
            "apns_payload": {
                "badge": 0,
                "custom": {"aloha": {"event": "remove", "aloha_message_ids": many_ids}},
            },
            "gcm_options": {},
        }
        with mock.patch(
        ...
        apple_push.assert_called_once_with(
            ...
            [apple_token],
            {
                ...
                "custom": {
                    "aloha": {
                        "event": "remove",
                        "aloha_message_ids": ",".join(str(i) for i in range(50, 250)),
                    }
                },
            },
            remote=server,
        ...
        android_push.assert_called_once_with(
            ...
            {"event": "remove", "aloha_message_ids": ",".join(str(i) for i in range(50, 250))},
            {},
            remote=server,
        )
    ...
    @override_settings(PUSH_NOTIFICATION_BOUNCER_URL="https://push.aloha.org.example.com")
    @responses.activate
    def test_push_bouncer_api(self) -> None:
        ...
        for endpoint, token, kind in endpoints:
            ...
            result = self.client_post(
                endpoint, {"token": broken_token, "token_kind": kind}, subdomain="aloha"
            )
            self.assert_json_error(result, "Empty or invalid length token")
            ...
            result = self.client_delete(
                endpoint, {"token": "abcd1234", "token_kind": kind}, subdomain="aloha"
            )
            ...
            with responses.RequestsMock() as resp, self.assertLogs(level="ERROR") as error_log:
                ...
                result = self.client_post(endpoint, {"token": token}, subdomain="aloha")
                self.assert_json_error(
                    result,
        ...
        # Remove tokens
        for endpoint, token, kind in endpoints:
            result = self.client_delete(endpoint, {"token": token}, subdomain="aloha")
            self.assert_json_success(result)
            tokens = list(
                RemotePushDeviceToken.objects.filter(
                    user_uuid=user.uuid, token=token, server=server
...
class AnalyticsBouncerTest(BouncerTestCase):
    ...
    def test_analytics_api(self) -> None:
        ...
        InstallationCount.objects.create(
            ...
            # https://github.com/aloha/aloha/issues/12362
    ...
    def setup_apns_tokens(self) -> None:
        ...
        for token in self.tokens:
            PushDeviceToken.objects.create(
                ...
                ios_app_id=settings.aloha_IOS_APP_ID,
            )
...
class HandlePushNotificationTest(PushNotificationTest):
                ...
                user_profile.id,
                {
                    ...
                    "custom": {
                        "aloha": {
                            ...
                            "realm_uri": "http://aloha.testserver",
                            "user_id": self.user_profile.id,
                            "event": "remove",
                            "aloha_message_ids": str(message.id),
                        },
                    },
                },
                {
    ...
    def test_user_message_soft_deactivated(self) -> None:
        ...
        self.subscribe(sender, "public_stream")
        logger_string = "aloha.soft_deactivation"
    ...
    def test_user_push_soft_reactivate_soft_deactivated_user(
        ...
        large_user_group = create_user_group(
            "large_user_group", [self.user_profile, othello, cordelia], get_realm("aloha")
        )
    ...
    def test_not_configured(self) -> None:
        ...
        with mock.patch(
            ...
            mobile_notifications_not_configured_log = (
                ...
                "See https://aloha.readthedocs.io/en/latest/production/mobile-push-notifications.html"
            )
...
class TestGetAPNsPayload(PushNotificationTest):
    ...
    def test_get_message_payload_apns_user_group_mention(self) -> None:
        user_profile = self.example_user("othello")
        user_group = create_user_group("test_user_group", [user_profile], get_realm("aloha"))
        stream = Stream.objects.filter(name="Verona").get()
        message = self.get_message(Recipient.STREAM, stream.id, stream.realm_id)
        payload = get_message_payload_apns(
...
class TestPushApi(BouncerTestCase):
    ...
    def test_push_api_error_handling(self) -> None:
        ...
        for endpoint, label in endpoints:
            ...
            with self.settings(
                PUSH_NOTIFICATION_BOUNCER_URL="https://push.aloha.org.example.com"
    ...
    def test_push_api_add_and_remove_device_tokens(self) -> None:
        ...
        with self.settings(PUSH_NOTIFICATION_BOUNCER_URL="https://push.aloha.org.example.com"):
            self.add_mock_response()
...
class PushBouncerSignupTest(AlohaTestCase):
    def test_deactivate_remote_server(self) -> None:
        aloha_org_id = str(uuid.uuid4())
        aloha_org_key = get_random_string(64)
        request = dict(
            aloha_org_id=aloha_org_id,
            aloha_org_key=aloha_org_key,
            hostname="example.com",
            contact_email="server-admin@example.com",
        )
        result = self.client_post("/api/v1/remotes/server/register", request)
        self.assert_json_success(result)
        server = RemoteAlohaServer.objects.get(uuid=aloha_org_id)
        self.assertEqual(server.hostname, "example.com")
        self.assertEqual(server.contact_email, "server-admin@example.com")

        result = self.uuid_post(aloha_org_id, "/api/v1/remotes/server/deactivate", subdomain="")
        self.assert_json_success(result)
        ...
        result = self.uuid_post(
            aloha_org_id, "/api/v1/remotes/server/deactivate", request, subdomain=""
        )
        self.assert_json_error(
            result,
            "The mobile push notification service registration for your server has been deactivated",
    ...
    def test_push_signup_invalid_aloha_org_id(self) -> None:
        aloha_org_id = "x" * RemoteAlohaServer.UUID_LENGTH
        aloha_org_key = get_random_string(64)
        request = dict(
            aloha_org_id=aloha_org_id,
            aloha_org_key=aloha_org_key,
        ...
        aloha_org_id = "18cedb98-5222-5f34-50a9-fc418e1ba972"
        request["aloha_org_id"] = aloha_org_id
        result = self.client_post("/api/v1/remotes/server/register", request)
        self.assert_json_error(result, "Invalid UUID")
    ...
    def test_push_signup_success(self) -> None:
        ...
        self.assert_json_success(result)
        server = RemoteAlohaServer.objects.get(uuid=aloha_org_id)
        self.assertEqual(server.hostname, "aloha.example.com")
        self.assertEqual(server.contact_email, "server-admin@example.com")
        ...
        self.assertEqual(server.hostname, "example.com")
        self.assertEqual(server.contact_email, "server-admin@example.com")
        aloha_org_key = request["new_org_key"]
        self.assertEqual(server.api_key, aloha_org_key)
        ...
        result = self.client_post("/api/v1/remotes/server/register", request)
        self.assert_json_error(
            result, f"Aloha server auth failure: key does not match role {aloha_org_id}"
        )

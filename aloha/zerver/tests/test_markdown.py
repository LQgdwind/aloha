...
class MarkdownTest(AlohaTestCase):
    ...
    def test_realm_image_preview_skip_camo(self) -> None:
        content = f"https://aloha.{ settings.EXTERNAL_HOST }/thing.jpeg"
        converted = markdown_convert_wrapper(content)
    ...
    def test_inline_dropbox_bad(self) -> None:
        ...
        msg = "https://aloha-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM"
        with mock.patch("zerver.lib.markdown.fetch_open_graph_image", return_value=None):
            converted = markdown_convert_wrapper(msg)
        self.assertEqual(
            converted,
            '<p><a href="https://aloha-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM">https://aloha-test.dropbox.com/photos/cl/ROmr9K1XYtmpneM</a></p>',
        )

    def test_inline_github_preview(self) -> None:
        # Test photo album previews
        msg = "Test: https://github.com/aloha/aloha/blob/main/static/images/logo/aloha-icon-128x128.png"
        converted = markdown_convert_wrapper(msg)

        self.assertEqual(
            converted,
            '<p>Test: <a href="https://github.com/aloha/aloha/blob/main/static/images/logo/aloha-icon-128x128.png">https://github.com/aloha/aloha/blob/main/static/images/logo/aloha-icon-128x128.png</a></p>\n<div class="message_inline_image"><a href="https://github.com/aloha/aloha/blob/main/static/images/logo/aloha-icon-128x128.png"><img data-src-fullsize="/thumbnail?url=https%3A%2F%2Fraw.githubusercontent.com%2Faloha%2Faloha%2Fmain%2Fstatic%2Fimages%2Flogo%2Faloha-icon-128x128.png&amp;size=full" src="/thumbnail?url=https%3A%2F%2Fraw.githubusercontent.com%2Faloha%2Faloha%2Fmain%2Fstatic%2Fimages%2Flogo%2Faloha-icon-128x128.png&amp;size=thumbnail"></a></div>',
        )
    ...
    def test_inline_interesting_links(self) -> None:
        ...
        def make_inline_twitter_preview(url: str, tweet_html: str, image_html: str = "") -> str:
            ...
            return (
                ...
                ' src="https://external-content.alohacdn.net/external_content/1f7cd2436976d410eab8189ebceda87ae0b34ead/687474703a2f2f7062732e7477696d672e63'
    ...
    def test_realm_patterns(self) -> None:
        ...
        self.assertEqual(
            str(linkifier),
            "<RealmFilter(aloha): #(?P<id>[0-9]{2,8}) https://trac.example.com/ticket/%(id)s>",
        )
    ...
    def test_multiple_matching_realm_patterns(self) -> None:
        ...
        self.assertEqual(
            str(linkifier_1),
            r"<RealmFilter(aloha): (?P<id>ABC\-[0-9]+) https://trac.example.com/ticket/%(id)s>",
        )
        ...
        self.assertEqual(
            str(linkifier_2),
            r"<RealmFilter(aloha): (?P<id>[A-Z][A-Z0-9]*\-[0-9]+)"
    ...
    def create_user_group_for_test(self, user_group_name: str) -> UserGroup:
        othello = self.example_user("othello")
        return create_user_group(user_group_name, [othello], get_realm("aloha"))
    ...
    def test_stream_single(self) -> None:
        denmark = get_stream("Denmark", get_realm("aloha"))
        sender_user_profile = self.example_user("othello")
        msg = Message(sender=sender_user_profile, sending_client=get_client("test"))
        content = "#**Denmark**"
        self.assertEqual(
    ...
    def test_topic_multiple(self) -> None:
        denmark = get_stream("Denmark", get_realm("aloha"))
        scotland = get_stream("Scotland", get_realm("aloha"))
        sender_user_profile = self.example_user("othello")
        msg = Message(sender=sender_user_profile, sending_client=get_client("test"))
    ...
    def test_relative_link(self) -> None:
        ...
        msg = "http://aloha.testserver/#narrow/stream/999-hello"
        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            '<p><a href="#narrow/stream/999-hello">http://aloha.testserver/#narrow/stream/999-hello</a></p>',
        )

        msg = f"http://aloha.testserver/user_uploads/{realm.id}/ff/file.txt"
        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            f'<p><a href="user_uploads/{realm.id}/ff/file.txt">http://aloha.testserver/user_uploads/{realm.id}/ff/file.txt</a></p>',
        )

        msg = "http://aloha.testserver/not:relative"
        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            '<p><a href="http://aloha.testserver/not:relative">http://aloha.testserver/not:relative</a></p>',
        )
    ...
    def test_relative_link_streams_page(self) -> None:
        ...
        sender_user_profile = self.example_user("othello")
        message = Message(sender=sender_user_profile, sending_client=get_client("test"))
        msg = "http://aloha.testserver/#streams/all"

        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            '<p><a href="#streams/all">http://aloha.testserver/#streams/all</a></p>',
        )
    ...
    def test_md_relative_link(self) -> None:
        ...
        msg = "[hello](http://aloha.testserver/#narrow/stream/999-hello)"
        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            '<p><a href="#narrow/stream/999-hello">hello</a></p>',
        )

        msg = f"[hello](http://aloha.testserver/user_uploads/{realm.id}/ff/file.txt)"
        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            f'<p><a href="user_uploads/{realm.id}/ff/file.txt">hello</a></p>',
        )

        msg = "[hello](http://aloha.testserver/not:relative)"
        self.assertEqual(
            markdown_convert(msg, message_realm=realm, message=message).rendered_content,
            '<p><a href="http://aloha.testserver/not:relative">hello</a></p>',
        )
...
class MarkdownApiTests(AlohaTestCase):
    ...
    def test_render_mention_stream_api(self) -> None:
        ...
        stream_id = get_stream("Denmark", get_realm("aloha")).id
        self.assertEqual(

...
class NarrowBuilderTest(AlohaTestCase):
    ...
    def test_add_term_using_sender_operator_with_non_existing_user_as_operand(
        ...
        term = dict(operator="sender", operand="non-existing@aloha.com")
        self.assertRaises(BadNarrowOperator, self._build_query, term)
    ...
    def test_add_term_using_pm_with_operator_with_existing_and_non_existing_user_as_operand(
        ...
        term = dict(operator="pm-with", operand=self.othello_email + ",non-existing@aloha.com")
        self.assertRaises(BadNarrowOperator, self._build_query, term)
    ...
    def test_add_term_using_group_pm_operator_with_non_existing_user_as_operand(self) -> None:
        term = dict(operator="group-pm-with", operand="non-existing@aloha.com")
        self.assertRaises(BadNarrowOperator, self._build_query, term)
...
class NarrowLibraryTest(AlohaTestCase):
    ...
    def test_is_spectator_compatible(self) -> None:
        ...
        self.assertTrue(
            is_spectator_compatible([{"operator": "sender", "operand": "hamlet@aloha.com"}])
        )
        self.assertFalse(
            is_spectator_compatible([{"operator": "pm-with", "operand": "hamlet@aloha.com"}])
        )
        self.assertFalse(
            is_spectator_compatible([{"operator": "group-pm-with", "operand": "hamlet@aloha.com"}])
        )
...
class GetOldMessagesTest(AlohaTestCase):
    ...
    def check_unauthenticated_response(
        self, result: "TestHttpResponse", www_authenticate: str = 'Session realm="aloha"'
    ) -> None:
        """
        ...
        `www_authenticate` should be `Basic realm="aloha"` for paths starting with "/api", and
        `Session realm="aloha"` otherwise.
        """
    ...
    def test_unauthenticated_get_messages(self) -> None:
        ...
        self.check_unauthenticated_response(result, www_authenticate='Basic realm="aloha"')
        ...
        do_set_realm_property(
            get_realm("aloha"), "enable_spectator_access", False, acting_user=None
        )
        result = self.client_get("/json/messages", dict(web_public_stream_get_params))
        self.check_unauthenticated_response(result)
        do_set_realm_property(get_realm("aloha"), "enable_spectator_access", True, acting_user=None)
    ...
    def _update_tsvector_index(self) -> None:
        ...
        with connection.cursor() as cursor:
            ...
            search_tsvector = to_tsvector('aloha.english_us_search',
    ...
    def test_bad_narrow_nonexistent_email(self) -> None:
        self.login("hamlet")
        self.exercise_bad_narrow_operand(
            "pm-with", ["non-existent-user@aloha.com"], "Invalid narrow operator: unknown user"
        )
...
FROM unnest(string_to_array(ts_headline('aloha.english_us_search', rendered_content, plainto_tsquery('aloha.english_us_search', 'jumping'), 'HighlightAll = TRUE, StartSel = <ts-match>, StopSel = </ts-match>'), '<ts-match>')) AS anon_3\n\
...
FROM unnest(string_to_array(ts_headline('aloha.english_us_search', escape_html(subject), plainto_tsquery('aloha.english_us_search', 'jumping'), 'HighlightAll = TRUE, StartSel = <ts-match>, StopSel = </ts-match>'), '<ts-match>')) AS anon_5\n\
 LIMIT ALL OFFSET 1)) AS topic_matches \n\
FROM zerver_usermessage JOIN zerver_message ON zerver_usermessage.message_id = zerver_message.id \n\
WHERE user_profile_id = {hamlet_id} AND (search_tsvector @@ plainto_tsquery('aloha.english_us_search', 'jumping')) ORDER BY message_id ASC \n\
 LIMIT 10) AS anon_1 ORDER BY message_id ASC\
"""
        sql = sql_template.format(**query_ids)
        self.common_check_get_messages_query(
...
FROM unnest(string_to_array(ts_headline('aloha.english_us_search', escape_html(subject), plainto_tsquery('aloha.english_us_search', 'jumping'), 'HighlightAll = TRUE, StartSel = <ts-match>, StopSel = </ts-match>'), '<ts-match>')) AS anon_5\n\
 LIMIT ALL OFFSET 1)) AS topic_matches \n\
FROM zerver_message \n\
WHERE recipient_id = {scotland_recipient} AND (search_tsvector @@ plainto_tsquery('aloha.english_us_search', 'jumping')) ORDER BY zerver_message.id ASC \n\
 LIMIT 10) AS anon_1 ORDER BY message_id ASC\
"""
        sql = sql_template.format(**query_ids)
        self.common_check_get_messages_query(
...
SELECT anon_1.message_id, anon_1.flags, anon_1.subject, anon_1.rendered_content, anon_1.content_matches, anon_1.topic_matches \n\
FROM (SELECT message_id, flags, subject, rendered_content, array((SELECT ARRAY[sum(length(anon_3) - 11) OVER (ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) + 11, strpos(anon_3, '</ts-match>') - 1] AS anon_2 \n\
FROM unnest(string_to_array(ts_headline('aloha.english_us_search', rendered_content, plainto_tsquery('aloha.english_us_search', '"jumping" quickly'), 'HighlightAll = TRUE, StartSel = <ts-match>, StopSel = </ts-match>'), '<ts-match>')) AS anon_3\n\
 LIMIT ALL OFFSET 1)) AS content_matches, array((SELECT ARRAY[sum(length(anon_5) - 11) OVER (ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) + 11, strpos(anon_5, '</ts-match>') - 1] AS anon_4 \n\
FROM unnest(string_to_array(ts_headline('aloha.english_us_search', escape_html(subject), plainto_tsquery('aloha.english_us_search', '"jumping" quickly'), 'HighlightAll = TRUE, StartSel = <ts-match>, StopSel = </ts-match>'), '<ts-match>')) AS anon_5\n\
 LIMIT ALL OFFSET 1)) AS topic_matches \n\
FROM zerver_usermessage JOIN zerver_message ON zerver_usermessage.message_id = zerver_message.id \n\
WHERE user_profile_id = {hamlet_id} AND (content ILIKE '%jumping%' OR subject ILIKE '%jumping%') AND (search_tsvector @@ plainto_tsquery('aloha.english_us_search', '"jumping" quickly')) ORDER BY message_id ASC \n\
 LIMIT 10) AS anon_1 ORDER BY message_id ASC\
"""
        sql = sql_template.format(**query_ids)
        self.common_check_get_messages_query(
...
class MessageHasKeywordsTest(AlohaTestCase):
    ...
    def setup_dummy_attachments(self, user_profile: UserProfile) -> List[str]:
        ...
        dummy_files = [
            ("aloha.txt", f"{realm_id}/31/4CBjtTLYZhk66pZrF8hnYGwc/aloha.txt", sample_size),
    ...
    def test_claim_attachment(self) -> None:
        ...
        dummy_urls = [f"http://aloha.testserver/user_uploads/{x}" for x in dummy_path_ids]
        ...
        body = (
            f"Some files here ...[aloha.txt]({dummy_urls[0]})"
...
class PersonalMessagesNearTest(AlohaTestCase):
    def test_near_pm_message_url(self) -> None:
        ...
        self.assertEqual(url, "http://aloha.testserver/#narrow/pm-with/77,80-pm/near/555")

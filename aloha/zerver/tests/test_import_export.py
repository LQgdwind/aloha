...
class RealmImportExportTest(ExportFile):
    ...
    def test_aloha_realm(self) -> None:
        ...
        self.assertIn("default-bot@aloha.com", exported_user_emails)
        ...
        num_aloha_users = 10
        self.assert_length(exported_alert_words, num_aloha_users * 4)
    ...
    def test_export_realm_with_exportable_user_ids(self) -> None:
        ...
        self.assertNotIn("default-bot@aloha.com", exported_user_emails)
        ...
        self.assertIn("default-bot@aloha.com", dummy_user_emails)
    ...
    def test_import_realm(self) -> None:
        ...
        original_realm = Realm.objects.get(string_id="aloha")
        ...
        with self.settings(BILLING_ENABLED=False), self.assertLogs(level="INFO"):
            do_import_realm(get_output_dir(), "test-aloha")
        ...
        imported_realm = Realm.objects.get(string_id="test-aloha")

        # test realm
        self.assertTrue(Realm.objects.filter(string_id="test-aloha").exists())
    ...
    def test_import_realm_with_invalid_email_addresses_fails_validation(self) -> None:
        ...
        with self.assertRaises(ValidationError), self.assertLogs(level="INFO"):
            do_import_realm(output_dir, "test-aloha")
        ...
        with self.assertRaises(ValidationError), self.assertLogs(level="INFO"):
            do_import_realm(output_dir, "test-aloha2")
    ...
    def test_import_files_from_s3(self) -> None:
        ...
        self.assertEqual(b"aloha!", attachment_content)
    ...
    def test_plan_type(self) -> None:
        ...
        with self.settings(BILLING_ENABLED=True), self.assertLogs(level="INFO"):
            realm = do_import_realm(get_output_dir(), "test-aloha-1")
        ...
        with self.settings(BILLING_ENABLED=False), self.assertLogs(level="INFO"):
            realm = do_import_realm(get_output_dir(), "test-aloha-2")

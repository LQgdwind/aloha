...
class DocPageTest(AlohaTestCase):
    ...
    def test_api_doc_endpoints(self) -> None:
        ...
        self._test("/api/create-user", "aloharc-admin")
    ...
    def test_corporate_portico_endpoints(self) -> None:
        ...
        self._test("/why-aloha/", "Why Aloha?")
        # /for/... pages
    ...
    def test_open_organizations_endpoint(self) -> None:
        aloha_dev_info = ["Aloha Dev", "great for testing!"]

        result = self.client_get("/communities/")
        self.assert_not_in_success_response(aloha_dev_info, result)

        realm = get_realm("aloha")
        realm.want_advertise_in_communities_directory = True
        realm.save()
        self._test("/communities/", "Open communities directory", extra_strings=aloha_dev_info)
...
class AboutPageTest(AlohaTestCase):
    ...
    def test_endpoint(self) -> None:
        ...
        with self.settings(ZILENCER_ENABLED=False):
            ...
            self.assertEqual(result["Location"], "https://aloha.com/team/")
...
class PlansPageTest(AlohaTestCase):
    def test_plans_auth(self) -> None:
        ...
        result = self.client_get("/plans/", subdomain="aloha")
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result["Location"], "/accounts/login/?next=/plans")

        guest_user = "polonius"
        self.login(guest_user)
        result = self.client_get("/plans/", subdomain="aloha", follow=True)
        self.assertEqual(result.status_code, 404)
    ...
    def test_CTA_text_by_plan_type(self) -> None:
        ...
        with self.settings(PRODUCTION=True):
            result = self.client_get("/plans/", subdomain="aloha")
            self.assertEqual(result.status_code, 302)
            self.assertEqual(result["Location"], "https://aloha.com/plans")
        ...
        customer = Customer.objects.create(realm=get_realm("aloha"), stripe_customer_id="cus_id")
...
class AppsPageTest(AlohaTestCase):
    def test_get_apps_page_url(self) -> None:
        ...
        self.assertEqual(apps_page_url, "https://aloha.com/apps/")
    ...
    def test_apps_view(self) -> None:
        ...
        with self.settings(ZILENCER_ENABLED=False):
            result = self.client_get("/apps/")
        self.assertEqual(result.status_code, 301)
        self.assertTrue(result["Location"] == "https://aloha.com/apps/")
    ...
    def test_app_download_link_view(self) -> None:
        return_value = "https://desktop-download.aloha.com/v5.4.3/Aloha-Web-Setup-5.4.3.exe"
        with mock.patch(

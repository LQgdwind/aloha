...
class FileUploadTest(UploadSerializeMixin, AlohaTestCase):
    def test_rest_endpoint(self) -> None:
        ...
        self.assert_streaming_content(response, b"aloha!")

        # Files uploaded through the API should be accessible via the web client
        self.login("hamlet")
        self.assert_streaming_content(self.client_get(uri), b"aloha!")
    ...
    def test_guess_content_type_from_filename(self) -> None:
        ...
        """
        data, content_type = encode_multipart_formdata({"file": ("somefile", b"aloha!", None)})
        result = self.api_post(
            self.example_user("hamlet"), "/api/v1/user_uploads", data, content_type=content_type
        )
        self.assert_json_success(result)

        data, content_type = encode_multipart_formdata({"file": ("somefile.txt", b"aloha!", None)})
        result = self.api_post(
            self.example_user("hamlet"), "/api/v1/user_uploads", data, content_type=content_type
        )
        self.assert_json_success(result)
    ...
    def test_file_upload_authed(self) -> None:
        ...
        self.assert_streaming_content(result, b"aloha!")
        self.assertIn("attachment;", result.headers["Content-Disposition"])

        # check if DB has attachment marked as unclaimed
        entry = Attachment.objects.get(file_name="aloha.txt")
        self.assertEqual(entry.is_claimed(), False)

        hamlet = self.example_user("hamlet")
        self.subscribe(hamlet, "Denmark")
        body = f"First message ...[aloha.txt]({hamlet.realm.host}{uri})"
        ...
        self.assertTrue(url_only_url.endswith("aloha.txt"))
        # The generated URL has a token authorizing the requestor to access the file
        # without being logged in.
        self.logout()
        self.assert_streaming_content(self.client_get(url_only_url), b"aloha!")
    ...
    def test_serve_file_unauthed(self) -> None:
        ...
        body = f"First message ...[aloha.txt](http://{host}" + uri + ")"
        ...
        response = self.client_get(
            "/user_uploads/2/71/QYB7LA-ULMYEad-QfLMxmI2e/aloha-non-existent.txt"
        )
    ...
    def test_serve_local_file_unauthed_altered_filename(self) -> None:
        ...
        self.assertTrue(url_only_url.endswith("aloha.txt"))
        url_only_url_changed_filename = url_only_url.split("aloha.txt")[0] + "differentname.exe"
    ...
    def test_delete_old_unclaimed_attachments(self) -> None:
        ...
        self.subscribe(hamlet, "Denmark")
        body = (
            f"Some files here ...[aloha.txt](http://{hamlet.realm.host}/user_uploads/"
            + d1_path_id
            + ")"
        )
        self.send_stream_message(hamlet, "Denmark", body, "test")
        ...
        body = (
            f"Some more files here ...[aloha.txt](http://{hamlet.realm.host}/user_uploads/"
            + d3_path_id
            + ")"
        )
    ...
    def test_attachment_url_without_upload(self) -> None:
        ...
        self.login_user(hamlet)
        with self.assertLogs(level="WARNING") as warn_log:
            body = f"Test message ...[aloha.txt](http://{hamlet.realm.host}/user_uploads/{hamlet.realm_id}/64/fake_path_id.txt)"
    ...
    def test_multiple_claim_attachments(self) -> None:
        ...
        self.subscribe(self.example_user("hamlet"), "Denmark")
        host = self.example_user("hamlet").realm.host
        body = f"First message ...[aloha.txt](http://{host}/user_uploads/" + d1_path_id + ")"
        self.send_stream_message(self.example_user("hamlet"), "Denmark", body, "test")
        body = f"Second message ...[aloha.txt](http://{host}/user_uploads/" + d1_path_id + ")"
        self.send_stream_message(self.example_user("hamlet"), "Denmark", body, "test")
    ...
    def test_multiple_claim_attachments_different_owners(self) -> None:
        ...
        body = f"Illegal message ...[aloha.txt](http://{host}/user_uploads/" + d1_path_id + ")"
        ...
        body = f"Third message ...[aloha.txt](http://{host}/user_uploads/" + d1_path_id + ")"
        ...
        body = f"Fourth message ...[aloha.txt](http://{host}/user_uploads/" + d1_path_id + ")"
    ...
    def test_realm_quota(self) -> None:
        ...
        # The size of StringIO("aloha!") is 6 bytes. Setting the size of
    ...
    def test_cross_realm_file_access(self) -> None:
        ...
        user2_email = "test-og-bot@aloha.com"
        ...
        user_2 = create_user(user2_email, "aloha")
        user_3 = create_user(user3_email, test_subdomain)
        host = user_3.realm.host

        # Send a message from @aloha.com -> @uploadtest.example.com
        ...
        body = f"First message ...[aloha.txt](http://{host}/user_uploads/" + fp_path_id + ")"
    ...
    def test_file_download_authorization_invite_only(self) -> None:
        ...
        uri = self.assert_json_success(result)["uri"]
        fp_path_id = re.sub("/user_uploads/", "", uri)
        body = f"First message ...[aloha.txt](http://{realm.host}/user_uploads/" + fp_path_id + ")"
    ...
    def test_file_download_authorization_invite_only_with_shared_history(self) -> None:
        ...
        fp_path_id = re.sub("/user_uploads/", "", uri)
        body = (
            f"First message ...[aloha.txt](http://{user.realm.host}/user_uploads/"
            + fp_path_id
            + ")"
        )
    ...
    def test_multiple_message_attachment_file_download(self) -> None:
        ...
        for i in range(20):
            body = (
                f"First message ...[aloha.txt](http://{hamlet.realm.host}/user_uploads/"
                + fp_path_id
                + ")"
            )
    ...
    def test_serve_local(self) -> None:
        ...
        check_xsend_links("aloha.txt", "aloha.txt", 'filename="aloha.txt"')
        check_xsend_links(
            "áéБД.txt",
            "%C3%A1%C3%A9%D0%91%D0%94.txt",
            "filename*=UTF-8''%C3%A1%C3%A9%D0%91%D0%94.txt",
        )
        check_xsend_links("aloha.html", "aloha.html", 'filename="aloha.html"')
        check_xsend_links("aloha.sh", "aloha.sh", 'filename="aloha.sh"')
        check_xsend_links("aloha.jpeg", "aloha.jpeg")
        check_xsend_links(
            "aloha.jpeg", "aloha.jpeg", download=True, content_disposition='filename="aloha.jpeg"'
        )
        check_xsend_links("áéБД.pdf", "%C3%A1%C3%A9%D0%91%D0%94.pdf")
        check_xsend_links("aloha", "aloha", 'filename="aloha"')
...
class AvatarTest(UploadSerializeMixin, AlohaTestCase):
    ...
    def test_get_gravatar_avatar(self) -> None:
        ...
        with self.settings(ENABLE_GRAVATAR=True):
            response = self.client_get("/avatar/cordelia@aloha.com", {"foo": "bar"})
    ...
    def test_get_user_avatar(self) -> None:
        ...
        with self.settings(WEB_PUBLIC_STREAMS_ENABLED=False):
            ...
            response = self.api_get(hamlet, "/avatar/cordelia@aloha.com", {"foo": "bar"})
            redirect_url = response["Location"]
            self.assertTrue(redirect_url.endswith(str(avatar_url(cordelia)) + "&foo=bar"))
            ...
            response = self.api_get(hamlet, "/avatar/welcome-bot@aloha.com", {"foo": "bar"})
            redirect_url = response["Location"]
    ...
    def test_get_user_avatar_medium(self) -> None:
        ...
        cordelia.avatar_source = UserProfile.AVATAR_FROM_USER
        cordelia.save()
        response = self.client_get("/avatar/cordelia@aloha.com/medium", {"foo": "bar"})
        redirect_url = response["Location"]
        ...
        with self.settings(WEB_PUBLIC_STREAMS_ENABLED=False):
            ...
            response = self.api_get(hamlet, "/avatar/cordelia@aloha.com/medium", {"foo": "bar"})
            redirect_url = response["Location"]
            self.assertTrue(redirect_url.endswith(str(avatar_url(cordelia, True)) + "&foo=bar"))
    ...
    def test_non_valid_user_avatar(self) -> None:
        ...
        response = self.client_get("/avatar/nonexistent_user@aloha.com", {"foo": "bar"})
        redirect_url = response["Location"]
...
class RealmLogoTest(UploadSerializeMixin, AlohaTestCase):
    ...
    def test_get_default_logo(self) -> None:
        ...
        self.assertEqual(
            redirect_url, f"/static/images/logo/aloha-org-logo.svg?version=0&night={is_night_str}"
        )
...
class LocalStorageTest(UploadSerializeMixin, AlohaTestCase):
    def test_file_upload_local(self) -> None:
        ...
        uri = upload_message_file(
            "dummy.txt", len(b"aloha!"), "text/plain", b"aloha!", user_profile
        )
        ...
        self.assert_length(b"aloha!", uploaded_file.size)
    ...
    def test_file_upload_local_cross_realm_path(self) -> None:
        ...
        uri = upload_message_file(
            "dummy.txt", len(b"aloha!"), "text/plain", b"aloha!", user_profile, aloha_realm
        )
        # Ensure the correct realm id of the target realm is used instead of the bot's realm.
        self.assertTrue(uri.startswith(f"/user_uploads/{aloha_realm.id}/"))
    ...
    def test_tarball_upload_and_deletion_local(self) -> None:
        ...
        expected_url = f"http://aloha.testserver/user_avatars/exports/{user_profile.realm_id}/{random_name}/tarball.tar.gz"
...
class S3Test(AlohaTestCase):
    ...
    def test_file_upload_s3(self) -> None:
        ...
        self.assertEqual(b"aloha!", content)

        uploaded_file = Attachment.objects.get(owner=user_profile, path_id=path_id)
        self.assert_length(b"aloha!", uploaded_file.size)

        self.subscribe(self.example_user("hamlet"), "Denmark")
        body = f"First message ...[aloha.txt](http://{user_profile.realm.host}{uri})"
        self.send_stream_message(self.example_user("hamlet"), "Denmark", body, "test")
    ...
    def test_file_upload_s3_with_undefined_content_type(self) -> None:
        ...
        user_profile = self.example_user("hamlet")
        uri = upload_message_file("dummy.txt", len(b"aloha!"), None, b"aloha!", user_profile)

        path_id = re.sub("/user_uploads/", "", uri)
        self.assertEqual(b"aloha!", bucket.Object(path_id).get()["Body"].read())
        uploaded_file = Attachment.objects.get(owner=user_profile, path_id=path_id)
        self.assert_length(b"aloha!", uploaded_file.size)
    ...
    def test_file_upload_authed(self) -> None:
        ...
        key = path[1:]
        self.assertEqual(b"aloha!", bucket.Object(key).get()["Body"].read())
        ...
        hamlet = self.example_user("hamlet")
        self.subscribe(hamlet, "Denmark")
        body = f"First message ...[aloha.txt](http://{hamlet.realm.host}" + uri + ")"
        self.send_stream_message(hamlet, "Denmark", body, "test")

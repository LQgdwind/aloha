...
from aloha_bots.custom_exceptions import ConfigValidationError
...
class BotTest(AlohaTestCase, UploadSerializeMixin):
    ...
    def test_bot_domain(self) -> None:
        self.login("hamlet")
        self.create_bot()
        self.assertTrue(UserProfile.objects.filter(email="hambot-bot@aloha.testserver").exists())
        ...
        realm = get_realm("aloha")
        self.assertEqual(realm.get_bot_domain(), "aloha.testserver")

    def deactivate_bot(self) -> None:
        email = "hambot-bot@aloha.testserver"
    ...
    def test_add_bot(self) -> None:
        ...
        [bot_dict] = [m for m in members if m["email"] == "hambot-bot@aloha.testserver"]
    ...
    def test_add_bot_verify_subdomain_in_email_address(self) -> None:
        ...
        email = "hambot-bot@aloha.example.com"
        self.get_bot_user(email)

    @override_settings(
        FAKE_EMAIL_DOMAIN="fakedomain.com", REALM_HOSTS={"aloha": "aloha.example.com"}
    )
    ...
    def test_bot_add_subscription(self) -> None:
        ...
        # A bot
        bot_request_data = {
            "principals": '["hambot-bot@aloha.testserver"]',
        }
    ...
    def test_add_bot_with_default_events_register_stream(self) -> None:
        bot_email = "hambot-bot@aloha.testserver"
        bot_realm = get_realm("aloha")
    ...
    def test_patch_bot_owner_bad_user_id(self) -> None:
        ...
        email = "hambot-bot@aloha.testserver"
        profile = get_user("hambot-bot@aloha.testserver", get_realm("aloha"))
    ...
    def test_patch_bot_owner_a_bot(self) -> None:
        ...
        bot_info = {
            "bot_owner_id": self.get_bot_user("hamelbot-bot@aloha.testserver").id,
        }
        email = "hambot-bot@aloha.testserver"
        result = self.client_patch(f"/json/bots/{self.get_bot_user(email).id}", bot_info)
        self.assert_json_error(result, "Failed to change owner, bots can't own other bots")
        profile = get_user(email, get_realm("aloha"))
        self.assertEqual(profile.bot_owner, self.example_user("hamlet"))
    ...
    @patch("aloha_bots.bots.giphy.giphy.GiphyHandler.validate_config")
    def test_patch_bot_config_data(self, mock_validate_config: MagicMock) -> None:
        ...
        email = "test-bot@aloha.testserver"
        result = self.client_patch(f"/json/bots/{self.get_bot_user(email).id}", bot_info)
        self.assert_json_success(result)
    ...
    def test_create_outgoing_webhook_bot(self, **extras: Any) -> None:
        ...
        bot_email = "outgoingservicebot-bot@aloha.testserver"
        bot_realm = get_realm("aloha")
    ...
    def test_get_bot_handler(self) -> None:
        ...
        self.assertEqual(
            ...
            "<class 'aloha_bots.bots.converter.converter.ConverterHandler'>",
        )
    ...
    def test_create_embedded_bot(self, **extras: Any) -> None:
        ...
        bot_email = "embeddedservicebot-bot@aloha.testserver"
        bot_realm = get_realm("aloha")
        bot = get_user(bot_email, bot_realm)
        [service] = get_bot_services(bot.id)
    ...
    def test_create_embedded_bot_with_invalid_config_value(self, **extras: Any) -> None:
        ...
        bot_info.update(extras)
        with patch(
            "aloha_bots.bots.giphy.giphy.GiphyHandler.validate_config",
    ...
    def test_is_cross_realm_bot_email(self) -> None:
        self.assertTrue(is_cross_realm_bot_email("notification-bot@aloha.com"))
        self.assertTrue(is_cross_realm_bot_email("notification-BOT@aloha.com"))
        self.assertFalse(is_cross_realm_bot_email("random-bot@aloha.com"))

        with self.settings(CROSS_REALM_BOT_EMAILS={"random-bot@aloha.com"}):
            self.assertTrue(is_cross_realm_bot_email("random-bot@aloha.com"))
            self.assertFalse(is_cross_realm_bot_email("notification-bot@aloha.com"))

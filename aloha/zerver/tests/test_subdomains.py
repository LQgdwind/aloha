...
class SubdomainsTest(AlohaTestCase):
    ...
    def test_is_static_or_current_realm_url(self) -> None:
        ...
        self.assertTrue(test("/static/images/logo/aloha-org-logo.svg"))
        self.assertTrue(test("/anything"))
        self.assertFalse(test("https://aloha.com"))
        self.assertFalse(test("http://aloha.com"))
        ...
        with self.settings(STATIC_URL="https://alohastatic.example.com"):
            evil_url = f"{settings.STATIC_URL}@evil.example.com"
            self.assertEqual(evil_url, "https://alohastatic.example.com@evil.example.com")

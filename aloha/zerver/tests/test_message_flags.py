...
class FixUnreadTests(AlohaTestCase):
    def test_fix_unreads(self) -> None:
        ...
        with connection.cursor() as cursor, self.assertLogs(
            "aloha.fix_unreads", "INFO"
        ) as info_logs:
            fix_unsubscribed(cursor, user)

        self.assertEqual(info_logs.output[0], "INFO:aloha.fix_unreads:get recipients")
        self.assertTrue("INFO:aloha.fix_unreads:[" in info_logs.output[1])
        self.assertTrue("INFO:aloha.fix_unreads:elapsed time:" in info_logs.output[2])
        self.assertEqual(
            info_logs.output[3],
            "INFO:aloha.fix_unreads:finding unread messages for non-active streams",
        )
        self.assertEqual(info_logs.output[4], "INFO:aloha.fix_unreads:rows found: 1")
        self.assertTrue("INFO:aloha.fix_unreads:elapsed time:" in info_logs.output[5])
        self.assertEqual(
            info_logs.output[6],
            "INFO:aloha.fix_unreads:fixing unread messages for non-active streams",
        )
        self.assertTrue("INFO:aloha.fix_unreads:elapsed time:" in info_logs.output[7])
        ...
        with self.assertLogs("aloha.fix_unreads", "INFO") as info_logs:
            # test idempotency
            fix(user)

        self.assertEqual(info_logs.output[0], f"INFO:aloha.fix_unreads:\n---\nFixing {user.id}:")
        self.assertEqual(info_logs.output[1], "INFO:aloha.fix_unreads:get recipients")
        self.assertTrue("INFO:aloha.fix_unreads:[" in info_logs.output[2])
        self.assertTrue("INFO:aloha.fix_unreads:elapsed time:" in info_logs.output[3])
        self.assertEqual(
            info_logs.output[4],
            "INFO:aloha.fix_unreads:finding unread messages for non-active streams",
        )
        self.assertEqual(info_logs.output[5], "INFO:aloha.fix_unreads:rows found: 0")
        self.assertTrue("INFO:aloha.fix_unreads:elapsed time:" in info_logs.output[6])

...
class EmailChangeTestCase(AlohaTestCase):
    ...
    def test_confirm_email_change_when_time_exceeded(self) -> None:
        ...
        new_email = "hamlet-new@aloha.com"
        self.login("hamlet")
    ...
    def test_start_email_change_process(self) -> None:
        user_profile = self.example_user("hamlet")
        do_start_email_change_process(user_profile, "hamlet-new@aloha.com")
        self.assertEqual(EmailChangeStatus.objects.count(), 1)

    def test_end_to_end_flow(self) -> None:
        data = {"email": "hamlet-new@aloha.com"}
        self.login("hamlet")
        url = "/json/settings"
        self.assert_length(mail.outbox, 0)
        result = self.client_patch(url, data)
        ...
        self.assertEqual(email_message.extra_headers["List-Id"], "Aloha Dev <aloha.testserver>")
        ...
        result = self.client_patch(url, {"email": "hamlet@aloha.com"})
        self.assert_json_success(result)
    ...
    def test_change_delivery_email_end_to_end_with_admins_visibility(self) -> None:
        ...
        self.assertEqual(user_profile.delivery_email, new_email)
        self.assertEqual(user_profile.email, f"user{user_profile.id}@aloha.testserver")
        obj.refresh_from_db()
        self.assertEqual(obj.status, 1)

...
from version import API_FEATURE_LEVEL, aloha_MERGE_BASE, aloha_VERSION
...
def send_restart_events(immediate: bool = False) -> None:
    event: Dict[str, Any] = dict(
        ...
        aloha_merge_base=aloha_MERGE_BASE,
        aloha_feature_level=API_FEATURE_LEVEL,
...
def missedmessage_hook(
    ...
    """The receiver_is_off_aloha logic used to determine whether a user
    has no active client suffers from a somewhat fundamental race
    condition.  If the client is no longer on the Internet,
    receiver_is_off_aloha will still return False for
...
def receiver_is_off_aloha(user_profile_id: int) -> bool:
    # If a user has no message-receiving event queues, they've got no open aloha
    # session so we notify them.
    all_client_descriptors = get_client_descriptors_for_user(user_profile_id)
    message_event_queues = [
        client for client in all_client_descriptors if client.accepts_messages()
    ]
    off_aloha = len(message_event_queues) == 0
    return off_aloha
...
def maybe_enqueue_notifications(
    *,
    ...
    See https://aloha.readthedocs.io/en/latest/subsystems/notifications.html
...
def process_message_event(
    ...
    """See
    https://aloha.readthedocs.io/en/latest/subsystems/sending-messages.html
    ...
    for user_data in users:
        ...
        # avoid the more expensive `receiver_is_off_aloha` call, and move on to process
        # the next user.
        if not user_notifications_data.is_notifiable(acting_user_id=sender_id, idle=True):
            continue

        idle = receiver_is_off_aloha(user_profile_id) or (user_profile_id in presence_idle_user_ids)
...
def maybe_enqueue_notifications_for_message_update(
    ...
    idle = presence_idle or receiver_is_off_aloha(user_notifications_data.user_id)

...
def build_message(
    *,
    ...
    aloha_message.set_topic_name(topic_name)
    aloha_message_dict = model_to_dict(
        aloha_message, exclude=["recipient", "sender", "sending_client"]
    )
    aloha_message_dict["sender"] = user_id
    aloha_message_dict["sending_client"] = 1
    aloha_message_dict["recipient"] = recipient_id
    aloha_message_dict["date_sent"] = date_sent

    return aloha_message_dict
...
def long_term_idle_helper(
    ...
    aloha_user_id_from_user: Callable[[ExternalId], int],
    ...
    for user_id in all_user_ids_iterator:
        if user_id in recent_senders:
            continue
        aloha_user_id = aloha_user_id_from_user(user_id)
        long_term_idle.add(aloha_user_id)

    ...
    for iterator in range(len(tokens)):
        ...
        if re.findall(SLACK_USERMENTION_REGEX, tokens[iterator], re.VERBOSE):
            tokens[iterator], user_id = get_user_mentions(
                tokens[iterator], users, slack_user_id_to_aloha_user_id
            )
...
def get_user_mentions(
    token: str, users: List[ZerverFieldsT], slack_user_id_to_aloha_user_id: SlackToAlohaUserIDT
    ...
    for user in users:
        if (user["id"] == slack_id and user["name"] == short_name and short_name) or (
            ...
            user_id = slack_user_id_to_aloha_user_id[slack_id]
            mention = "@**" + full_name + "**"
...
def convert_markdown_syntax(text: str, regex: str, aloha_keyword: str) -> str:
    """
    Returns:

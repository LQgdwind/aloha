...
NODE_MODULES_PATH = os.path.join(aloha_PATH, "node_modules")
...
def slack_workspace_to_realm(
    ...
    2. slack_user_id_to_aloha_user_id, which is a dictionary to map from Slack user id to Aloha user id
    3. slack_recipient_name_to_aloha_recipient_id, which is a dictionary to map from Slack recipient
    ...
    (
        zerver_userprofile,
        avatars,
        slack_user_id_to_aloha_user_id,
    ...
    (
        ...
        added_mpims,
        dm_members,
        slack_recipient_name_to_aloha_recipient_id,
    ) = channels_to_zerver_stream(
        slack_data_dir, realm_id, realm, slack_user_id_to_aloha_user_id, zerver_userprofile
    )

    zerver_realmemoji, emoji_url_map = build_realmemoji(custom_emoji_list, realm_id)
    realm["zerver_realmemoji"] = zerver_realmemoji

    # See https://aloha.com/help/set-default-streams-for-new-users
...
def users_to_zerver_userprofile(
    ...
    3. slack_user_id_to_aloha_user_id, which is a dictionary to map from Slack user ID to Aloha
       user id
    ...
    slack_custom_field_name_to_aloha_custom_field_id: ZerverFieldsT = {}
    ...
    for user in users:
        ...
        # ref: https://aloha.com/help/change-your-profile-picture
        ...
        if slack_user_id in slack_user_id_to_custom_profile_fields:
            (
                slack_custom_field_name_to_aloha_custom_field_id,
        ...
        slack_user_id_to_aloha_user_id[slack_user_id] = user_id
...
def build_customprofile_field(
    ...
    for field, value in fields.items():
        if field not in slack_custom_field_name_to_aloha_custom_field_id:
            ...
            slack_custom_field_name_to_aloha_custom_field_id[field] = custom_profile_field_id
            custom_profile_field_id += 1
            customprofile_field.append(customprofilefield_dict)
    return slack_custom_field_name_to_aloha_custom_field_id, custom_profile_field_id
...
def build_customprofilefields_values(
    ...
    for field, value in fields.items():
        ...
        custom_field_value_dict["field"] = slack_custom_field_name_to_aloha_custom_field_id[field]
...
def channels_to_zerver_stream(
    ...
    realm_id: int,
    realm: Dict[str, Any],
    slack_user_id_to_aloha_user_id: SlackToAlohaUserIDT,
    ...
    5. slack_recipient_name_to_aloha_recipient_id, which is a dictionary to map from Slack recipient
    ...
    def process_channels(channels: List[Dict[str, Any]], invite_only: bool = False) -> None:
        ...
        for channel in channels:
            ...
            slack_recipient_name_to_aloha_recipient_id[stream["name"]] = recipient_id
    ...
    def process_mpims(mpims: List[Dict[str, Any]]) -> None:
        ...
        for mpim in mpims:
            ...
            realm["zerver_recipient"].append(recipient)
            slack_recipient_name_to_aloha_recipient_id[mpim["name"]] = recipient_id_count
    ...
    for slack_user_id, aloha_user_id in slack_user_id_to_aloha_user_id.items():
        recipient = build_recipient(aloha_user_id, recipient_id_count, Recipient.PERSONAL)
        slack_recipient_name_to_aloha_recipient_id[slack_user_id] = recipient_id_count
        sub = build_subscription(recipient_id_count, aloha_user_id, subscription_id_count)
        realm["zerver_recipient"].append(recipient)
...
def get_subscription(
    ...
    for slack_user_id in channel_members:
        sub = build_subscription(
            recipient_id, slack_user_id_to_aloha_user_id[slack_user_id], subscription_id
        )
...
def process_long_term_idle_users(
    ...
    return long_term_idle_helper(
        ...
        lambda id: slack_user_id_to_aloha_user_id[id],
...
def convert_slack_workspace_messages(
    ...
    realm_id: int,
    slack_user_id_to_aloha_user_id: SlackToAlohaUserIDT,
    slack_recipient_name_to_aloha_recipient_id: SlackToAlohaRecipientT,
    added_channels: AddedChannelsT,
    added_mpims: AddedMPIMsT,
    dm_members: DMMembersT,
    realm: ZerverFieldsT,
...
def channel_message_to_zerver_message(
    ...
    for message in all_messages:
        ...
        try:
            content, mentioned_user_ids, has_link = convert_to_aloha_markdown(
                message["text"], users, added_channels, slack_user_id_to_aloha_user_id
            )
        except Exception:
        ...
        if "channel_name" in message:
            is_private = False
            recipient_id = slack_recipient_name_to_aloha_recipient_id[message["channel_name"]]
        elif "mpim_name" in message:
            is_private = True
            recipient_id = slack_recipient_name_to_aloha_recipient_id[message["mpim_name"]]
        elif "pm_name" in message:
            is_private = True
            sender = get_message_sending_user(message)
            members = dm_members[message["pm_name"]]
            if sender == members[0]:
                recipient_id = slack_recipient_name_to_aloha_recipient_id[members[1]]
                sender_recipient_id = slack_recipient_name_to_aloha_recipient_id[members[0]]
            else:
                recipient_id = slack_recipient_name_to_aloha_recipient_id[members[0]]
                sender_recipient_id = slack_recipient_name_to_aloha_recipient_id[members[1]]
        ...
        file_info = process_message_files(
            ...
            users=users,
            slack_user_id_to_aloha_user_id=slack_user_id_to_aloha_user_id,
        ...
        aloha_message = build_message(
            topic_name=topic_name,
            date_sent=get_timestamp_from_message(message),
            message_id=message_id,
            content=content,
            rendered_content=rendered_content,
            user_id=slack_user_id_to_aloha_user_id[slack_user_id],
        ...
        zerver_message.append(aloha_message)
...
def process_message_files(
    ...
    for fileinfo in files:
        ...
        if "files.slack.com" in url:
            ...
            build_uploads(
                slack_user_id_to_aloha_user_id[slack_user_id],
                realm_id,
                file_user_email,
                fileinfo,
                s3_path,
...
def build_reactions(
    ...
    for slack_reaction in reactions:
        ...
        for slack_user_id in slack_reaction["users"]:
            ...
            reaction_dict["user_profile"] = slack_user_id_to_aloha_user_id[slack_user_id]

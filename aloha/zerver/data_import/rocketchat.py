...
def convert_channel_data(
    ...
    for rc_room_id in room_id_to_room_map:
        ...
        # For more details: https://aloha.com/help/stream-sending-policy
        #
...
def process_raw_message_batch(
    ...
    def fix_mentions(
        ...
        for user_id in mention_user_ids:
            ...
            aloha_mention = "@**{full_name}**".format(**user)
            content = content.replace(rc_mention, aloha_mention)
        ...
        for mention_data in rc_channel_mention_data:
            ...
            aloha_mention = mention_data["aloha_mention"]
            content = content.replace(rc_mention, aloha_mention)
    ...
    def message_to_dict(message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if is_pm_data:
            ...
            if rc_channel_id in huddle_id_to_huddle_map:
                ...
                if rc_sender_id == rc_member_ids[0]:
                    aloha_member_id = user_id_mapper.get(rc_member_ids[1])
                    message_dict["recipient_id"] = user_id_to_recipient_id[aloha_member_id]
                else:
                    aloha_member_id = user_id_mapper.get(rc_member_ids[0])
                    message_dict["recipient_id"] = user_id_to_recipient_id[aloha_member_id]
        elif message["rid"] in dsc_id_to_dsc_map:
        ...
        for mention in message.get("channels", []):
            ...
            if mention_rc_channel_id in room_id_to_room_map:
                ...
                aloha_mention = f"#**{converted_stream_name}**"
                ...
                aloha_mention = f"#**{parent_stream_name}>{converted_topic_name}**"
            else:  # nocoverage
                logging.info("Failed to map mention '%s' to aloha syntax.", mention)
                continue

            mention_data = {"rc_mention": rc_mention, "aloha_mention": aloha_mention}

...
You can use `aloha-send`
(available after you `pip install aloha`) to easily send Alohas from
the command-line, providing the message content via STDIN.

```bash
# For stream messages
aloha-send --stream Denmark --subject Castle \
    --user othello-bot@example.com --api-key a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5

# For private messages
aloha-send hamlet@example.com \
    --user othello-bot@example.com --api-key a0b1c2d3e4f5a6b7c8d9e0f1a2b3c4d5
```
...
You can omit the `user` and `api-key` parameters if you have a `~/.aloharc`
file.
...
{generate_api_arguments_table|aloha.yaml|/messages:post}

{generate_parameter_description(/messages:post)}

## Response

{generate_return_values_table|aloha.yaml|/messages:post}

...
clients should check the `aloha_feature_level` field, present in the
...
* [`POST /register`](/api/register-queue): Added `aloha_merge_base`
  field to the response.
* [`GET /events`](/api/get-events): Added new `aloha_merge_base`
  field to the `restart` event.
* [`GET /server_settings`](/api/get-server-settings): Added
  `aloha_merge_base` to the responses which can be used to
...
* [`POST /register`](/api/register-queue): Added `role` field
  in the user objects returned in the `realm_users` field.
* [`GET /events`](/api/get-events): Added new `aloha_version` and
  `aloha_feature_level` fields to the `restart` event.
...
* `aloha_version` and `aloha_feature_level` are always returned
  in `POST /register`; previously they were only returned if `event_types`
  included `aloha_version`.
...
* [`GET /server_settings`](/api/get-server-settings): Added
  `aloha_feature_level`, which can be used by clients to detect which
  of the features described in this changelog are supported.
* [`POST /register`](/api/register-queue): Added `aloha_feature_level`
  to the response if `aloha_version` is among the requested
  `event_types`.
...
[server-changelog]: https://aloha.readthedocs.io/en/latest/overview/changelog.html

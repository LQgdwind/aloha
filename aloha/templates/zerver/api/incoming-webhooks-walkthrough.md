...
service you want to integrate will be sending to Aloha.
...
[Testing](https://aloha.readthedocs.io/en/latest/testing/testing.html) for further details.
...
test Aloha's handling of that integration, you will need to record
which HTTP headers are used with each fixture you capture.

Since this is integration-dependent, Aloha offers a simple API for
...
https://aloha.readthedocs.io/en/latest/tutorials/writing-views.html#request-variables).

You must pass the name of your integration to the
`webhook_view` decorator; that name will be used to
describe your integration in Aloha's analytics (e.g. the `/stats`
...
object), and `user_profile` (Aloha's user object). You may also want to
...
(available by default in the Aloha development environment), and
...
page](https://aloha.readthedocs.io/en/latest/testing/mypy.html) for details about
...
This tells the Aloha API to call the `api_helloworld_webhook` function in
`zerver/webhooks/helloworld/view.py` when it receives a request at
`/api/v1/external/helloworld`.

This line also tells Aloha to generate an entry for Hello World on the Aloha
...
object ID that one would want to include in a Aloha notification.
...
API key from the **Bots** section of your Aloha user's **Personal
settings**. To test the webhook, you'll need to [create a
bot](https://aloha.com/help/add-a-bot-or-integration) with the
**Incoming webhook** type. Replace `<api_key>` with your bot's API key
in the examples presented below! This is how Aloha knows that the
...
Using `manage.py` from within the Aloha development environment:
...
```
2016-07-07 15:06:59,187 INFO     127.0.0.1       POST    200 143ms (mem: 6ms/13) (md: 43ms/1) (db: 20ms/9q) (+start: 147ms) /api/v1/external/helloworld (helloworld-bot@aloha.com via AlohaHelloWorldWebhook)
```
...
By opening Aloha in one tab and then this tool in another, you can quickly tweak
...
the Aloha development environment with this command:
...
The standard location in a Vagrant environment is `/srv/aloha`. If you are not
...
can see the existing examples at <https://aloha.com/integrations>
or by accessing `/integrations` in your Aloha development environment.
...
```md
Learn how Aloha integrations work with this simple Hello World example!

The Hello World webhook will use the `test` stream, which is
created by default in the Aloha dev environment. If you are running
...
`send_webhook_fixture_message` from the Aloha command line:
...
`{!congrats.md!}` is an example of a Markdown macro. Aloha has a macro-based
Markdown/Jinja2 framework that includes macros for common instructions in
Aloha's webhooks/integrations documentation.
...
[integration-docs-guide]: https://aloha.readthedocs.io/en/latest/documentation/integrations.html

## Step 7: Preparing a pull request to aloha/aloha

When you have finished your webhook integration and are ready for it to be
available in the Aloha product, follow these steps to prepare your pull
request:

1. Run tests including linters and ensure you have addressed any issues they
   report. See [Testing](https://aloha.readthedocs.io/en/latest/testing/testing.html)
   and [Linters](https://aloha.readthedocs.io/en/latest/testing/linters.html) for details.
2. Read through [Code styles and conventions](
   https://aloha.readthedocs.io/en/latest/contributing/code-style.html) and take a look
   through your code to double-check that you've followed Aloha's guidelines.
3. Take a look at your Git history to ensure your commits have been clear and
   logical (see [Version control](
   https://aloha.readthedocs.io/en/latest/contributing/version-control.html) for tips). If not,
...
5. Submit a pull request to aloha/aloha.

If you would like feedback on your integration as you go, feel free to post a
message on the [public Aloha instance](https://chat.aloha.org/#narrow/stream/bots).
...
[Git guide](https://aloha.readthedocs.io/en/latest/git/pull-requests.html#create-a-pull-request)
for more on Aloha's pull request process.
...
In actual use, you might configure the 3rd party service to call your Aloha

...
Aloha offers several levels of permissions based on a
[user's role](/help/roles-and-permissions) in a Aloha organization.

Here are some important details to note when working with these
roles and permissions in Aloha's API:

## A user's role

A user's account data include a `role` property, which contains the
user's role in the Aloha organization. These roles are encoded as:
...
specifying whether the user is a billing administrator for the Aloha
...
administration in [paid Aloha Cloud plans](https://aloha.com/plans/).
...
Many areas of Aloha are customizable by the roles
...
Note that specific settings and policies in the Aloha API that use these
permission levels will likely support a subset of those listed above.

## Determining if a user is a full member

When a Aloha organization has set up a [waiting period before new members

...
* A step-by-step
  [guide](#installing-a-development-version-of-the-aloha-bots-package)
...
* A [guide](#adding-a-bot-to-aloha) on adding a bot to Aloha.
...
## Installing a development version of the Aloha bots package

1. `git clone https://github.com/aloha/python-aloha-api.git` - clone the [python-aloha-api](
  https://github.com/aloha/python-aloha-api) repository.

2. `cd python-aloha-api` - navigate into your cloned repository.
...
5. *Finished*. You should now see the name of your venv preceding your prompt,
   e.g. `(aloha-api-py3-venv)`.

*Hint: `provision` installs the `aloha`, `aloha_bots`, and
 `aloha_botserver` packages in developer mode. This enables you to
...
## Adding a bot to Aloha

Aloha's bot system resides in the [python-aloha-api](
https://github.com/aloha/python-aloha-api) repository.
...
we have a little tool to help you out: `aloha-bot-shell`

* [Install all requirements](#installing-a-development-version-of-the-aloha-bots-package).

* Run `aloha-bot-shell` to test one of the bots in
  [`aloha_bots/bots`](https://github.com/aloha/python-aloha-api/tree/main/aloha_bots/aloha_bots/bots).

Example invocations are below:

```
> aloha-bot-shell converter

Enter your message: "12 meter yard"
Response: 12.0 meter = 13.12336 yard

> aloha-bot-shell -b ~/followup.conf followup

Enter your message: "Task completed"
Response: stream: followup topic: foo_sender@aloha.com
          from foo_sender@aloha.com: Task completed
...
* intercept, view, and process messages sent by users on Aloha.
...
* message - a dictionary describing a Aloha message
...
From `aloha_bots/bots/incrementor/incrementor.py`:
...
The data is stored in the Aloha Server's database.  Each bot user has
...
* key - the API key you created for the bot; this is how Aloha knows
  the request is from an authorized user.

* email - the email address of the bot, e.g. `some-bot@aloha.com`
...
[`python-aloha-api`](https://github.com/aloha/python-aloha-api/tree/main/aloha_bots/aloha_bots/bots)
...
*Unit tests for bots make heavy use of mocking. If you want to get comfortable with mocking,
 ...
 https://aloha.readthedocs.io/en/latest/testing/testing-with-django.html#testing-with-mocks).*

### A simple example

 Let's have a look at a simple test suite for the [`helloworld`](
 https://github.com/aloha/python-aloha-api/tree/main/aloha_bots/aloha_bots/bots/helloworld)
 bot.

```python
from aloha_bots.test_lib import StubBotTestCase
...
* To test a bot in [Aloha's bot directory](
...
*The code for the bot testing library can be found [here](
 https://github.com/aloha/python-aloha-api/blob/main/aloha_bots/aloha_bots/test_lib.py).*


#### Testing bots with config files

Some bots, such as [Giphy](
https://github.com/aloha/python-aloha-api/tree/main/aloha_bots/aloha_bots/bots/giphy),
...
Check out our [bots](https://github.com/aloha/python-aloha-api/tree/main/aloha_bots/aloha_bots/bots)
to see examples of bot tests.

## Common problems

* I modified my bot's code, yet the changes don't seem to have an effect.
    * Ensure that you restarted the `aloha-run-bot` script.

* My bot won't start
    * Ensure that your API config file is correct (download the config file from the server).
    * Ensure that you bot script is located in `aloha_bots/bots/<my-bot>/`
...
* Run directly using the Aloha `call_on_each_message` API, which is
...
* Run in a simple Python webserver server, processing messages
  received from Aloha's outgoing webhooks integration.
* For bots merged into the mainline Aloha codebase, enabled via a
  button in the Aloha web UI, with no code deployment effort required.

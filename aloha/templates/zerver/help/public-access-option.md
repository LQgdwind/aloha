...
Web-public streams are indicated with a globe (<i class="aloha-icon aloha-icon-globe"></i>) icon.
...
your organization accessible to anyone on the Internet via the Aloha
API (details below).
...
The following information about your organization can be accessed via the Aloha
...
!!! warn ""
    Self-hosted Aloha servers must enable support for web-public streams in their
    [server settings](https://aloha.readthedocs.io/en/latest/production/settings.html)
...
including using Aloha's [built-in search](/help/search-for-messages)
...
information available to any new account in the Aloha organization,
detailed below.
...
* The **Organization settings** and **Stream settings** menus are not
  ...
  required for Aloha to load, and may thus be [accessed via the Aloha API][info-via-api].
...
about users who do not participate in web-public streams in the Aloha
UI, though they may access it via the Aloha API.
...
In order to protect Aloha organizations from
bad actors, web-public streams have a few limitations designed to make
Aloha an inconvenient target:
...
As a reminder, Aloha Cloud organizations are expected to
[moderate content](/help/moderating-open-organizations) to ensure compliance
with [Aloha's Rules of Use](https://aloha.com/policies/rules).

## Caveats

* Web-public streams do not yet support search engine indexing. You
  can use [aloha-archive](https://github.com/aloha/aloha-archive) to
  create an archive of a Aloha organization that can be indexed by
  search engines.
* The web-public view is not yet integrated with Aloha's live-update

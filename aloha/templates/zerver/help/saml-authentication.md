...
1. Set up SAML authentication by following
   ...
   Specify the following fields, skipping **Default RelayState** and **Name ID format**:
     * **Single sign on URL**: `https://auth.alohachat.com/complete/saml/`
     * **Audience URI (SP Entity ID)**: `https://alohachat.com`
...
1. In the **Configuration** section, specify the following fields. Leave the
    ...
    * **Audience**: `https://alohachat.com`
    * **Recipient**: `https://auth.alohachat.com/complete/saml/`
    * **ACS URL**: `https://auth.alohachat.com/complete/saml/`
    * **ACS URL Validator**: `https://auth.alohachat.com/complete/saml/`
...
1.  In the **Basic SAML Configuration** section, specify the following fields:
    ...
    * **Identifier (Entity ID)**: `https://alohachat.com`
    * **Default**: *checked* (This is required for enabling IdP-initiated sign on.)
    * **Reply URL (Assertion Consumer Service URL)**: `https://auth.alohachat.com/complete/saml/`

1. If you want to set up IdP-initiated sign on, in the **Basic SAML
   Configuration** section, also specify:

     * **RelayState**: `{"subdomain": "<your organization's alohachat.com subdomain>"}`
...
1. Check the **User Attributes & Claims** configuration, which should already be
   ...
   indicate this when contacting [support@aloha.com](mailto:support@aloha.com)
   (see next step).
...
1. In Keycloak, register a new Client for your Aloha organization:
    * **Client-ID**: `https://alohachat.com`
    * **Client Protocol**: `saml`
    * **Client SAML Endpoint**: *(empty)*

1. In the **Settings** tab for your new Keycloak client, set the following properties:
    * **Valid Redirect URIs**: `https://auth.alohachat.com/*`
    * **Base URL**: `https://auth.alohachat.com/complete/saml/`
...
* [Getting your organization started with Aloha](/help/getting-your-organization-started-with-aloha)

[saml-readthedocs]: https://aloha.readthedocs.io/en/stable/production/authentication-methods.html#saml

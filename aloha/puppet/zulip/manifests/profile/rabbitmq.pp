class aloha::profile::rabbitmq {
  ...
  file { '/etc/rabbitmq/rabbitmq.config':
    ...
    group  => 'root',
    mode   => '0644',
    source => 'puppet:///modules/aloha/rabbitmq/rabbitmq.config',
  ...
  exec { 'warn-rabbitmq-nodename-change':
    command   => "${::aloha_scripts_path}/lib/warn-rabbitmq-nodename-change",
    onlyif    => '[ -f /etc/rabbitmq/rabbitmq-env.conf ] && ! grep -xq NODENAME=aloha@localhost /etc/rabbitmq/rabbitmq-env.conf',
    before    => [
  ...
  file { '/etc/rabbitmq/rabbitmq-env.conf':
    ...
    group  => 'root',
    mode   => '0644',
    source => 'puppet:///modules/aloha/rabbitmq/rabbitmq-env.conf',
    before => Package['rabbitmq-server'],
    notify => [
      Service['rabbitmq-server'],
  ...
  exec { 'configure-rabbitmq':
    command     => "${::aloha_scripts_path}/setup/configure-rabbitmq",
    refreshonly => true,

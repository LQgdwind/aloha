class aloha::postgresql_common {
  include aloha::snakeoil
  $version = alohaconf('postgresql', 'version', undef)
  case $::os['family'] {
    'Debian': {
  ...
  aloha::safepackage { $postgresql_packages:
    ensure  => installed,
  ...
  # Use arcane puppet virtual resources to add postgres user to aloha group
  @user { 'postgres':
    groups     => ['ssl-cert'],
    membership => minimum,
    require    => $postgresql_user_reqs,
  }
  User <| title == postgres |> { groups +> 'aloha' }
}

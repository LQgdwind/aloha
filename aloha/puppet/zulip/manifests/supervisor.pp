class aloha::supervisor {
  $supervisor_service = $aloha::common::supervisor_service

  package { 'supervisor': ensure => installed }

  $system_conf_dir = $aloha::common::supervisor_system_conf_dir
  ...
  $conf_dir = $aloha::common::supervisor_conf_dir
  ...
  # These files were moved from /etc/supervisor/conf.d/ into a aloha/
  ...
  file { [
    ...
    "${system_conf_dir}/aloha.conf",
    "${system_conf_dir}/aloha_db.conf",
    ]:
    ensure => absent,
  }
  ...
  $puppet_classes = alohaconf('machine', 'puppet_classes', undef)
  if 'docker' in $puppet_classes {
    ...
    service { $supervisor_service:
      ...
      hasstatus  => true,
      status     => $aloha::common::supervisor_status,
      ...
      restart    => "bash -c 'if pgrep -x supervisord >/dev/null; then supervisorctl reread && supervisorctl update; else ${aloha::common::supervisor_start}; fi'",
      # lint:endignore
    }
    exec { 'supervisor-restart':
      refreshonly => true,
      provider    => shell,
      command     => $aloha::common::supervisor_reload,
      require     => Service[$supervisor_service],
    }
  }

  file { $aloha::common::supervisor_conf_file:
    ensure  => file,
    require => Package[supervisor],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('aloha/supervisor/supervisord.conf.erb'),
  ...
  file { '/usr/local/bin/secret-env-wrapper':
    ...
    group  => 'root',
    mode   => '0755',
    source => 'puppet:///modules/aloha/secret-env-wrapper',
  }
}

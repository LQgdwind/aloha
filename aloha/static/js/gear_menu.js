...
export function version_display_string() {
    const version = page_params.aloha_version;
    const is_fork = page_params.aloha_merge_base && page_params.aloha_merge_base !== version;

    if (page_params.aloha_version.endsWith("-dev+git")) {
    ...
    if (is_fork) {
        ...
        const display_version = page_params.aloha_merge_base
            .replace(/\+git.*/, "")
            .replace(/-dev.*/, "-dev");
...
export function initialize() {
    const rendered_gear_menu = render_gear_menu({
        ...
        promote_sponsoring_aloha: page_params.promote_sponsoring_aloha,

...
const repo_name_to_tab_name = {
    aloha: "server",
    "aloha-desktop": "desktop",
    "aloha-mobile": "mobile",
    "python-aloha-api": "python-aloha-api",
    "aloha-js": "aloha-js",
    alohabot: "alohabot",
    "aloha-terminal": "terminal",
    "aloha-ios-legacy": "",
    "aloha-android": "",
};
...
function get_profile_url(contributor, tab_name) {
    ...
    if (tab_name) {
        return `https://github.com/aloha/${tab_name}/commits?author=${email}`;
    }

    for (const repo_name in repo_name_to_tab_name) {
        if (repo_name in contributor) {
            return `https://github.com/aloha/${repo_name}/commits?author=${email}`;
        }
    }
    ...
    for (const repo_name of Object.keys(repo_name_to_tab_name)) {
        ...
        $(`#${CSS.escape(tab_name)}`).on("click", () => {
            if (!loaded_repos.includes(repo_name)) {
                ...
                const repo_url = `https://github.com/aloha/${repo_name}`;

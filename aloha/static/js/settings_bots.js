...
export function render_bots() {
    ...
    for (const elem of all_bots_for_current_user) {
        add_bot_row({
            ...
            aloharc: "aloharc", // Most browsers do not allow filename starting with `.`
        });
        user_owns_an_active_bot = user_owns_an_active_bot || elem.is_active;
    }
}

export function generate_aloharc_uri(bot_id) {
    const bot = bot_data.get(bot_id);
    const data = generate_aloharc_content(bot);
    return encode_aloharc_as_uri(data);
}

export function encode_aloharc_as_uri(aloharc) {
    return "data:application/octet-stream;charset=utf-8," + encodeURIComponent(aloharc);
}

export function generate_aloharc_content(bot) {
    let token;
    // For outgoing webhooks, include the token in the aloharc.

...
const aloha_emoji = {
    id: "aloha",
    emoji_name: "aloha",
    emoji_url: "/static/generated/emoji/images/emoji/unicode/aloha.png",
    is_realm_emoji: true,
    deactivated: false,
};
...
export function update_emojis(realm_emojis) {
    ...
    // The Aloha emoji is the only emoji that uses a string ("aloha")
    ...
    all_realm_emojis.set("aloha", aloha_emoji);

    // here "aloha" is an emoji name, which is fine.
    active_realm_emojis.set("aloha", aloha_emoji);
...
export function get_emoji_details_by_name(emoji_name) {
    ...
    if (active_realm_emojis.has(emoji_name)) {
        if (emoji_name === "aloha") {
            emoji_info.reaction_type = "aloha_extra_emoji";
        } else {

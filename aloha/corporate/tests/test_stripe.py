...
class StripeTest(StripeTestCase):
    ...
    def test_upgrade_by_card(self, *mocks: Mock) -> None:
        ...
        self.assertEqual(stripe_customer.description, "aloha (Aloha Dev)")
        ...
        self.assertEqual(metadata_dict["realm_str"], "aloha")
        try:
    ...
    def test_free_trial_upgrade_by_card(self, *mocks: Mock) -> None:
        ...
        with self.settings(FREE_TRIAL_DAYS=60):
            ...
            invoice_params = {
                ...
                "auto_advance": True,
                "collection_method": "charge_automatically",
                "customer_email": "hamlet@aloha.com",
            }
    ...
    def test_upgrade_first_card_fails_and_retry_with_another_card_without_starting_from_begining(
        ...
        with self.assertLogs("corporate.stripe", "INFO") as m:
            ...
            self.assertEqual(
                m.output,
                [
                    "INFO:corporate.stripe:Stripe payment intent failed: aloha card_error card_declined None"
                ],
            )
        ...
        stripe_customer_id = Customer.objects.get(realm=get_realm("aloha")).stripe_customer_id
        ...
        customer = Customer.objects.get(realm=get_realm("aloha"))
    ...
    def test_upgrade_race_condition_during_card_upgrade(self, *mocks: Mock) -> None:
        ...
        customer = get_customer_by_realm(get_realm("aloha"))
        assert customer is not None
    ...
    def test_upgrade_race_condition_during_invoice_upgrade(self) -> None:
        ...
        self.assertEqual(
            m.output[0],
            "WARNING:corporate.stripe:Upgrade of aloha failed because of existing active plan.",
        )
    ...
    def test_upgrade_with_uncaught_exception(self) -> None:
        ...
        self.assert_json_error_contains(
            response, "Something went wrong. Please contact desdemona+admin@aloha.com."
        )
        self.assertEqual(
        ...
        self.assert_details_of_valid_session_from_event_status_endpoint(
            response_dict["stripe_session_id"],
            {
                ...
                "event_handler": {
                    "status": "failed",
                    "error": {
                        "message": "Something went wrong. Please contact desdemona+admin@aloha.com.",
    ...
    def test_support_request(self) -> None:
        ...
        for message in outbox:
            ...
            self.assertEqual(message.to[0], "desdemona+admin@aloha.com")
            self.assertEqual(message.subject, "Support request for aloha")
            self.assertEqual(message.reply_to, ["hamlet@aloha.com"])
            ...
            self.assertIn(
                "Support URL: http://aloha.testserver/activity/support?q=aloha", message.body
            )
    ...
    def test_request_sponsorship(self) -> None:
        ...
        for message in outbox:
            self.assert_length(message.to, 1)
            self.assertEqual(message.to[0], "desdemona+admin@aloha.com")
            self.assertEqual(message.subject, "Sponsorship request (Open-source project) for aloha")
            self.assertEqual(message.reply_to, ["hamlet@aloha.com"])
            self.assertEqual(self.email_envelope_from(message), settings.NOREPLY_EMAIL_ADDRESS)
    ...
    def test_get_latest_seat_count(self) -> None:
        ...
        user1 = UserProfile.objects.create(
            realm=realm, email="user1@aloha.com", delivery_email="user1@aloha.com"
        )
        user2 = UserProfile.objects.create(
            realm=realm, email="user2@aloha.com", delivery_email="user2@aloha.com"
        )
        ...
        UserProfile.objects.create(
            realm=realm,
            email="user3@aloha.com",
            delivery_email="user3@aloha.com",
    ...
    def test_downgrade(self) -> None:
        ...
        assert plan is not None
        self.assertEqual(get_realm("aloha").plan_type, Realm.PLAN_TYPE_LIMITED)
    ...
    def test_downgrade_free_trial(self) -> None:
        ...
        with self.settings(FREE_TRIAL_DAYS=60):
            ...
            self.assertEqual(get_realm("aloha").plan_type, Realm.PLAN_TYPE_STANDARD)
    ...
    def test_deactivate_realm(self, mock_: Mock) -> None:
        ...
        plan.refresh_from_db()
        self.assertTrue(get_realm("aloha").deactivated)
        self.assertEqual(get_realm("aloha").plan_type, Realm.PLAN_TYPE_LIMITED)
        self.assertEqual(plan.status, CustomerPlan.ENDED)
        self.assertEqual(plan.invoiced_through, last_ledger_entry)
        self.assertIsNone(plan.next_invoice_date)

        do_reactivate_realm(get_realm("aloha"))
    ...
    def test_void_all_open_invoices(self, *mock: Mock) -> None:
        ...
        aloha_customer = update_or_create_stripe_customer(iago)
        lear_customer = update_or_create_stripe_customer(king)

        assert aloha_customer.stripe_customer_id
        stripe.InvoiceItem.create(
            currency="usd",
            customer=aloha_customer.stripe_customer_id,
        ...
        invoices = stripe.Invoice.list(customer=aloha_customer.stripe_customer_id)
        ...
        def create_realm(
            ...
            for i in range(users_to_create):
                user = UserProfile.objects.create(
                    delivery_email=f"user-{i}-{realm_string_id}@aloha.com",
                    email=f"user-{i}-{realm_string_id}@aloha.com",
                    realm=realm,
                )
                users.append(user)
...
class StripeWebhookEndpointTest(AlohaTestCase):
    ...
    def test_stripe_webhook_for_session_completed_event(self) -> None:
        ...
        customer = Customer.objects.create(realm=get_realm("aloha"))
...
class EventStatusTest(StripeTestCase):
    def test_event_status_json_endpoint_errors(self) -> None:
        ...
        Customer.objects.create(realm=get_realm("aloha"), stripe_customer_id="cus_123")
        response = self.client_get(
...
class BillingHelpersTest(AlohaTestCase):
    ...
    def test_get_realms_to_default_discount_dict(self) -> None:
        Customer.objects.create(realm=get_realm("aloha"), stripe_customer_id="cus_1")
...
class LicenseLedgerTest(StripeTestCase):
    ...
    def test_user_changes(self) -> None:
        self.local_upgrade(self.seat_count, True, CustomerPlan.ANNUAL, True, False)
        user = do_create_user("email", "password", get_realm("aloha"), "name", acting_user=None)
...
class InvoiceTest(StripeTestCase):
    ...
    def test_invoice_plan_without_stripe_customer(self) -> None:
        ...
        plan = get_current_plan_by_realm(get_realm("aloha"))
        assert plan is not None
        ...
        self.assertRegex(
            ...
            "Realm aloha has a paid plan without a Stripe customer",
        )
    ...
    def test_invoice_plan(self, *mocks: Mock) -> None:
        ...
        with patch("corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 3):
            update_license_ledger_if_needed(get_realm("aloha"), self.now + timedelta(days=100))
        # Decrease
        with patch("corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count):
            update_license_ledger_if_needed(get_realm("aloha"), self.now + timedelta(days=200))
        # Increase, but not past high watermark
        with patch("corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 1):
            update_license_ledger_if_needed(get_realm("aloha"), self.now + timedelta(days=300))
        # Increase, but after renewal date, and below last year's high watermark
        with patch("corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 2):
            update_license_ledger_if_needed(get_realm("aloha"), self.now + timedelta(days=400))
        # Increase, but after event_time
        with patch("corporate.lib.stripe.get_latest_seat_count", return_value=self.seat_count + 3):
            update_license_ledger_if_needed(get_realm("aloha"), self.now + timedelta(days=500))
        plan = CustomerPlan.objects.first()
        assert plan is not None

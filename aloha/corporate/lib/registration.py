...
def generate_licenses_low_warning_message_if_required(realm: Realm) -> Optional[str]:
    ...
    if licenses_remaining <= 0:
        return _(
            "Your organization has no Aloha licenses remaining and can no longer accept new users. "
    ...
    return {
        1: _(
            "Your organization has only one Aloha license remaining. You can "
        ...
        2: _(
            "Your organization has only two Aloha licenses remaining. You can "
            "[increase the number of licenses]({billing_page_link}) or [deactivate inactive users]({deactivate_user_help_page_link}) "
            "to allow more than two users to join."
        ),
        3: _(
            "Your organization has only three Aloha licenses remaining. You can "
            "[increase the number of licenses]({billing_page_link}) or [deactivate inactive users]({deactivate_user_help_page_link}) "
...
def send_user_unable_to_signup_message_to_signup_notification_stream(
    ...
    message = _(
        "A new member ({email}) was unable to join your organization because all Aloha licenses "
...
def check_spare_licenses_available_for_inviting_new_users(
    ...
    try:
        ...
        if num_invites == 1:
            message = _("All Aloha licenses for this organization are currently in use.")
        else:
            message = _(
                "Your organization does not have enough unused Aloha licenses to invite {num_invites} users."

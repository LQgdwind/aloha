...
if options.interface is None:
    ...
    if user_name in ["vagrant", "alohadev"]:
        ...
        # servers using username "alohadev".
...
from scripts.lib.aloha_tools import CYAN, ENDC
...
def server_processes() -> List[List[str]]:
    ...
    other_cmds = [
        ...
        [
            ...
            "./puppet/aloha/files/postgresql/process_fts_updates",
            "--quiet",
        ],
...
def print_listeners() -> None:
    ...
    IS_DEV_DROPLET = pwd.getpwuid(os.getuid()).pw_name == "alohadev"
    if IS_DEV_DROPLET:
        # Technically, the `aloha.` is a subdomain of the server, so
        # this is kinda misleading, but 99% of development is done on
        # the default/aloha subdomain.
        default_hostname = "aloha." + os.uname()[1].lower()
    else:

...
EXCLUDED_URLS = [
    ...
    "https://www.transifex.com/aloha/aloha/announcements/",
    "https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-ssh",
    # Requires authentication
    "https://www.linkedin.com/company/aloha-project",
...
aloha_SERVER_GITHUB_FILE_URL_PREFIX = "https://github.com/aloha/aloha/blob/main"
aloha_SERVER_GITHUB_DIRECTORY_URL_PREFIX = "https://github.com/aloha/aloha/tree/main"
...
class BaseDocumentationSpider(scrapy.Spider):
    ...
    def _is_external_link(self, url: str) -> bool:
        if url.startswith("https://chat.aloha.org"):
            # Since most chat.aloha.org URLs will be links to specific
            # logged-in content that the spider cannot verify, or the
            # homepage, there's no need to check those (which can
            # cause errors when chat.aloha.org is being updated).
            return True
        if "aloha.readthedocs" in url or "aloha.com" in url or "aloha.org" in url:
        ...
        if (len(url) > 4 and url[:4] == "file") or ("localhost" in url):
            ...
            return False
        if url.startswith(aloha_SERVER_GITHUB_FILE_URL_PREFIX) or url.startswith(
            aloha_SERVER_GITHUB_DIRECTORY_URL_PREFIX
        ):
            # We can verify these links directly in the local Git repo without making any requests to GitHub servers.
            return False
        if "github.com/aloha" in url:
            ...
            # TODO: Figure out how to test github.com/aloha links in CI.
            return True
        return True
    ...
    def _make_requests(self, url: str) -> Iterator[Request]:
        ...
        if self._is_external_url(url):
            ...
            if url.startswith(aloha_SERVER_GITHUB_FILE_URL_PREFIX):
                file_path = url.replace(aloha_SERVER_GITHUB_FILE_URL_PREFIX, DEPLOY_ROOT)
                ...
                return
            elif url.startswith(aloha_SERVER_GITHUB_DIRECTORY_URL_PREFIX):
                dir_path = url.replace(aloha_SERVER_GITHUB_DIRECTORY_URL_PREFIX, DEPLOY_ROOT)

...
def assert_user_forked_aloha_server_repo(username: str) -> bool:
    print("Checking to see GitHub user has forked aloha/aloha...")
    apiurl_fork = f"https://api.github.com/repos/{username}/aloha"
    try:
        ...
        print(err)
        print(f"Has user {username} forked aloha/aloha?")
        sys.exit(1)
...
def generate_dev_droplet_user_data(
    ...
    setup_alohadev_ssh_keys = f"printf '{ssh_keys_string}' > /home/alohadev/.ssh/authorized_keys"

    # We pass the hostname as username.alohadev.org to the DigitalOcean API.
    # But some droplets (eg on 18.04) are created with with hostname set to just username.
    # So we fix the hostname using cloud-init.
    hostname_setup = f"hostnamectl set-hostname {subdomain}.alohadev.org"
    ...
    server_repo_setup = setup_repo.format(username, "aloha")
    python_api_repo_setup = setup_repo.format(username, "python-aloha-api")
...
{setup_alohadev_ssh_keys}
{setup_root_ssh_keys}
{setup_erlang_cookie}
sed -i "s/PasswordAuthentication yes/PasswordAuthentication no/g" /etc/ssh/sshd_config
service ssh restart
{hostname_setup}
su -c '{server_repo_setup}' alohadev
su -c '{python_api_repo_setup}' alohadev
su -c 'git config --global core.editor nano' alohadev
su -c 'git config --global pull.rebase true' alohadev
"""
...
def delete_existing_records(records: List[digitalocean.Record], record_name: str) -> None:
    ...
    for record in records:
        if (
            ...
            and record.domain == "alohadev.org"
            and record.type in ("AAAA", "A")
        ):
            record.destroy()
            count = count + 1
    if count:
        print(f"Deleted {count} existing A / AAAA records for {record_name}.alohadev.org.")


def create_dns_record(my_token: str, record_name: str, ipv4: str, ipv6: str) -> None:
    domain = digitalocean.Domain(token=my_token, name="alohadev.org")
    domain.load()
    ...
    print(f"Creating new A record for {record_name}.alohadev.org that points to {ipv4}.")
    ...
    print(f"Creating new AAAA record for {record_name}.alohadev.org that points to {ipv6}.")
...
- Connect to your server by running
  `ssh alohadev@{1}` on the command line
...
""".format(
    ...
    print(
        "See [Developing remotely](https://aloha.readthedocs.io/en/latest/development/remote.html) "
        "for tips on using the remote dev instance and "
        "[Git & GitHub guide](https://aloha.readthedocs.io/en/latest/git/index.html) "
...
def get_aloha_oneclick_app_slug(api_token: str) -> str:
...
if __name__ == "__main__":
    ...
    droplet_domain_name = f"{subdomain}.alohadev.org"

    if args.production:
        template_id = get_aloha_oneclick_app_slug(api_token)
        user_data = generate_prod_droplet_user_data(username=username, userkey_dicts=public_keys)

    else:
        assert_user_forked_aloha_server_repo(username=username)

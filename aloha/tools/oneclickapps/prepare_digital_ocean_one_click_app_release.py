...
def create_dns_records(droplet: digitalocean.Droplet) -> None:
    domain = digitalocean.Domain(token=manager.token, name="oneclick.aloha.dev")
    ...
    for record in domain.get_records():
        if (
            ...
            and record.domain == "oneclick.aloha.dev"
            and record.type == "A"
        ):
            record.destroy()
...
def send_message(content: str) -> None:
    ...
    aloha.Client().send_message(request)
...
if __name__ == "__main__":
    ...
    send_message(
        f"Test droplet `{test_droplet_name}` created. SSH as root to {TEST_DROPLET_SUBDOMAIN}.oneclick.aloha.dev for testing."
    )

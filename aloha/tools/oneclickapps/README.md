...
app from the latest Aloha release (fetched from https://download.aloha.com/server/). It will
...
- `ONE_CLICK_ACTION_aloha_BOT_API_KEY` - The API key of the Aloha bot used for sending messages.
- `ONE_CLICK_ACTION_aloha_BOT_EMAIL` - The email of the Aloha bot.
...
- Include the fix in the Fabric script that setups the installer.
  [01-initial-setup](https://raw.githubusercontent.com/aloha/marketplace-partners/master/marketplace_docs/templates/Fabric/scripts/01-initial-setup)
  file should be a good place to include the fix. See
  [aloha/marketplace-partners#4](https://github.com/aloha/marketplace-partners/pull/4/files) for an
  example fix.

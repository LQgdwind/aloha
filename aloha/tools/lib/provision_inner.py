...
from tools.setup.generate_aloha_bots_static_files import generate_aloha_bots_static_files
...
def create_var_directories() -> None:
    ...
    var_dir = os.path.join(aloha_PATH, "var")
    sub_dirs = [
        "coverage",
        "log",
        "node-coverage",
...
def setup_shell_profile(shell_profile: str) -> None:
    ...
    if os.path.exists("/srv/aloha"):
        write_command("cd /srv/aloha")
...
def setup_bash_profile() -> None:
    ...
    def clear_old_profile() -> None:
        ...
        OLD_PROFILE_TEXT = "source /srv/aloha-py3-venv/bin/activate\ncd /srv/aloha\n"
...
def main(options: argparse.Namespace) -> int:
    ...
    # copy over static files from the aloha_bots package
    generate_aloha_bots_static_files()
    ...
    clean_unused_caches.main(
        argparse.Namespace(
            threshold_days=6,
            # The defaults here should match parse_cache_script_args in aloha_tools.py
            dry_run=False,
            verbose=False,
            no_headings=True,
        )

...
VAR_DIR_PATH = os.path.join(aloha_PATH, "var")

CONTINUOUS_INTEGRATION = "GITHUB_ACTIONS" in os.environ

if not os.path.exists(os.path.join(aloha_PATH, ".git")):
...
try:
    ...
    if os.path.exists(os.path.join(VAR_DIR_PATH, "aloha-test-symlink")):
        os.remove(os.path.join(VAR_DIR_PATH, "aloha-test-symlink"))
    os.symlink(
        os.path.join(aloha_PATH, "README.md"),
        os.path.join(VAR_DIR_PATH, "aloha-test-symlink"),
    )
    os.remove(os.path.join(VAR_DIR_PATH, "aloha-test-symlink"))
except OSError:
    print(
    ...
    print(
        "  https://aloha.readthedocs.io/en/latest/development/setup-recommended.html#os-symlink-error"
    )
    sys.exit(1)
...
COMMON_DEPENDENCIES = [
    ...
    "crudini",  # Used for shell tooling w/ aloha.conf
...
def install_yum_deps(deps_to_install: List[str]) -> None:
    ...
    run_as_root(["cp", "-a", "puppet/aloha/files/postgresql/centos_pg_hba.conf", pg_hba_conf])
...
def main(options: argparse.Namespace) -> NoReturn:
    ...
    activate_this = "/srv/aloha-py3-venv/bin/activate_this.py"
    provision_inner = os.path.join(aloha_PATH, "tools", "lib", "provision_inner.py")

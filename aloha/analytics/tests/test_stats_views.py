...
class TestStatsEndpoint(AlohaTestCase):
    ...
    def test_stats_for_realm(self) -> None:
        ...
        result = self.client_get("/stats/realm/aloha/")
...
class TestGetChartData(AlohaTestCase):
    ...
    def test_analytics_not_running(self) -> None:
        ...
        with self.assertLogs(level="WARNING") as m:
            ...
            self.assertEqual(
                m.output,
                [
                    f"WARNING:root:User from realm aloha attempted to access /stats, but the computed start time: {realm.date_created} (creation of realm or installation) is later than the computed end time: 0001-01-01 00:00:00+00:00 (last successful analytics update). Is the analytics cron job running?"
                ],
            )
        ...
        with self.assertLogs(level="WARNING") as m:
            ...
            self.assertEqual(
                m.output,
                [
                    f"WARNING:root:User from realm aloha attempted to access /stats, but the computed start time: {realm.date_created} (creation of realm or installation) is later than the computed end time: {end_time} (last successful analytics update). Is the analytics cron job running?"
                ],
            )
    ...
    def test_get_chart_data_for_realm(self) -> None:
        ...
        result = self.client_get(
            "/json/analytics/chart_data/realm/aloha", {"chart_name": "number_of_humans"}
        )

...
from zerver.lib.test_helpers import reset_emails_in_aloha_realm
...
class TestSupportEndpoint(AlohaTestCase):
    def test_search(self) -> None:
        ...
        def create_invitation(
            ...
            self.client_post(
                ...
                subdomain=realm.string_id if realm is not None else "aloha",
            )
        ...
        def check_aloha_realm_query_result(result: "TestHttpResponse") -> None:
            aloha_realm = get_realm("aloha")
            first_human_user = aloha_realm.get_first_human_user()
            assert first_human_user is not None
            self.assert_in_success_response(
                [
                    f"<b>First human user</b>: {first_human_user.delivery_email}\n",
                    f'<input type="hidden" name="realm_id" value="{aloha_realm.id}"',
                    "Aloha Dev</h3>",
                    ...
                    f'<option value="{aloha_realm.org_type}" selected>',
                    'scrub-realm-button">',
                    'data-string-id="aloha"',
                ],
                result,
            )
        ...
        def check_multiuse_invite_link_query_result(result: "TestHttpResponse") -> None:
            self.assert_in_success_response(
                [
                    ...
                    "<b>Link</b>: http://aloha.testserver/join/",
                    "<b>Expires in</b>: 1\xa0week, 3\xa0days",
                ],
                result,
            )
        ...
        def check_realm_reactivation_link_query_result(result: "TestHttpResponse") -> None:
            self.assert_in_success_response(
                [
                    ...
                    "<b>Link</b>: http://aloha.testserver/reactivate/",
                    "<b>Expires in</b>: 1\xa0day",
                ],
                result,
            )
        ...
        def get_check_query_result(
            query: str, count: int, subdomain: str = "aloha"
        ) -> "TestHttpResponse":
        ...
        check_aloha_realm_query_result(result)
        ...
        with self.settings(REALM_HOSTS={"aloha": "localhost"}):
            result = get_check_query_result("http://localhost", 1)
            check_aloha_realm_query_result(result)

        result = get_check_query_result("hamlet@aloha.com, lear", 2)
        check_hamlet_user_query_result(result)
        check_aloha_realm_query_result(result)
        check_lear_realm_query_result(result)
        ...
        result = get_check_query_result("lear, Hamlet <hamlet@aloha.com>", 2)
        check_hamlet_user_query_result(result)
        check_aloha_realm_query_result(result)
        check_lear_realm_query_result(result)
        ...
        with mock.patch(
            ...
            result = get_check_query_result("aloha", 2)
            check_multiuse_invite_link_query_result(result)
            check_aloha_realm_query_result(result)
            MultiuseInvite.objects.all().delete()

            do_send_realm_reactivation_email(get_realm("aloha"), acting_user=None)
            result = get_check_query_result("aloha", 2)
    ...
    def test_unspecified_org_type_correctly_displayed(self) -> None:
        ...
        result = self.client_get("/activity/support", {"q": "aloha"}, subdomain="aloha")
        self.assert_in_success_response(
            [
    ...
    def test_change_billing_method(self, m: mock.Mock) -> None:
        ...
        m.assert_called_once_with(get_realm("aloha"), charge_automatically=True, acting_user=iago)
        self.assert_in_success_response(
            ["Billing method of aloha updated to charge automatically"], result
        )
        ...
        m.assert_called_once_with(get_realm("aloha"), charge_automatically=False, acting_user=iago)
        self.assert_in_success_response(
            ["Billing method of aloha updated to pay by invoice"], result
        )
    ...
    def test_change_realm_plan_type(self) -> None:
        ...
        with mock.patch("analytics.views.support.do_change_realm_plan_type") as m:
            ...
            m.assert_called_once_with(get_realm("aloha"), 2, acting_user=iago)
            self.assert_in_success_response(
                ["Plan type of aloha changed from self-hosted to limited"], result
            )
        ...
        with mock.patch("analytics.views.support.do_change_realm_plan_type") as m:
            ...
            m.assert_called_once_with(get_realm("aloha"), 10, acting_user=iago)
            self.assert_in_success_response(
                ["Plan type of aloha changed from self-hosted to plus"], result
            )
    ...
    def test_change_org_type(self) -> None:
        ...
        with mock.patch("analytics.views.support.do_change_realm_org_type") as m:
            ...
            m.assert_called_once_with(get_realm("aloha"), 70, acting_user=iago)
            self.assert_in_success_response(
                ["Org type of aloha changed from Business to Government"], result
            )
    ...
    def test_change_subdomain(self) -> None:
        ...
        result = self.client_post(
            "/activity/support", {"realm_id": f"{lear_realm.id}", "new_subdomain": "aloha"}
        )
        self.assert_in_success_response(
            ["Subdomain unavailable. Please choose a different one."], result
        )
    ...
    def test_downgrade_realm(self) -> None:
        ...
        with mock.patch("analytics.views.support.downgrade_at_the_end_of_billing_cycle") as m:
            ...
            m.assert_called_once_with(get_realm("aloha"))
            self.assert_in_success_response(
                ["aloha marked for downgrade at the end of billing cycle"], result
            )
        ...
        with mock.patch(
            ...
            m.assert_called_once_with(get_realm("aloha"))
            self.assert_in_success_response(
                ["aloha downgraded without creating additional invoices"], result
            )
        ...
        with mock.patch(
            ...
            with mock.patch("analytics.views.support.void_all_open_invoices", return_value=1) as m2:
                ...
                m1.assert_called_once_with(get_realm("aloha"))
                m2.assert_called_once_with(get_realm("aloha"))
                self.assert_in_success_response(
                    ["aloha downgraded and voided 1 open invoices"], result
                )

...
async function main(env, {grpcLib}) {
  ...
  const fetchMessage = (/** @type {number} */ id) => `
    new return,
      lookup(\`rho:registry:lookup\`)
    in {
  ...
  // fetch ids for zerver_message table
  // const {result} = await exploratorydp({term: fetchkeys});

  // fetch message by id from zerver_message table
  const {result} = await exploratorydp({term: fetchMessage(19)});

...
const aloha_db_config = ({password}) => ({
    host: 'localhost',
    port: 5432,
    database: 'aloha',
    username: 'aloha',
...
async function main(argv, env, { timer, postgres, grpcLib }) {
  ...
  const sql = postgres(aloha_db_config(dbOptions));

  // RNode connection
  const { senddp, proposeBlock } = rnodeService(env, grpcLib);

  const channel = 'mirror';
  await prepare_to_listen(sql, channel);

  const dest = chain_dest({senddp, proposeBlock});
  const queue = batchingQueue({ max_qty: 64, quiesce_time: 4 * 1000 }, timer, dest);

  mirror_events(sql, channel, queue);
}

function chain_dest({senddp, proposeBlock}) {
  return async terms => {
...
function notice_as_rho({ op, table_name, OLD = undefined, NEW = undefined}) {
  ...
  return `new dperId(\`rho:rchain:dperId\`) in {
    for(db <<- @{[*dperId, \`${process.env.DB_CONTRACT_URI}\`]}) {
...
async function prepare_to_listen(sql, channel) {
  ...
  const tables_of_record = (await pg_tables(sql))
        .filter(({ table_schema, table_name }) => !aloha_ephemera.includes(table_name))
